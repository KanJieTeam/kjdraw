...
test('workbench creates ghosted native leaders and edits the owned annotation', async ({ page }) => {
  ...
  await dialog.locator('[data-draft-option="styleId"]').selectOption({ label: 'LEADER-CJK' }); await dialog.locator('[data-draft-option="textHeight"]').fill('4'); await dialog.locator('[data-action="start-draft"]').click()
  ...
  await expect.poll(() => page.evaluate(() => window.__leaderPreviews.some(item => item.type === 'LEADER' && item.text === '泵出口压力表 PI-101'))).toBe(true)
...
test('playground exposes real leader controls and saves the native pair', async ({ page }) => {
  ...
  await page.locator('#leader-style').selectOption({ label: 'NOTES' }); await page.locator('#leader-height').fill('4'); await page.locator('#leader-arrow').check()
  ...
  expect(note).toMatchObject({ type: 'MTEXT', payload: { text: '设备检修空间', height: 4 } }); expect(session.activeDocument.getObject(note.payload.styleId).name).toBe('NOTES')
  session.destroy()
})
