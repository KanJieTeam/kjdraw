...
async function open(page){
 ...
 if(!(await page.locator('#ai-chat-window').isVisible()))await page.locator('#ai-assistant-launcher').click()
...
test('attachment rejection, removal, late file reads and document changes remain bounded at narrow Chinese width',async({page})=>{
 ...
 await page.reload();await expect(page.locator('.workbench')).toHaveAttribute('data-demo-state','ready');if(!(await page.locator('#ai-chat-window').isVisible()))await page.locator('#ai-assistant-launcher').click()
 if(await page.locator('#language').textContent()!=='EN')await page.locator('#language').click()
 await page.setViewportSize({width:390,height:844});await expect(page.locator('#ai-chat-window')).toBeVisible()
 ...
 const overflow=await page.locator('#ai-chat-window').evaluate(node=>node.scrollWidth-node.clientWidth);expect(overflow).toBeLessThanOrEqual(1)
