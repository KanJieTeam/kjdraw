...
test('chat queries the real drawing, previews native geometry, applies once, saves and undoes', async ({ page }) => {
  ...
  await page.route('**/api/model', async route => {
    const body = route.request().postDataJSON(); requests.push(body)
    expect(body.tools.map(tool => tool.function.name).sort()).toEqual(['cad_read_drawing', 'cad_read_page', 'cad_query_drawing', 'cad_read_layouts', 'cad_read_designs', 'cad_propose_design_bind', 'cad_propose_design_update', 'cad_measure_distance', 'cad_check_geometry', 'cad_propose_move', 'cad_propose_rotate', 'cad_propose_scale', 'cad_propose_stretch', 'cad_propose_lengthen', 'cad_propose_polyline_edit', 'cad_propose_drawing_pattern', 'cad_propose_drawing_annotated'].sort())
