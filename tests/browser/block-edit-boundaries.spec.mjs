...
test('Workbench makes instance and shared-definition layer edits explicit',async({page})=>{
  ...
  const scope=inspector.locator('select[data-block-scope]'),member=inspector.locator('select[data-block-member]'),layer=inspector.locator('select[data-property="layer"]')
  await expect(scope).toHaveCount(1);await expect(member).toHaveCount(1);await expect(layer).toHaveCount(1)
  await scope.selectOption('definition')
  await expect(member).toBeVisible()
  await layer.selectOption('target-layer')
  ...
  await scope.selectOption('instance')
  await layer.selectOption('target-layer')
  await inspector.locator('button.apply').click()
