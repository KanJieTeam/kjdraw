...
test('workbench resolves direct distance and angle input and keeps every draft boundary retryable', async ({ page }) => {
  ...
  const workbenchScale = await page.evaluate(() => window.draftInputWorkbench.workbench.renderer.camera.scale)
  expect(line.start).toEqual([0, 0, 0]); expect(Math.hypot(line.end[0], line.end[1])).toBeCloseTo(10, 9)
  expect(Math.abs(line.end[0] - 6)).toBeLessThanOrEqual(2 / workbenchScale); expect(Math.abs(line.end[1] - 8)).toBeLessThanOrEqual(2 / workbenchScale)
  ...
  await expect(page.locator('#draft-input-host [data-message]')).toContainText(/pointer.*(?:direction|away)/)
...
test('playground uses the same direct input and retry/cancel boundaries', async ({ page }) => {
  ...
  const playgroundScale = await page.evaluate(() => window.draftInputRenderer.camera.scale)
  expect(Math.hypot(line.end[0], line.end[1])).toBeCloseTo(10, 9)
  expect(Math.abs(line.end[0] - 6)).toBeLessThanOrEqual(2 / playgroundScale); expect(Math.abs(line.end[1] - 8)).toBeLessThanOrEqual(2 / playgroundScale)
