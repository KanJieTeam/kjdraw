...
async function screenPoint(page, world) {
  return page.evaluate(world => {
    const workbench = window.splineCircleIntersection.workbench
...
test('pointer drawing commits the native rational SPLINE and CIRCLE intersection', async ({ page }) => {
  await page.goto('/')
  await page.evaluate(async () => {
    document.body.replaceChildren()
    const host = document.createElement('div'); host.id = 'spline-circle-host'; host.style.cssText = 'width:1000px;height:700px'; document.body.append(host)
    ...
    const sdk = createKJDrawSDK(), drawing = sdk.createDocument({ documentId: 'spline-circle-pointer', units: 'millimeter' })
    const locked = await sdk.executeCommand('LAYERNEW', { name: 'Locked native references', locked: true }, { document: drawing })
    await drawing.transact('Native rational circular references', tx => {
      ...
      tx.createEntity('CIRCLE', { center: [5, 0, 0], radius: 7.5, layerId: locked.id }, { id: 'native-reference-circle' })
    })
    ...
    workbench.renderer.resize(); Object.assign(workbench.renderer.camera, { centerX: 4, centerY: 5, scale: 20 }); workbench.renderer.render()
    window.splineCircleIntersection = { workbench, drawing }
  })

  const expected = [6.875, Math.sqrt(100 - 6.875 ** 2)], start = [0, 4]
  const startScreen = await screenPoint(page, start), expectedScreen = await screenPoint(page, expected)
  await page.locator('#spline-circle-host [data-tool="line"]').click()
  await page.mouse.click(startScreen.x, startScreen.y)
  await page.mouse.move(expectedScreen.x + 2, expectedScreen.y - 1)
  const marker = page.locator('#spline-circle-host [data-snap]')
  ...
  await expect.poll(() => page.evaluate(() => window.splineCircleIntersection.drawing.listEntities({ type: 'LINE' }).length)).toBe(1)
  const result = await page.evaluate(() => ({
    line: window.splineCircleIntersection.drawing.listEntities({ type: 'LINE' })[0].payload,
    spline: window.splineCircleIntersection.drawing.getObject('native-rational-spline').payload,
  }))
