...
test('Playground multi-selection applies the same atomic batch layer property command',async({page})=>{
  await openFixture(page)
  const original=await save(page)
  if(!(await page.locator('.workbench').getAttribute('class')).includes('layers-open'))await page.locator('#toggle-layers').click()
  await page.locator('#new-layer').click()
  await page.locator('#dialog-fields input[name="name"]').fill('Batch target')
  await page.locator('#dialog-submit').click()
  await expect(page.locator('.layer[data-layer-name="Batch target"]')).toBeVisible()
  const withLayer=await save(page),targetLayer=Object.values(withLayer.objects).find(item=>item.kind==='table-record'&&item.type==='LAYER'&&item.name==='Batch target')
  expect(targetLayer).toBeTruthy()
  await boxSelect(page,[-2,-2],[62,8],{mode:'window'})
  ...
  expect(['line-a','line-b'].map(id=>changed.objects[id].payload.layerId)).toEqual([targetLayer.id,targetLayer.id])
  await page.locator('#undo').click()
  const undone=await save(page)
  expect(['line-a','line-b'].map(id=>undone.objects[id].payload.layerId)).toEqual(['line-a','line-b'].map(id=>original.objects[id].payload.layerId))
  await page.locator('#redo').click()
  const redone=await save(page)
  expect(['line-a','line-b'].map(id=>redone.objects[id].payload.layerId)).toEqual([targetLayer.id,targetLayer.id])
})
