...
function render() {
  ...
  if(modification){
    ...
    try{
      if(['PEDIT','BREAK'].includes(modification.definition.command)&&cursor&&translationValid(modification)){
        ...
        if(target){const directPreview=modification.definition.command==='PEDIT'?[{type:target.type,payload:editPolylinePayload(target,request.arguments)}]:breakEntityPayloads(target,request.arguments);for(const entity of directPreview)drawOverlayEntity(entity,'#77a7ff');workbench.dataset.modificationPreviewCount=String(directPreview.length)}
...
function refresh() {
  ...
  for(const layer of layers){
    ...
    const current=document.createElement('button'),isCurrent=doc().getTable('layers').currentId===layer.id;current.className='layer-current';current.textContent=isCurrent?'●':'○';current.title=`${t(isCurrent?'currentLayer':'setCurrentLayer')} · ${layer.name}`;current.setAttribute('aria-label',current.title);current.setAttribute('aria-pressed',String(isCurrent));current.disabled=isCurrent;current.onclick=()=>run(()=>execute('LAYERCURRENT',{id:layer.id}))
    ...
    edit.onclick=()=>run(async()=>{const linetypes=doc().getTable('linetypes').records,values=await requestLocalCommand({title:t('editLayer'),fields:[{name:'name',label:t('layerName'),value:layer.name},{name:'color',label:t('layerColor'),type:'number',value:layer.payload.color??7,step:1},{name:'linetypeId',label:t('layerLinetype'),value:layer.payload.linetypeId??linetypes[0]?.id,options:linetypes.map(item=>[item.id,item.name])},{name:'lineweight',label:t('layerLineweight'),value:String(layer.payload.lineweight??-1),options:LAYER_LINEWEIGHTS.map(value=>[String(value),value===-1?t('byLayer'):value===0?'0.00 mm':`${(value/100).toFixed(2)} mm`])},{name:'visible',label:t('visibleLayer'),type:'checkbox',value:layer.payload.visible!==false,required:false},{name:'locked',label:t('lockedLayer'),type:'checkbox',value:layer.payload.locked===true,required:false},{name:'frozen',label:t('freezeLayer'),type:'checkbox',value:layer.payload.frozen===true,required:false},{name:'current',label:t('setCurrentLayer'),type:'checkbox',value:isCurrent,required:false}]});if(!values)return;const color=Number(values.color);if(!Number.isInteger(color)||color<1||color>255)throw new Error(t('layerColorRange'));await execute('LAYERUPDATE',{id:layer.id,newName:values.name.trim()||layer.name,patch:{color,trueColor:null,linetypeId:values.linetypeId,lineweight:Number(values.lineweight),visible:values.visible,locked:values.locked,frozen:values.frozen},current:values.current})})
  ...
  if(entity){
    ...
    const layerLabel=document.createElement('label');layerLabel.textContent=t('layer');const layerSelect=document.createElement('select');layerSelect.dataset.property='layer';const layerIds=new Set(selectedEntities.map(item=>item.payload.layerId)),commonLayerId=layerIds.size===1?[...layerIds][0]:''
    ...
    const styleSelect=(property,labelText,values,selectedValues)=>{const label=document.createElement('label');label.textContent=labelText;const select=document.createElement('select');select.dataset.property=property;const common=selectedValues.size===1?[...selectedValues][0]:'';if(selectedValues.size!==1){const option=document.createElement('option');option.value='';option.textContent='—';option.selected=true;option.disabled=true;select.append(option)}for(const [value,text] of values){const option=document.createElement('option');option.value=value;option.textContent=text;option.selected=value===common;select.append(option)}label.append(select);editor.append(label);return{select,common}}
    const colorStyle=styleSelect('color',t('entityColor'),[['256',t('byLayer')],...Array.from({length:255},(_,index)=>[String(index+1),String(index+1)])],new Set(selectedEntities.map(item=>String(item.payload.color??256))))
    const linetypeStyle=styleSelect('linetype',t('entityLinetype'),[['',t('byLayer')],...doc().getTable('linetypes').records.map(item=>[item.id,item.name])],new Set(selectedEntities.map(item=>String(item.payload.linetypeId??''))))
    const lineweightStyle=styleSelect('lineweight',t('entityLineweight'),LAYER_LINEWEIGHTS.map(value=>[String(value),value===-1?t('byLayer'):value===0?'0.00 mm':`${(value/100).toFixed(2)} mm`]),new Set(selectedEntities.map(item=>String(item.payload.lineweight??-1))))
    ...
    const scaleValues=new Set(selectedEntities.map(item=>String(item.payload.linetypeScale??1))),commonScale=scaleValues.size===1?[...scaleValues][0]:''
    const scaleLabel=document.createElement('label');scaleLabel.textContent=t('linetypeScale');const scaleInput=document.createElement('input');scaleInput.type='number';scaleInput.min='0.000001';scaleInput.step='0.1';scaleInput.value=commonScale;scaleInput.placeholder='—';scaleInput.dataset.property='linetype-scale';scaleLabel.append(scaleInput);editor.append(scaleLabel)
    let valueInput=null
    ...
    if(selectedEntities.length===1&&entity.type==='LEADER'){const annotation=entity.payload.annotationId?doc().getObject(entity.payload.annotationId):null,labeled=(labelText,input)=>{const label=document.createElement('label');label.textContent=labelText;label.append(input);editor.append(label);return input},text=labeled(i18n.locale==='zh'?'引线文字':'Annotation text',document.createElement('textarea')),style=labeled(i18n.locale==='zh'?'文字样式':'Text style',document.createElement('select')),height=labeled(i18n.locale==='zh'?'文字高度':'Text height',document.createElement('input')),width=labeled(i18n.locale==='zh'?'文字宽度':'Text width',document.createElement('input')),rotation=labeled(i18n.locale==='zh'?'旋转（度）':'Rotation (degrees)',document.createElement('input')),attachment=labeled(i18n.locale==='zh'?'文字对齐':'Text attachment',document.createElement('select')),arrow=labeled(i18n.locale==='zh'?'显示箭头':'Arrowhead',document.createElement('input'));text.value=annotation?.payload.text??entity.payload.text??'';text.dataset.property='leader-text';for(const record of doc().getTable('textStyles').records)style.add(new Option(record.name,record.id,false,record.id===(annotation?.payload.styleId??doc().getTable('textStyles').currentId)));style.dataset.property='leader-style';height.type='number';height.required=true;height.min=String(Number.EPSILON);height.step='any';height.value=String(annotation?.payload.height??entity.payload.textHeight??2.5);height.dataset.property='leader-height';width.type='number';width.min=String(Number.EPSILON);width.step='any';width.placeholder=i18n.locale==='zh'?'自动':'Auto';width.value=annotation?.payload.width==null?'':String(annotation.payload.width);width.dataset.property='leader-width';rotation.type='number';rotation.required=true;rotation.step='any';rotation.value=String(Number(annotation?.payload.rotation??0)*180/Math.PI);rotation.dataset.property='leader-rotation';for(const [value,en,zh] of [[1,'Top left','左上'],[2,'Top center','中上'],[3,'Top right','右上'],[4,'Middle left','左中'],[5,'Middle center','居中'],[6,'Middle right','右中'],[7,'Bottom left','左下'],[8,'Bottom center','中下'],[9,'Bottom right','右下']])attachment.add(new Option(i18n.locale==='zh'?zh:en,String(value),false,Number(annotation?.payload.attachmentPoint??7)===value));attachment.dataset.property='leader-attachment';arrow.type='checkbox';arrow.checked=entity.payload.arrowEnabled!==false;arrow.dataset.property='leader-arrow';leaderFields={text,style,height,width,rotation,attachment,arrow}}
    ...
    save.onclick=()=>run(async()=>{
      ...
      if(colorStyle.select.value&&colorStyle.select.value!==colorStyle.common){payload.color=Number(colorStyle.select.value);payload.trueColor=null}
      if(linetypeStyle.select.value!==linetypeStyle.common){payload.linetypeId=linetypeStyle.select.value||null;payload.linetypeName=linetypeStyle.select.value?doc().getObject(linetypeStyle.select.value)?.name:'BYLAYER'}
      if(lineweightStyle.select.value&&lineweightStyle.select.value!==lineweightStyle.common)payload.lineweight=Number(lineweightStyle.select.value)
      if(scaleInput.value&&scaleInput.value!==commonScale)payload.linetypeScale=Number(scaleInput.value)
      if(textStyleSelect?.value&&textStyleSelect.value!==commonTextStyleId)payload.styleId=textStyleSelect.value
      if(selectedEntities.length>1){if(Object.keys(payload).length)await execute('PROPERTIES',{ids:selectedEntities.map(item=>item.id),patch:{payload}});return}
      if(leaderFields){const invalid=[leaderFields.height,leaderFields.width,leaderFields.rotation].find(input=>!input.checkValidity());if(!leaderFields.text.value.trim()||invalid){invalid?.reportValidity();return}await execute('LEADEREDIT',{id:entity.id,vertices:entity.payload.vertices,textPosition:entity.payload.textPosition,text:leaderFields.text.value,styleId:leaderFields.style.value,textHeight:Number(leaderFields.height.value),width:leaderFields.width.value?Number(leaderFields.width.value):null,rotation:Number(leaderFields.rotation.value)*Math.PI/180,attachmentPoint:Number(leaderFields.attachment.value),arrowEnabled:leaderFields.arrow.checked,layerId:layerSelect.value});return}
      ...
      if(entity.type==='INSERT'&&blockScope?.value==='definition'){
        ...
        const member=blockMembers.find(item=>item.id===blockMember.value)
        if(member&&blockMemberValue&&['TEXT','MTEXT','ATTDEF'].includes(member.type))payload.text=blockMemberValue.value
        if(member&&blockMemberValue&&['CIRCLE','ARC'].includes(member.type))payload.radius=Number(blockMemberValue.value)
        if(member?.type==='INSERT'&&blockMemberTransform){const position=blockMemberPosition.value.split(/[ ,]+/).filter(Boolean).map(Number);if(position.length!==2||position.some(value=>!Number.isFinite(value)))throw new Error(t('componentPosition'));payload.position=position;payload.scale=Number(blockMemberScale.value);payload.rotation=Number(blockMemberRotation.value)*Math.PI/180}
  ...
  else {field($('inspector'),t('revision'),doc().revision);field($('inspector'),t('modelEntities'),currentModel.length);field($('inspector'),t('kernel'),authority?'Rust / WASM':t('jsReference'));field($('inspector'),t('units'),doc().snapshot().header.units??'unspecified');const editor=document.createElement('div');editor.className='property-editor';const label=document.createElement('label');label.textContent=t('globalLinetypeScale');const input=document.createElement('input');input.type='number';input.min='0.000001';input.step='0.1';input.value=String(doc().snapshot().header.systemVariables.LTSCALE??1);input.dataset.property='global-linetype-scale';label.append(input);const apply=document.createElement('button');apply.textContent=t('applyProperties');apply.onclick=()=>run(()=>execute('SETVAR',{name:'LTSCALE',value:Number(input.value)}));editor.append(label,apply);$('inspector').append(editor);const help=document.createElement('div');help.className='inspector-empty';help.innerHTML=kjdrawIcon('select');help.append(document.createTextNode(t('inspectHint')));$('inspector').append(help)}
...
async function runTypedCommand(){
  ...
  if(drawCommand==='BREAK'){
    const selected=selectedIds().length===1?doc().getObject(selectedIds()[0]):null
...
canvas.onpointermove=e=>{
  ...
  cursor=['PEDIT','BREAK'].includes(modification?.definition.command)?world(p):transformBase?translationPoint(world(p),transformBase):constrainedPoint(world(p));$('coordinates').textContent=`X ${cursor[0].toFixed(2)} · Y ${cursor[1].toFixed(2)}`
...
canvas.onpointerdown=e=>{
  ...
  const raw=world(pointer(e)),p=['PEDIT','BREAK'].includes(modification?.definition.command)?raw:translation?.base?translationPoint(raw,translation.base):constrainedPoint(raw)
  if(boundaryEdit){
