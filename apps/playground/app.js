...
function refresh() {
  ...
  if(entity){
    ...
    const styleSelect=(labelText,values,selectedValues)=>{const label=document.createElement('label');label.textContent=labelText;const select=document.createElement('select'),common=selectedValues.size===1?[...selectedValues][0]:'';if(selectedValues.size!==1){const option=document.createElement('option');option.value='';option.textContent='—';option.selected=true;option.disabled=true;select.append(option)}for(const [value,text] of values){const option=document.createElement('option');option.value=value;option.textContent=text;option.selected=value===common;select.append(option)}label.append(select);editor.append(label);return{select,common}}
    const colorStyle=styleSelect(t('entityColor'),[['256',t('byLayer')],...Array.from({length:255},(_,index)=>[String(index+1),String(index+1)])],new Set(selectedEntities.map(item=>String(item.payload.color??256))))
    const linetypeStyle=styleSelect(t('entityLinetype'),[['',t('byLayer')],...doc().getTable('linetypes').records.map(item=>[item.id,item.name])],new Set(selectedEntities.map(item=>String(item.payload.linetypeId??''))))
    const lineweightStyle=styleSelect(t('entityLineweight'),LAYER_LINEWEIGHTS.map(value=>[String(value),value===-1?t('byLayer'):value===0?'0.00 mm':`${(value/100).toFixed(2)} mm`]),new Set(selectedEntities.map(item=>String(item.payload.lineweight??-1))))
