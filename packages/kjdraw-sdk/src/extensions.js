...
export class KJExtensionRegistry {
  ...
  get(point, id) { return this.#points.get(point)?.get(String(id)) ?? null }
  has(point, id) { return this.#points.get(point)?.has(String(id)) ?? false }
  list(point) { return [...(this.#points.get(point)?.values() ?? [])] }

  removeOwner(owner) {
    let count = 0
    for (const registry of this.#points.values()) {
      for (const [id, entry] of registry) if (entry.owner === owner) { registry.delete(id); count += 1 }
    }
    return count
  }
}
