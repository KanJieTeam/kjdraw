...
const copy = {
    en: {
        ...
        pageScaleNote: 'Choose Fit to paper or edit the custom ratio. Window coordinates use drawing units; physical offsets use millimeters.',
        pageUnchanged: 'Keep existing',
        pageInches: 'Inches',
        pageMm: 'Millimeters',
        pagePixels: 'Pixels',
        pageArea: 'Plot area',
        pageDisplay: 'Last display',
        pageExtents: 'Drawing extents',
        pageLimits: 'Drawing limits',
        pageView: 'Named view',
        pageWindow: 'Window',
        pageLayout: 'Layout',
        pageViewName: 'View name',
        pageMinX: 'Window minimum X',
        pageMinY: 'Window minimum Y',
        pageMaxX: 'Window maximum X',
        pageMaxY: 'Window maximum Y',
        pageOriginX: 'Origin X (mm)',
        pageOriginY: 'Origin Y (mm)',
        pageScaleMode: 'Scale mode',
        ...
        confirmBoundaries: 'Confirm boundaries',
        boundarySelectionHint: 'Click or drag to add boundaries · Ctrl/⌘ removes · Enter confirms',
        boundaryTargetHint: 'Click target portions repeatedly · Enter or Esc finishes',
        boundaryEmpty: 'No editable target here — pick another portion',
    ...
    'zh-CN': {
        ...
        confirmBoundaries: '确认边界',
        boundarySelectionHint: '点击或框选添加边界 · Ctrl/⌘ 减选 · Enter 确认',
        boundaryTargetHint: '连续点选目标区段 · Enter 或 Esc 结束',
        boundaryEmpty: '这里没有可编辑目标，请重新点选区段',
        boundaryBusy: '正在应用修改…'
    }
};
...
export class KJDrawWorkbench {
    ...
    constructor(container, options = {}){
        ...
        query(this.root, '.canvas-wrap').append(boundaryActions);
    ...
    get drawingLayoutId() {
        return this.#drawingLayoutId;
    }
    get spaceId() {
        return this.renderer.spaceId ?? this.document?.spaces.modelSpaceId ?? null;
    }
    get paperPreview() {
        return this.#drawingLayoutId !== null;
    }
    get #readOnly() {
        return this.#options.readonly === true || this.paperPreview;
    }
    get tool() {
        return this.#tool;
    }
    setDrawingLayout(layoutId) {
        ...
        this.root.dispatchEvent(new CustomEvent('kjdraw:space', {
            detail: {
                document: drawing,
                layoutId,
    ...
    #paperLayouts() {
        const drawing = this.document;
        if (!drawing) return [];
        const spaces = drawing.spaces, result = [];
    ...
    #syncDrawingLayouts() {
        ...
        const select = this.root.querySelector('[data-drawing-layout]');
        if (select) {
            const rows = [
                {
                    id: '',
                    name: this.#t('modelSpace')
                },
                ...layouts.map((layout)=>({
                        id: layout.id,
            ...
            select.value = this.#drawingLayoutId ?? '';
            select.setAttribute('aria-label', this.#t('drawingSpace'));
            select.title = this.#t('drawingSpace');
        }
        this.root.dataset.drawingSpace = this.paperPreview ? 'paper' : 'model';
        const notice = this.root.querySelector('[data-paper-preview]');
        if (notice) {
            notice.hidden = !this.paperPreview;
            notice.textContent = this.#t('paperPreview');
        }
    }
    #syncEditability() {
        const allowed = this.paperPreview ? [
            'select',
            'fence',
            'pan'
        ] : [
    ...
    setLocale(locale) {
        ...
        this.#setMessage(this.#draftGesture ? this.#draftPrompt(this.#draftGesture.session.state.nextPoint) : this.#boundarySession ? this.#boundaryPrompt() : modificationPoint ? this.#localizedControlText(modificationPoint.label) : this.#fenceSelection ? this.#t('fenceHint') : this.#tool === 'select' ? this.#t('selectionHint') : this.#t('ready'));
        return this;
    }
    snapshot() {
        const drawing = this.document;
    ...
    setTool(tool) {
        ...
        if (this.#readOnly && !(this.paperPreview ? [
            'select',
            'fence',
            'pan'
        ] : [
    ...
    async #setDocument(document1, generated) {
        ...
        try {
            ...
            const selection = this.sdk.getSelectionManager(document1.id)?.active;
            const modelIds = selection?.ids.filter((id)=>document1.getObject(id)?.ownerId === document1.spaces.modelSpaceId) ?? [];
    ...
    async save(format = 'KJD', options = {}) {
        ...
        const output = await this.sdk.writeDocument(drawing, {
            ...
            format,
            ...format === 'SVG' ? {
                layoutId: options.layoutId ?? this.#drawingLayoutId ?? drawing.spaces.activeLayoutId
            } : {},
    ...
    #bind() {
        ...
        query(this.root, '[data-action="boundary-confirm"]').addEventListener('click', ()=>void this.#run(()=>this.#confirmBoundaryEdit()), {
            signal
        });
        query(this.root, '[data-action="boundary-finish"]').addEventListener('click', ()=>this.#finishBoundaryEdit(), {
            signal
        });
        query(this.root, '[data-action="boundary-cancel"]').addEventListener('click', ()=>this.setTool('select'), {
            signal
        });
        ...
        this.#canvas.addEventListener('lostpointercapture', (event)=>{
            if ([
                this.#boundaryPointer?.pointerId,
        ...
        this.root.addEventListener('keydown', (event)=>{
            ...
            if (!this.#readOnly && (event.ctrlKey || event.metaKey) && event.key.toLowerCase() === 'z') {
                event.preventDefault();
                void this.#run(()=>this.execute(event.shiftKey ? 'REDO' : 'UNDO'));
            }
            if (!this.#readOnly && (event.ctrlKey || event.metaKey) && event.key.toLowerCase() === 'y') {
                event.preventDefault();
    ...
    #syncDraftActions() {
        ...
        if (undo) {
            undo.hidden = !state;
            undo.disabled = !state?.points.length;
        }
        if (finish) {
            finish.hidden = !variableLength;
    ...
    #loadPageSheet() {
        ...
        view.type = 'text';
        view.dataset.pageField = 'viewName';
        view.value = binding.settings.viewName ?? '';
        labelFor('viewName', 'pageViewName').append(view);
        const scale = document.createElement('select');
        scale.dataset.pageScaleMode = '';
        ...
        ])scale.add(new Option(this.#t(text), value));
        scale.value = binding.settings.flags === undefined ? '' : !(binding.settings.flags & 16) ? 'custom' : binding.settings.standardScaleType === 0 ? 'fit' : '';
        scale.dataset.initial = scale.value;
        labelFor('scaleMode', 'pageScaleMode').append(scale);
        const update = ()=>{
            const area = query(fields, '[data-page-field="plotType"]').value;
            for (const input of fields.querySelectorAll('[data-page-field^="window"]')){
                input.disabled = area !== '4';
                input.required = area === '4';
            }
            view.disabled = area !== '3';
            view.required = area === '3';
            for (const input of fields.querySelectorAll('[data-page-field^="scale"]'))input.disabled = scale.value === 'fit';
        };
        query(fields, '[data-page-field="plotType"]').addEventListener('change', update);
        scale.addEventListener('change', update);
        for (const input of fields.querySelectorAll('[data-page-field^="scale"]'))input.addEventListener('input', ()=>{
            scale.value = 'custom';
            update();
        });
        update();
    ...
    async #applyPageSetup() {
        ...
        try {
            ...
            for (const input of dialog.querySelectorAll('[data-page-field]')){
                ...
                const key = input.dataset.pageField, value = key === 'viewName' ? input.value : Number(input.value);
    ...
    #renderModificationForm() {
        ...
        const previousWorkflow = fields.querySelector('[data-boundary-workflow]')?.value;
        fields.replaceChildren();
        if (id === 'trim' || id === 'extend') {
            ...
            const updateDescription = ()=>{
                description.textContent = workflow.value === 'boundaries' ? `${this.#t('boundarySelectionHint')} · ${this.#t('boundaryTargetHint')}` : this.#localizedControlText(definition.description);
            };
            workflow.addEventListener('change', updateDescription, {
                signal: this.#abort.signal
            });
            updateDescription();
            label.append(text, workflow);
            fields.append(label);
        }
    ...
    async #commitModification(gesture) {
        ...
        if (!receipt) {
            if (gesture.document.revision !== gesture.revision) {
                this.setTool('select');
                return;
            }
            if (gesture.definition.pointKeys.length) {
    ...
    #beginBoundaryEdit(operation) {
        ...
        this.#boundarySession = createBoundaryEditSession(operation, {
            document: drawing,
            boundaryIds: ids,
            locale: this.#locale === 'zh-CN' ? 'zh' : 'en',
            isDocumentCurrent: ()=>this.document === drawing && this.sdk.documents.get(drawing.id) === drawing && !this.#readOnly && !this.#abort.signal.aborted
        });
    ...
    #syncBoundaryActions() {
        const host = this.root.querySelector('[data-boundary-actions]');
        if (!host) return;
        ...
        query(host, '[data-boundary-summary]').textContent = !state ? '' : this.#locale === 'zh-CN' ? `${state.operation === 'trim' ? '修剪' : '延伸'} · ${state.boundaryIds.length} 条边界 · ${state.committedCount} 次` : `${state.operation.toUpperCase()} · ${state.boundaryIds.length} ${state.boundaryIds.length === 1 ? 'boundary' : 'boundaries'} · ${state.committedCount} ${state.committedCount === 1 ? 'edit' : 'edits'}`;
        const confirm = query(host, '[data-action="boundary-confirm"]');
        const finish = query(host, '[data-action="boundary-finish"]');
    ...
    #previewBoundaryTarget(location) {
        ...
        if (!hit || session.state.boundaryIds.includes(hit.entity.id)) {
    ...
    async #boundaryPointerUp(gesture, location) {
        const session = this.#boundarySession;
        ...
        try {
            if (gesture.phase === 'boundaries') {
                ...
                const hit = dragged ? null : this.renderer.hitTest(location, 9, {
                    includeLocked: true
                });
                const found = dragged ? this.renderer.selectBox(gesture.baseScreen, location, {
                    includeLocked: true
                }) : hit ? [
                    hit.entity.id
                ] : [];
                const ids = gesture.operation === 'remove' ? session.state.boundaryIds.filter((id)=>!found.includes(id)) : [
                    ...new Set([
                        ...session.state.boundaryIds,
                        ...found
                    ])
                ];
                session.setBoundaries(ids);
                await this.execute('SELECT', {
                    ids,
                    operation: 'replace'
                }, {
                    expectedRevision: gesture.revision
    ...
    #drawOverlay() {
        ...
        const grips = this.#tool === 'select' && !this.#readOnly && !this.#modificationGesture && !this.#boundarySession && (this.#selection?.size === 1 || this.#gripGesture) && !this.#boxSelection && !this.#selectionDrag ? this.renderer.getGrips(this.#gripGesture ? [
