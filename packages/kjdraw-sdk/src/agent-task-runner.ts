...
export async function runPersistedKJAgentTask(options: KJPersistedAgentTaskRunOptions): Promise<ReadonlyDeep<KJPersistedAgentTaskRunResult>> {
  ...
  assertSessionBinding(options.document, options.session, expectedRevision)
  const atomicallyCheckable = task.definition.requirements.every(requirement =>
    requirement.check.toolName === 'cad_check_geometry'
    && requirement.check.geometryCheck
  ...
  if (result.proposalIds.length) {
    ...
    options.session.bindTaskProposal(result.proposalIds[0]!, {
      ...
      toolNames: [...task.definition.tools.names],
      toolContractHash: task.definition.tools.contractHash,
      capabilityLocks: task.definition.capabilities.map(lock => ({ ...lock })),
      ...(options.capabilityRegistry ? { capabilityRegistry: options.capabilityRegistry } : {}),
    })
  }
