...
function point3(value, label = 'grip point') {
  ...
  const z = Number(Array.isArray(value) ? value[2] ?? 0 : value?.z ?? 0)
...
function vertexPoint(vertex) { return vertex.point ?? vertex }
...
export function getEntityGrips(entity) {
  ...
  const add = (id, role, point, detail = {}) => result.push(Object.freeze({ id, entityId: entity.id, role, point: Object.freeze(point3(point)), ...detail }))
  switch (entity.type) {
    case 'LINE':
      add('start', 'endpoint', payload.start); add('mid', 'move', midpoint2(payload.start, payload.end)); add('end', 'endpoint', payload.end); break
    case 'RAY':
    case 'XLINE':
      add('origin', 'move', payload.origin); add('direction', 'direction', [payload.origin[0] + payload.direction[0], payload.origin[1] + payload.direction[1], (payload.origin[2] ?? 0) + (payload.direction[2] ?? 0)]); break
    ...
    case 'SOLID':
    case 'TRACE': for (const [index, point] of payload.vertices.entries()) add(`vertex:${index}`, 'vertex', point, { vertexIndex: index }); break
    case 'ELLIPSE': {
      const [mx, my] = vec2(payload.majorAxis), center = point3(payload.center), minor = [-my * payload.ratio, mx * payload.ratio]
    ...
    case 'LEADER':
    case 'MLEADER':
      for (const [index, point] of (payload.vertices ?? []).entries()) add(`vertex:${index}`, 'vertex', point, { vertexIndex: index })
...
function updateVertex(vertex, target) { return Array.isArray(vertex) ? target : { ...clone(vertex), point: target } }
...
export function editEntityGrip(entity, gripId, targetPoint) {
  ...
  gripId = String(gripId); const target = point3(targetPoint), payload = clone(entity.payload ?? {})
  ...
  switch (entity.type) {
    ...
    case 'XLINE':
      ...
      payload.direction = [target[0] - payload.origin[0], target[1] - payload.origin[1], target[2] - (payload.origin[2] ?? 0)]; return payload
    ...
    case 'ARC':
      ...
      if (gripId === 'start') payload.startAngle = Math.atan2(target[1] - payload.center[1], target[0] - payload.center[0])
      else payload.endAngle = Math.atan2(target[1] - payload.center[1], target[0] - payload.center[0])
      return payload
    case 'LWPOLYLINE':
    case 'POLYLINE': {
      const vertices = [...payload.vertices]
    ...
    case 'ELLIPSE': {
      ...
      } else payload.ratio = distance2(center, target) / Math.hypot(payload.majorAxis[0], payload.majorAxis[1])
      return payload
    }
    case 'SPLINE': {
    ...
    case 'IMAGE': {
      ...
      const origin = point3(payload.position), vector = [target[0] - origin[0], target[1] - origin[1], target[2] - origin[2]]
      if (gripId === 'u') payload.uVector = vector
      else if (gripId === 'v') payload.vVector = vector
      else { payload.uVector = [vector[0] - payload.vVector[0], vector[1] - payload.vVector[1], vector[2] - (payload.vVector[2] ?? 0)] }
      return payload
    }
    case 'LEADER':
    case 'MLEADER':
