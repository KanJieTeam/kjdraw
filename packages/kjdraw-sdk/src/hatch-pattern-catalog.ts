...
import { validateKnowledgePack, type KJKnowledgePack, type KJKnowledgePackSource } from './knowledge-pack.js'
...
const fail = (message: string): never => { throw new KJValidationError(`PAT catalog: ${message}`) }
const boundedName = (value: string, label: string, maximum: number): string => {
...
export function parseAutoCADPat(source: string): ReadonlyDeep<KJHatchPatternCatalog> {
  ...
  const patterns: KJHatchPatternCatalogEntry[] = []
  let current: KJHatchPatternCatalogEntry | undefined
  ...
  for (const [lineIndex, rawLine] of normalizedSource.split(/\r?\n/).entries()) {
    ...
    if (line.startsWith('*')) {
      ...
      current = { name, description: comma < 0 ? '' : line.slice(comma + 1).trim().slice(0, 256), lines: [] }
  ...
  return deepFreeze({ version: '1.0.0', contentHash: stableHash(patterns), patterns })
}

/** Build a native HATCH payload fragment from one parsed pattern. */
export function hatchPatternFromCatalog(source: ReadonlyDeep<KJHatchPatternCatalog>, name: string, options: { scale?: number; angleDegrees?: number } = {}): Readonly<Record<string, unknown>> {
  ...
  const patternLines = pattern.lines.map((line, index) => {
    const values = [line.angle, ...line.base, ...line.offset, ...line.dashes]
    ...
    return { angle: line.angle, base: [...line.base] as [number, number], offset: [...line.offset] as [number, number], dashes: [...line.dashes] }
  })
  return deepFreeze({
...
export function buildHatchPatternKnowledgePack(input: KJHatchPatternKnowledgePackInput): ReadonlyDeep<KJKnowledgePack> {
  const catalog = parseAutoCADPat(input.patSource)
  const patternBaseOffset = input.patternBaseOffset ?? [0, 0]
  if (!Array.isArray(patternBaseOffset) || patternBaseOffset.length !== 2 ||
    patternBaseOffset.some(value => !Number.isFinite(value) || Math.abs(value) > 1e12))
    fail('patternBaseOffset must contain two bounded finite coordinates')
  ...
  const selected = input.selectedPatterns.map(name => {
    ...
    return { ...pattern!, lines: pattern!.lines.map(line => {
      const base: [number, number] = [line.base[0] + patternBaseOffset[0]!, line.base[1] + patternBaseOffset[1]!]
      if (base.some(value => !Number.isFinite(value) || Math.abs(value) > 1e12))
        fail('translated pattern base is outside the bounded coordinate range')
      return { ...line, base }
    }) }
  })
  ...
  return validateKnowledgePack({
    ...
    ontology: { objectKinds: ['hatch-pattern'], relationKinds: ['uses-hatch-pattern'] },
    rules: { 'hatch-pattern-catalog': { version: catalog.version, contentHash: stableHash(selected), patterns: selected, mappings } },
  })
}

/** Resolve a semantic hatch key from a pack built by buildHatchPatternKnowledgePack. */
export function hatchPatternFromKnowledgePack(packSource: unknown, semanticKey: string, options: { scale?: number; angleDegrees?: number } = {}): Readonly<Record<string, unknown>> {
  ...
  const mapping = (source.mappings as Record<string, unknown>)[boundedName(semanticKey, 'semantic key', 96)]
  if (typeof mapping !== 'string') fail(`semantic mapping does not exist: ${semanticKey}`)
  const patterns = source.patterns as KJHatchPatternCatalogEntry[]
  return hatchPatternFromCatalog({ version: '1.0.0', contentHash: stableHash(patterns), patterns }, mapping as string, options)
}
