...
export const KJDRAW_MANUFACTURING_SHEET_VERSION = '1.0.0' as const
...
const INPUT_KEYS = ['version', 'expectedRevision', 'units', 'locale', 'drawingId', 'title', 'revision', 'material', 'quantity', 'length', 'width', 'thickness', 'holePatterns', 'slots', 'sheet', 'textHeight']
...
function point2(value: unknown, label: string, minimum = -1_000_000, maximum = 1_000_000): [number, number] {
  if (!Array.isArray(value) || value.length !== 2) throw new KJValidationError(`${label} must contain exactly two coordinates`)
  return [boundedNumber(value[0], `${label}[0]`, minimum, maximum), boundedNumber(value[1], `${label}[1]`, minimum, maximum)]
}
...
function validateInput(document: ManufacturingDocument, source: KJAgentManufacturingSheetInput) {
  ...
  let holeCount = 0
  const holePatterns = holeSource.map((source: unknown, index: number) => {
...
function boundsForEntities(entities: EntitySpec[]) {
  ...
  return { min: [minX, minY] as [number, number], max: [maxX, maxY] as [number, number], width: maxX - minX, height: maxY - minY }
}

export function buildAgentManufacturingSheet(document: ManufacturingDocument, source: KJAgentManufacturingSheetInput) {
  ...
  const idPrefix = `mfg-${stableHash({ drawingId: input.drawingId, version: input.version }).slice(0, 12)}`
  ...
  const layerDefinitions = {
    OUTLINE: { id: `${idPrefix}-layer-object`, color: 7, linetypeId: linetypeIds.continuous, lineweight: 35 },
    CENTER: { id: `${idPrefix}-layer-center`, color: 3, linetypeId: linetypeIds.center, lineweight: 18 },
    HIDDEN: { id: `${idPrefix}-layer-hidden`, color: 8, linetypeId: linetypeIds.hidden, lineweight: 18 },
    DIMENSIONS: { id: `${idPrefix}-layer-dim`, color: 2, linetypeId: linetypeIds.continuous, lineweight: 18 },
    SHEET: { id: `${idPrefix}-layer-frame`, color: 7, linetypeId: linetypeIds.continuous, lineweight: 25 },
    NOTES: { id: `${idPrefix}-layer-text`, color: 7, linetypeId: linetypeIds.continuous, lineweight: 18 },
  } as const
  const add = (type: string, layerName: keyof typeof layerDefinitions, payload: Record<string, unknown>) => {
    const id = `${idPrefix}-${String(entities.length + 1).padStart(4, '0')}`
    entities.push({ type, payload: { ...payload, layerId: layerDefinitions[layerName].id }, options: { id } })
  }
  const p3 = (x: number, y: number): Point3 => [x, y, 0]
  const rectangle = (x: number, y: number, width: number, height: number, layer: keyof typeof layerDefinitions) => add('LWPOLYLINE', layer, { vertices: [p3(x, y), p3(x + width, y), p3(x + width, y + height), p3(x, y + height)], closed: true })
  const line = (x1: number, y1: number, x2: number, y2: number, layer: keyof typeof layerDefinitions) => add('LINE', layer, { start: p3(x1, y1), end: p3(x2, y2) })
  const arc = (x: number, y: number, radius: number, startAngle: number, endAngle: number, layer: keyof typeof layerDefinitions) => add('ARC', layer, { center: p3(x, y), radius, startAngle, endAngle, clockwise: false })
  const text = (x: number, y: number, value: string, height = input.textHeight) => add('TEXT', 'NOTES', { position: p3(x, y), text: value, height })
  const dimension = (definitionPoints: Point3[], textPosition: Point3, dimensionType = 'ALIGNED') => add('DIMENSION', 'DIMENSIONS', {
    dimensionType,
  ...
  dimension([p3(topX, topY), p3(topX + input.length, topY)], p3(topX + input.length / 2, topY - dimensionPad / 2))
  dimension([p3(topX, topY), p3(topX, topY + input.width)], p3(topX - dimensionPad / 2, topY + input.width / 2))
  ...
  input.slots.forEach((slot, index) => {
    ...
    if (slot.orientationDegrees === 0) {
      ...
      dimension([p3(cx - halfLength, cy), p3(cx + halfLength, cy)], p3(cx, cy + radius + dimensionPad / 2))
    } else {
      ...
      dimension([p3(cx, cy - halfLength), p3(cx, cy + halfLength)], p3(cx + radius + dimensionPad / 2, cy))
    }
  ...
  const sheetBounds: { min: [number, number]; max: [number, number] } = { min: [sheetX, sheetY], max: [sheetX + sheetWidth, sheetY + sheetHeight] }
  const epsilon = 1e-7
  ...
  const resources = {
    linetypes: [
      { id: linetypeIds.continuous, name: `KJ_${idPrefix.slice(4, 16)}_CONT`, pattern: [] },
      { id: linetypeIds.center, name: `KJ_${idPrefix.slice(4, 16)}_CENTER`, pattern: [8, -1, 1, -1] },
      { id: linetypeIds.hidden, name: `KJ_${idPrefix.slice(4, 16)}_HIDDEN`, pattern: [3, -1] },
    ],
  ...
  return {
    ...
    evidence: {
      ...
      limitations: ['Rectangular hole arrays only', 'Slot orientations are limited to 0 or 90 degrees', 'Views are orthographic and compiled at 1:1; the compiler refuses a sheet that cannot contain the requested geometry'],
    },
  }
}
