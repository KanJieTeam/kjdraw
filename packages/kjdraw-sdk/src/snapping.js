...
function derivativeBernstein(values) {
    const degree = values.length - 1;
    return Array.from({
        length: degree
    }, (_, index)=>degree * (values[index + 1] - values[index]));
}
...
function splineDefinitionsCoincide(first, second) {
    ...
    const normalizeKnots = (definition)=>{
    ...
    const normalizeWeights = (definition)=>{
    ...
    const close = (a, b)=>Math.abs(a - b) <= Number.EPSILON * Math.max(1, Math.abs(a), Math.abs(b)) * 128;
    const samePoint = (a, b)=>a[0] === b[0] && a[1] === b[1];
    const forward = left.controlPoints.every((point, index)=>samePoint(point, right.controlPoints[index]) && close(leftWeights[index], rightWeights[index])) && leftKnots.every((knot, index)=>close(knot, rightKnots[index]));
    if (forward) return true;
    const lastControl = left.controlPoints.length - 1, lastKnot = leftKnots.length - 1;
    return left.controlPoints.every((point, index)=>samePoint(point, right.controlPoints[lastControl - index]) && close(leftWeights[index], rightWeights[lastControl - index])) && leftKnots.every((knot, index)=>close(knot, 1 - rightKnots[lastKnot - index]));
}
function splineBezierSegments(geometry, controls, weights, distanceTolerance) {
    ...
    const spans = [], knots = geometry.definition.knots, degree = geometry.definition.degree;
    for(let index = degree; index < geometry.definition.controlPoints.length; index += 1){
        const start = knots[index], end = knots[index + 1];
        if (!(end > start)) continue;
        const x = splineSpanBernstein(xNumerator, start, end), y = splineSpanBernstein(yNumerator, start, end), w = splineSpanBernstein(denominator, start, end);
        ...
        if (x.some((value, position)=>!Number.isFinite(value / w[position]) || Math.abs(value / w[position]) > 2e12) || y.some((value, position)=>!Number.isFinite(value / w[position]) || Math.abs(value / w[position]) > 2e12)) throw new KJValidationError('SPLINE/SPLINE intersection Bezier controls exceed their accuracy bound');
        spans.push({
            start,
            end,
            x,
...
function splineBezierBounds(segment) {
...
function splineBezierFatLinesOverlap(first, second, tolerance) {
    const controls = (segment)=>segment.x.map((x, index)=>[
                x / segment.w[index],
                segment.y[index] / segment.w[index]
            ]);
    const compatible = (reference, candidate)=>{
        ...
        const signed = (point)=>((point[0] - start[0]) * dy - (point[1] - start[1]) * dx) / length;
...
function splitSplineBezier(segment) {
...
function consumeSplineIntersectionBudget(budget) {
    if (--budget.remaining < 0) throw new KJValidationError(`SPLINE intersection exceeds the ${MAX_SPLINE_INTERSECTION_WORK} interval work budget`);
}
function splineSplineIntersection(first, second, budget) {
    ...
    const allControls = [
        ...leftControls,
        ...rightControls
    ], minimumX = Math.min(...allControls.map((point)=>point[0])), maximumX = Math.max(...allControls.map((point)=>point[0]));
    const minimumY = Math.min(...allControls.map((point)=>point[1])), maximumY = Math.max(...allControls.map((point)=>point[1]));
    const coordinateScale = Math.max(1, maximumX - minimumX, maximumY - minimumY), absoluteScale = Math.max(1, ...allControls.flatMap((point)=>[
            Math.abs(point[0]),
            Math.abs(point[1])
        ]));
    const distanceTolerance = Math.max(1e-9 * coordinateScale, Number.EPSILON * absoluteScale * 64);
    ...
    for (const bounds of [
        curveBounds(leftSpans),
        curveBounds(rightSpans)
    ])if (Math.hypot(bounds[2] - bounds[0], bounds[3] - bounds[1]) <= distanceTolerance) {
        throw new KJValidationError('SPLINE/SPLINE intersection requires nondegenerate planar curves');
    }
    if (splineDefinitionsCoincide(leftGeometry, rightGeometry)) return {
        kind: 'overlap',
        points: [],
        infinite: true
    };
    const overlaps = (a, b)=>a[0] <= b[2] + distanceTolerance && a[2] + distanceTolerance >= b[0] && a[1] <= b[3] + distanceTolerance && a[3] + distanceTolerance >= b[1];
    const roots = [];
    const addRoot = (leftParameter, rightParameter)=>{
        ...
        if (duplicate < 0) roots.push({
            left: leftParameter,
            right: rightParameter,
            point,
            residual
        });
        else if (residual < roots[duplicate].residual) roots[duplicate] = {
            left: leftParameter,
            right: rightParameter,
            point,
            residual
    ...
    const minimize = (lower, upper, distanceAt)=>{
        const ratio = (Math.sqrt(5) - 1) / 2;
        let left = upper - (upper - lower) * ratio, right = lower + (upper - lower) * ratio;
        let leftDistance = distanceAt(left), rightDistance = distanceAt(right);
    ...
    for (const leftSpan of leftSpans)for (const rightSpan of rightSpans){
        ...
        while(pending.length){
            ...
            if (roots.some((root)=>root.point[0] >= leftBounds[0] - distanceTolerance * 8 && root.point[0] <= leftBounds[2] + distanceTolerance * 8 && root.point[1] >= leftBounds[1] - distanceTolerance * 8 && root.point[1] <= leftBounds[3] + distanceTolerance * 8 && root.point[0] >= rightBounds[0] - distanceTolerance * 8 && root.point[0] <= rightBounds[2] + distanceTolerance * 8 && root.point[1] >= rightBounds[1] - distanceTolerance * 8 && root.point[1] <= rightBounds[3] + distanceTolerance * 8)) continue;
            ...
            if (Math.max(leftSize, rightSize) <= refinementSize || depth >= 104) {
                refine(left, right);
                continue;
            }
            if (leftSize >= rightSize) for (const half of splitSplineBezier(left))pending.push([
                half,
                right,
                depth + 1
            ]);
            else for (const half of splitSplineBezier(right))pending.push([
                left,
                half,
                depth + 1
            ]);
    ...
    return {
        kind: roots.length ? 'point' : 'none',
        points: roots.map((root)=>root.point)
    };
}
...
function splineRelationCandidates(entity, cursor, referenceInput, payload, mode) {
    ...
    const absoluteScale = Math.max(1, Math.abs(reference[0]), Math.abs(reference[1]), ...controls.flatMap((point)=>[
            Math.abs(point[0]),
            Math.abs(point[1])
        ]));
    const distanceTolerance = Math.max(1e-9 * coordinateScale, Number.EPSILON * absoluteScale * 64);
    ...
    const polynomial = (value)=>normalizeSplineDefinition({
            ...
            knots: geometry.definition.knots,
            controlPoints: controls.map((point, index)=>[
                    value(point) * normalizedWeights[index],
                    0
                ])
        });
    ...
    const denominator = polynomial(()=>1), roots = [];
    const budget = {
    ...
    for(let index = degree; index < geometry.definition.controlPoints.length; index += 1){
        ...
        const addRoot = (parameter)=>{
            ...
            const point = splinePointAt3(geometry, parameter);
            if (!roots.some((root)=>Math.abs(root.parameter - parameter) <= 1e-9 * Math.max(1, Math.abs(parameter), Math.abs(root.parameter)) || distance2(root.point, point) <= distanceTolerance * 4)) roots.push({
                parameter,
                point
            });
        };
        isolateSplinePolynomial(equation, start, end, equationTolerance, budget, addRoot, ()=>{
            indeterminate = true;
        });
    }
    ...
    return roots.map((root)=>({
            ...
            distance: distance2(cursor, root.point),
...
function primitiveIntersection(a, b, splineBudget = {
    ...
    if (a.kind === 'spline' || b.kind === 'spline') {
        if (a.kind === 'spline' && b.kind === 'spline') {
            const [first, second] = a.entityId.localeCompare(b.entityId) <= 0 ? [
                a,
                b
            ] : [
                b,
