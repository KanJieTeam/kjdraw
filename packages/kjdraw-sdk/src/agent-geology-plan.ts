...
export interface KJGeologyPlanBaseMapHatch {
  ...
  patternScale: number
  patternLines: { angleDegrees: number; base: Point2; offset: Point2; dashes: number[] }[]
  seedPoints: Point2[]
  boundaryLoops: {
    external: boolean
    flags: number
    closed: true
    vertices: { point: Point2; bulge: number }[]
...
type NormalizedBaseMapLinework = Exclude<NormalizedBaseMapEntity, { kind: 'insert' } | { kind: 'attributeDefinition' } | { kind: 'hatch' } | { kind: 'solid' } | { kind: 'point' }>
...
const BASE_MAP_STYLE_KEYS = ['id', 'color', 'lineweight', 'pattern']
...
const BASE_MAP_POINT_KEYS = ['id', 'styleId', 'kind', 'position']
...
function dashPattern(value: unknown, label: string): number[] {
  ...
  for (let index = 0; index < result.length; index += 1) {
    if (Math.abs(result[index]!) <= EPSILON || (index % 2 === 0 ? result[index]! < 0 : result[index]! > 0))
      throw new KJValidationError(`${label} must alternate positive dashes and negative gaps`)
  }
  return result
}
...
function validatePolygon(points: Point2[], label = 'input.boundary'): void {
...
function validateInput(document: GeologyPlanDocument, source: KJAgentGeologyPlanInput) {
  ...
  const modelViewportCenter: Point2 = [(boundaryMinimum[0] + boundaryMaximum[0]) / 2, (boundaryMinimum[1] + boundaryMaximum[1]) / 2]
  ...
  const legacyPolyline = (value: Record<string, unknown>, label: string, modelSpace: boolean) => {
    ...
    const splineVertexFlags = new Set([8, 16])
  ...
  let coordinateGrid: KJGeologyPlanCoordinateGrid | undefined
  if (input.coordinateGrid !== undefined) {
    ...
    coordinateGrid = { origin: point(grid.origin, 'input.coordinateGrid.origin'), spacing: finite(grid.spacing, 'input.coordinateGrid.spacing', 0.1, 1_000_000) }
  }
  if (input.coordinateCallouts !== undefined && (!Array.isArray(input.coordinateCallouts) || input.coordinateCallouts.length > 64))
    throw new KJValidationError('input.coordinateCallouts must contain at most 64 supplied callouts')
  const coordinateCalloutIds = new Set<string>()
  const coordinateCallouts = (input.coordinateCallouts ?? []).map((raw, index) => {
    const value = plain(raw, `input.coordinateCallouts[${index}]`); exactKeys(value, COORDINATE_CALLOUT_KEYS, `input.coordinateCallouts[${index}]`)
    ...
    if (![callout.point, callout.elbow, callout.landingEnd, callout.xLabelPosition, callout.yLabelPosition].every(insideModelViewport))
      throw new KJValidationError(`input.coordinateCallouts[${index}] geometry must lie inside the declared model viewport`)
    if (Math.hypot(callout.elbow[0] - callout.point[0], callout.elbow[1] - callout.point[1]) <= EPSILON)
      throw new KJValidationError(`input.coordinateCallouts[${index}] point-to-elbow leader must have positive length`)
    if (Math.hypot(callout.landingEnd[0] - callout.elbow[0], callout.landingEnd[1] - callout.elbow[1]) <= EPSILON)
      throw new KJValidationError(`input.coordinateCallouts[${index}] elbow-to-landing leader must have positive length`)
    return callout
  })
  ...
  const buildingFootprints = (input.buildingFootprints ?? []).map((raw, index) => {
    const value = plain(raw, `input.buildingFootprints[${index}]`); exactKeys(value, BUILDING_KEYS, `input.buildingFootprints[${index}]`)
    ...
    buildingIds.add(id)
    if (!Array.isArray(value.outline) || value.outline.length < 3 || value.outline.length > 65)
      throw new KJValidationError(`input.buildingFootprints[${index}].outline must contain 3-65 supplied points`)
    ...
    const outline = suppliedOutline.length > 3 && Math.hypot(firstOutlinePoint[0] - lastOutlinePoint[0], firstOutlinePoint[1] - lastOutlinePoint[1]) <= EPSILON
      ? suppliedOutline.slice(0, -1) : suppliedOutline
    ...
    const segments = value.segments.map((rawSegment, segmentIndex) => {
      ...
      if (segment.kind === 'line') {
        ...
        for (let sample = 1; sample < 10; sample += 1) {
          ...
          if (!insideModelViewport([segmentStart[0] + (end[0] - segmentStart[0]) * ratio, segmentStart[1] + (end[1] - segmentStart[1]) * ratio]))
      ...
      for (let sample = 1; sample < sampleCount; sample += 1) {
        ...
        if (!insideModelViewport([center[0] + Math.cos(angle) * startRadius, center[1] + Math.sin(angle) * startRadius]))
          throw new KJValidationError(`${label} must remain inside the declared model viewport`)
      }
  ...
  if (input.baseMapStyles !== undefined && (!Array.isArray(input.baseMapStyles) || input.baseMapStyles.length > 64))
    throw new KJValidationError('input.baseMapStyles must contain at most 64 supplied styles')
  const baseMapStyleIds = new Set<string>()
  const baseMapStyles = (input.baseMapStyles ?? []).map((raw, index) => {
    const value = plain(raw, `input.baseMapStyles[${index}]`); exactKeys(value, BASE_MAP_STYLE_KEYS, `input.baseMapStyles[${index}]`)
  ...
  const baseMapLinework: NormalizedBaseMapLinework[] = (input.baseMapLinework ?? []).map((raw, index) => {
    ...
    if (kind === 'arc') {
      ...
      for (let sample = 0; sample <= sampleCount; sample += 1) {
        const angle = startAngle + (clockwise ? -1 : 1) * sweep * sample / sampleCount
        if (!insideModelViewport([center[0] + Math.cos(angle) * radius, center[1] + Math.sin(angle) * radius]))
          throw new KJValidationError(`${label} geometry must lie inside the declared model viewport`)
      }
      return { id, styleId, kind, center, radius, startAngle, endAngle, clockwise }
    }
    if (kind === 'circle') {
      const center = point(value.center, `${label}.center`), radius = finite(value.radius, `${label}.radius`, 0.000001, 1_000_000)
      const extrema: Point2[] = [[center[0] - radius, center[1]], [center[0] + radius, center[1]], [center[0], center[1] - radius], [center[0], center[1] + radius]]
      ...
      return { id, styleId, kind, center, radius }
    }
    if (!Array.isArray(value.points) || value.points.length < 2 || value.points.length > 256)
      throw new KJValidationError(`${label}.points must contain 2-256 supplied points`)
  ...
  const validateBlockEntity = (raw: unknown, label: string, topLevel = false): NormalizedBaseMapEntity => {
    ...
    if (kind === 'point') {
      ...
      return { id, styleId, kind, position: point(value.position, `${label}.position`) }
    }
    if (kind === 'solid') {
    ...
    if (kind === 'hatch') {
      ...
      const patternLines = value.patternLines.map((rawLine, lineIndex) => {
        const lineLabel = `${label}.patternLines[${lineIndex}]`, line = plain(rawLine, lineLabel); exactKeys(line, BASE_MAP_HATCH_PATTERN_LINE_KEYS, lineLabel)
        ...
        return { angleDegrees: finite(line.angleDegrees, `${lineLabel}.angleDegrees`, -360_000, 360_000), base: point(line.base, `${lineLabel}.base`), offset: point(line.offset, `${lineLabel}.offset`), dashes }
      })
      ...
      const boundaryLoops = value.boundaryLoops.map((rawLoop, loopIndex) => {
        const loopLabel = `${label}.boundaryLoops[${loopIndex}]`, loop = plain(rawLoop, loopLabel); exactKeys(loop, BASE_MAP_HATCH_LOOP_KEYS, loopLabel)
        ...
        const vertices = loop.vertices.map((rawVertex, vertexIndex) => {
          const vertexLabel = `${loopLabel}.vertices[${vertexIndex}]`, vertex = plain(rawVertex, vertexLabel); exactKeys(vertex, BASE_MAP_HATCH_VERTEX_KEYS, vertexLabel)
          return { point: point(vertex.point, `${vertexLabel}.point`), bulge: finite(vertex.bulge, `${vertexLabel}.bulge`, -1_000_000, 1_000_000) }
        })
        ...
        return { external: loop.external, flags, closed: true as const, vertices, sourceMemberIds }
      })
      ...
      return { id, styleId, kind, patternName: text(value.patternName, `${label}.patternName`, 80), solid: false, associative: value.associative,
        patternAngleDegrees: finite(value.patternAngleDegrees, `${label}.patternAngleDegrees`, -360_000, 360_000), patternScale: finite(value.patternScale, `${label}.patternScale`, 0.000001, 1_000_000), patternLines, seedPoints, boundaryLoops }
    }
    ...
    if (kind === 'circle') return { id, styleId, kind, center: point(value.center, `${label}.center`), radius: finite(value.radius, `${label}.radius`, 0.000001, 1_000_000) }
  ...
  const rawBaseMapBlocks = (input.baseMapBlocks ?? []).map((raw, blockIndex) => {
    const label = `input.baseMapBlocks[${blockIndex}]`, value = plain(raw, label); exactKeys(value, BASE_MAP_BLOCK_KEYS, label)
    const id = text(value.id, `${label}.id`, 40)
    ...
    return { id, basePoint: point(value.basePoint, `${label}.basePoint`), rawEntities: value.entities }
  })
  ...
  const baseMapBlocks = rawBaseMapBlocks.map((block, blockIndex) => {
    ...
    const membersById = new Map(entities.map(entity => [entity.id, entity]))
    for (const [memberIndex, entity] of entities.entries()) if (entity.kind === 'hatch') for (const [loopIndex, loop] of entity.boundaryLoops.entries()) {
      ...
      const samePoint = (first: Point2, second: Point2) => Math.hypot(second[0] - first[0], second[1] - first[1]) <= EPSILON
    ...
    return { id: block.id, basePoint: block.basePoint, entities }
  })
  ...
  const blocksById = new Map(baseMapBlocks.map(block => [block.id, block]))
  const reachable = new Set<string>(), stack = new Set<string>()
  const visitBlock = (id: string, depth: number): void => {
...
export function buildAgentGeologyPlan(document: GeologyPlanDocument, source: KJAgentGeologyPlanInput) {
  ...
  const addToLayer = (type: string, layerId: string, payload: Record<string, unknown>) => entities.push({ type, payload: { ...payload, layerId }, options: { id: `${prefix}-${String(entities.length + 1).padStart(4, '0')}` } })
  ...
  const baseMapEntitySpec = (item: (typeof input.baseMapBlocks)[number]['entities'][number], id: string, memberIds: ReadonlyMap<string, string>): EntitySpec => {
    ...
    if (item.kind === 'hatch') return { type: 'HATCH', payload: {
      patternName: item.patternName, solid: item.solid, associative: item.associative,
      patternAngle: item.patternAngleDegrees * Math.PI / 180, patternScale: item.patternScale,
      patternDefinitionAngle: item.patternAngleDegrees * Math.PI / 180, patternDefinitionScale: item.patternScale,
      patternLines: item.patternLines.map(line => ({ angle: line.angleDegrees * Math.PI / 180, base: line.base, offset: line.offset, dashes: line.dashes })),
      seedPoints: item.seedPoints,
      boundaryLoops: item.boundaryLoops.map(loop => ({ external: loop.external, flags: loop.flags, closed: loop.closed,
        vertices: loop.vertices.map(vertex => ({ point: p3(vertex.point), bulge: vertex.bulge })),
        sourceIds: loop.sourceMemberIds.map(sourceId => memberIds.get(sourceId)!),
      })), ...common,
    }, options: { id } }
    if (item.kind === 'point') return { type: 'POINT', payload: { position: p3(item.position), ...common }, options: { id } }
  ...
  const baseMapBlockResources = input.baseMapBlocks.map((block, blockIndex) => {
    const memberIds = new Map(block.entities.map((item, memberIndex) => [item.id, `${prefix}-block-${String(blockIndex + 1).padStart(2, '0')}-member-${String(memberIndex + 1).padStart(4, '0')}`]))
    return { id: baseMapBlockRecordIds.get(block.id)!, name: `BASEMAP_BLOCK_${String(blockIndex + 1).padStart(2, '0')}`, basePoint: p3(block.basePoint),
  ...
  for (const item of input.baseMapLinework) {
    const style = baseMapStylesById.get(item.styleId)!
    const common = { semanticRole: 'source-backed-base-map-linework', sourceId: item.id, sourceBacked: true }
  ...
  for (const callout of input.coordinateCallouts) {
    add('LINE', 'COORDINATES', { start: p3(callout.point), end: p3(callout.elbow), semanticRole: 'coordinate-callout-leader', segmentRole: 'point-to-elbow', sourceId: callout.id, sourceBacked: true })
    add('LINE', 'COORDINATES', { start: p3(callout.elbow), end: p3(callout.landingEnd), semanticRole: 'coordinate-callout-leader', segmentRole: 'elbow-to-landing', sourceId: callout.id, sourceBacked: true })
