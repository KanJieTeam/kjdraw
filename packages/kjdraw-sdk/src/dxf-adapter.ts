...
interface DxfRecord { type: string; tags: DxfTag[]; vertices?: DxfRecord[]; attributes?: DxfRecord[]; sequenceEnd?: DxfRecord | null }
...
export const DXF_DEFAULT_READ_LIMITS: Readonly<DxfReadLimits> = Object.freeze({ maxBytes: 64 * 1024 ** 2, maxTags: 4_000_000, maxEntities: 250_000 })

function abortDXF(options: DxfReadOptions): void {
  if (options.signal?.aborted) throw new KJValidationError('DXF read aborted')
}

function reportDXF(options: DxfReadOptions, phase: 'source' | 'parse' | 'import', completed: number, unit: 'bytes' | 'tags' | 'entities', total?: number): void {
  options.onProgress?.(Object.freeze({ phase, completed, ...(total === undefined ? {} : { total }), unit }))
  abortDXF(options)
}

const yieldDXF = (): Promise<void> => new Promise(resolve => setTimeout(resolve, 0))
...
async function sourceText(source: unknown, options: DxfReadOptions | Partial<DxfReadLimits> = {}): Promise<string> {
  ...
  const readOptions = options as DxfReadOptions
  abortDXF(readOptions)
  if (typeof source === 'string') { const size = new TextEncoder().encode(source).byteLength; assertSourceSize(size, limits); reportDXF(readOptions, 'source', size, 'bytes', size); return source }
  if (source instanceof Uint8Array) { assertSourceSize(source.byteLength, limits); reportDXF(readOptions, 'source', source.byteLength, 'bytes', source.byteLength); return decodeBytes(source) }
  if (source instanceof ArrayBuffer) { assertSourceSize(source.byteLength, limits); reportDXF(readOptions, 'source', source.byteLength, 'bytes', source.byteLength); return decodeBytes(new Uint8Array(source)) }
  if (source !== null && typeof source === 'object' && 'stream' in source && typeof source.stream === 'function') {
    const streamed = source as { size?: number; stream(): ReadableStream<Uint8Array> }
    assertSourceSize(streamed.size, limits)
    const reader = streamed.stream().getReader(), chunks: Uint8Array[] = []
...
async function tagsFromText(text: string, options: DxfReadOptions = {}): Promise<DxfTag[]> {
  const limits = readLimits(options)
  const tags: DxfTag[] = []
  ...
  while (cursor < text.length) {
    ...
    tags.push({ code, value: code === 1 || code === 3 ? valueLine : valueLine.trimEnd() })
    lineNumber += 2
...
// Attribute coordinates already belong to the INSERT's containing space. Keep
// the native sequence together before resolving top-level space ownership.
function collapseInsertAttributes(source: readonly DxfRecord[]): DxfRecord[] {
  const output: DxfRecord[] = []
  for (let index = 0; index < source.length; index++) {
    const record = source[index]!
    if (record.type !== 'INSERT' || number(record, 66) !== 1) { output.push(record); continue }
    const attributes: DxfRecord[] = []
    ...
    for (const child of [...attributes, sequenceEnd]) {
    ...
    output.push({ ...record, attributes, sequenceEnd })
    index = cursor
  }
  return output
}

function entityRecords(tags: readonly DxfTag[]): DxfRecord[] { return collapseInsertAttributes(collapseLegacyPolylines(records(tags))) }
...
function blockDefinitions(tags: readonly DxfTag[]): DxfBlockDefinition[] {
  ...
  for (let index = 0; index < source.length; index += 1) {
    ...
    if (name) output.push({ name, header, basePoint: point(header), flags: number(header, 70, 0), records: collapseInsertAttributes(collapseLegacyPolylines(inner)) })
  }
  return output
}
    ...
    if (flags & 2) {
      ...
      for (let edgeIndex = 0; edgeIndex < edgeCount && cursor < tags.length; edgeIndex += 1) {
        ...
        if (edgeType === 1) {
          ...
          const edge:DxfHatchSplineEdge={type:'SPLINE',degree:0,periodic:false,controlPoints:[],knots:[],weights:[],fitPoints:[]}
          let rational=false,knotCount=0,controlCount=0
          while(cursor<tags.length&&![72,92].includes(tags[cursor]!.code)){
            const tag=tags[cursor]!
            if(tag.code===94){edge.degree=Number(tag.value);cursor++}else if(tag.code===73){rational=Number(tag.value)!==0;cursor++}else if(tag.code===74){edge.periodic=Number(tag.value)!==0;cursor++}
            else if(tag.code===95){knotCount=Number(tag.value);cursor++;break}else cursor++
          }
          while(cursor<tags.length&&tags[cursor]!.code!==96)cursor++
          if(tags[cursor]?.code===96){controlCount=Number(tags[cursor++]!.value)}
          while(cursor<tags.length&&edge.knots.length<knotCount){if(tags[cursor]!.code===40)edge.knots.push(Number(tags[cursor]!.value));cursor++}
          while(cursor<tags.length&&edge.controlPoints.length<controlCount){
            if(tags[cursor]!.code!==10){cursor++;continue}
            const control:Point3=[Number(tags[cursor++]!.value),0,0];let weight:number|undefined
            while(cursor<tags.length&&tags[cursor]!.code!==10&&tags[cursor]!.code!==97&&![72,92].includes(tags[cursor]!.code)){const tag=tags[cursor++]!;if(tag.code===20)control[1]=Number(tag.value);else if(tag.code===42)weight=Number(tag.value)}
            edge.controlPoints.push(control);if(rational)edge.weights.push(Number(weight))
          }
          if(tags[cursor]?.code===97&&!(Number(tags[cursor]!.value)>0&&tags[cursor+1]?.code===330)){
            ...
            while(cursor<tags.length&&edge.fitPoints.length<fitCount){
              if(tags[cursor]!.code!==11){cursor++;continue}
              const fit:Point3=[Number(tags[cursor++]!.value),0,0]
              while(cursor<tags.length&&tags[cursor]!.code!==11&&![12,13,72,92,97].includes(tags[cursor]!.code)){const tag=tags[cursor++]!;if(tag.code===21)fit[1]=Number(tag.value)}
              edge.fitPoints.push(fit)
            }
          }
          const tangent=(xCode:number,yCode:number):Point3|undefined=>{if(tags[cursor]?.code!==xCode)return undefined;const value:Point3=[Number(tags[cursor++]!.value),0,0];if(tags[cursor]?.code===yCode)value[1]=Number(tags[cursor++]!.value);return value}
          const startTangent=tangent(12,22),endTangent=tangent(13,23)
          if(startTangent)edge.startTangent=startTangent;if(endTangent)edge.endTangent=endTangent;edges.push(edge)
        } else {
          const rawTags: DxfTag[] = []
          while (cursor < tags.length && ![72, 92, 97].includes(tags[cursor]!.code)) rawTags.push(tags[cursor++]!)
          edges.push({ type: 'UNKNOWN', dxfEdgeType: edgeType, rawTags })
        }
      }
...
async function readDXF(source: unknown, options: DxfReadOptions = {}): Promise<KJDocument> {
  ...
  await document.transact('Import ASCII DXF', async transaction => {
    ...
    const allSourceRecords = [...sourceEntityRecords, ...definitions.flatMap(definition => definition.records)].flatMap(record => [record, ...(record.attributes ?? []), ...(record.attributes && record.sequenceEnd ? [record.sequenceEnd] : [])])
    ...
    const importCheckpoint = async (): Promise<void> => {
      importCompleted += 1
    ...
    const importRecord = (record: DxfRecord, index: number, ownerId: string | undefined, scope: string, parentInsertId?: string): KJObjectRecord => {
      ...
      try {
        ...
        created = transaction.createEntity('PROXY_ENTITY', { originalType: record.type, rawTags: record.tags, importError: error instanceof Error ? error.message : String(error), layerId }, {
          ...(ownerId === undefined ? {} : { ownerId }),
          source: { format: 'DXF', scope, entityIndex: index, originalHandle: sourceHandle || null },
        })
        occupiedHandles.add(created.handle)
      ...
      if (record.attributes && record.sequenceEnd) {
        ...
        const sequenceEnd = transaction.createObject({
          kind: 'custom', type: 'SEQEND', ownerId: created.id,
          ...(endHandleAvailable ? { handle: sourceEndHandle } : {}),
          payload: { layerId: layerIds.get(normalizeName(first(end, 8, '0'))) ?? defaultLayerId, dxfOwnerMode: recordOwner(end) && recordOwner(end) === recordOwner(record) ? 'space' : 'insert' },
          source: { format: 'DXF', scope: `${scope}:attributes`, originalHandle: sourceEndHandle || null },
        })
        occupiedHandles.add(sequenceEnd.handle)
        created = transaction.updateObject(created.id, { payload: { attributeIds: attributes.map(attribute => attribute.id), sequenceEndId: sequenceEnd.id } })
      }
      return created
    }
...
function recordSubclass(record: DxfRecord, name: string): DxfRecord {
  ...
  return { type: record.type, tags: record.tags.slice(start + 1, next < 0 ? undefined : next) }
}
  ...
  for (const loop of p.boundaryLoops ?? []) {
    ...
    for (const edge of loop.edges ?? []) {
      if (edge.type === 'LINE') { requireXY(edge.start); requireXY(edge.end) }
        ...
        for(const value of [...edge.controlPoints,...edge.fitPoints])requireXY(value)
        if(edge.startTangent)requireXY(edge.startTangent);if(edge.endTangent)requireXY(edge.endTangent)
    ...
    if (loop.vertices?.length) {
      ...
      for (const edge of loop.edges ?? []) {
        if (edge.type === 'LINE') { emit(output, 72, 1); emit(output, 10, edge.start[0]); emit(output, 20, edge.start[1]); emit(output, 11, edge.end[0]); emit(output, 21, edge.end[1]) }
          ...
          emit(output,72,4);emit(output,94,edge.degree);emit(output,73,edge.weights.length?1:0);emit(output,74,edge.periodic?1:0);emit(output,95,edge.knots.length);emit(output,96,edge.controlPoints.length)
          for(const knot of edge.knots)emit(output,40,knot)
          for(const [index,control] of edge.controlPoints.entries()){emit(output,10,control[0]);emit(output,20,control[1]);if(edge.weights.length)emit(output,42,edge.weights[index])}
          emit(output,97,edge.fitPoints.length);for(const fit of edge.fitPoints){emit(output,11,fit[0]);emit(output,21,fit[1])}
          if(edge.startTangent){emit(output,12,edge.startTangent[0]);emit(output,22,edge.startTangent[1])}if(edge.endTangent){emit(output,13,edge.endTangent[0]);emit(output,23,edge.endTangent[1])}
        }
...
function emitSingleLineText(output: string[], p: DxfPayload, version: DxfVersion, resources: DxfExportResources): void {
  emitSubclass(output, version, 'AcDbText'); emitPoint(output, p.position!); emit(output, 40, p.height); emit(output, 1, writeDxfText(p))
...
function writeDXF(document: unknown, options: KJFileAdapterContext = {}): string {
  ...
  const emitSpaceEntity = (sourceEntity: KJReadonlyObjectRecord, ownerHandle: string | null, space: DxfSpace | null): void => {
    ...
    if (!end || end.erased || end.type !== 'SEQEND' || end.ownerId !== sourceEntity.id || !['insert', 'space'].includes(String(end.payload.dxfOwnerMode))) throw new KJValidationError('DXF INSERT has an invalid SEQEND')
