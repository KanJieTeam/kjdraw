...
export async function createAgentGeometryPreview(document: KJDocument, command: 'CREATEBATCH' | 'MOVE' | 'ROTATE' | 'SCALE', args: Record<string, unknown>, options: KJAgentGeometryPreviewOptions = {}): Promise<KJAgentGeometryPreview> {
  if (!['CREATEBATCH', 'MOVE', 'ROTATE', 'SCALE'].includes(command)) throw new KJValidationError('This preview supports only CREATEBATCH, MOVE, ROTATE and SCALE')
  ...
  const blockDependencies = command !== 'CREATEBATCH' ? captureAgentBlockDependencies(document, args.ids as string[]) : undefined
  const workingSet = command !== 'CREATEBATCH' ? (args.ids as string[]).map(id => document.getObject(String(id))) : args.entities
  ...
  if (blockDependencies && canonicalStringify(captureAgentBlockDependencies(draft, args.ids as string[])) !== canonicalStringify(blockDependencies)) throw new KJValidationError('Block definitions or styles changed during transform preview')
