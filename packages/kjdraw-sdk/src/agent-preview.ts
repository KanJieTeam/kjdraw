...
export interface KJAgentGeometryPreview {
  ...
  readonly command: 'CREATEBATCH' | 'MOVE' | 'ROTATE' | 'SCALE' | 'PEDIT' | 'ROAD_DRAWING_UPDATE'
...
export async function createAgentGeometryPreview(document: KJDocument, command: 'CREATEBATCH' | 'MOVE' | 'ROTATE' | 'SCALE' | 'PEDIT', args: Record<string, unknown>, options: KJAgentGeometryPreviewOptions = {}): Promise<KJAgentGeometryPreview> {
  if (!['CREATEBATCH', 'MOVE', 'ROTATE', 'SCALE', 'PEDIT'].includes(command)) throw new KJValidationError('This preview supports only CREATEBATCH, MOVE, ROTATE, SCALE and PEDIT')
