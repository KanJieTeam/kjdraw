...
export interface KJAgentGeometryPreview {
  ...
  readonly command: 'CREATEBATCH' | 'MOVE' | 'ROTATE' | 'SCALE' | 'STRETCH' | 'PEDIT' | 'ROAD_DRAWING_UPDATE'
...
export async function createAgentGeometryPreview(document: KJDocument, command: 'CREATEBATCH' | 'MOVE' | 'ROTATE' | 'SCALE' | 'STRETCH' | 'PEDIT', args: Record<string, unknown>, options: KJAgentGeometryPreviewOptions = {}): Promise<KJAgentGeometryPreview> {
  if (!['CREATEBATCH', 'MOVE', 'ROTATE', 'SCALE', 'STRETCH', 'PEDIT'].includes(command)) throw new KJValidationError('This preview supports only CREATEBATCH, MOVE, ROTATE, SCALE, STRETCH and PEDIT')
  ...
  if (command === 'CREATEBATCH') {
    ...
    const ids = command === 'PEDIT' ? [args.id] : args.ids
  ...
  const ids = command === 'CREATEBATCH' ? [] : command === 'PEDIT' ? [String(args.id)] : args.ids as string[]
