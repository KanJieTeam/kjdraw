...
function inDomain(parameter: number, mode: LineDomain, tolerance: KJTolerance): boolean {
...
const none = (): KJIntersectionResult => ({
  kind: 'none',
  points: [],
  parametersA: [],
  parametersB: [],
...
function intersectLineLineReference2(
  a0: Point2Input,
  a1: Point2Input,
  b0: Point2Input,
  b1: Point2Input,
  ...
  return {
    kind: 'point',
    points: [add2(startA, multiply2(r, parameterA))],
    parametersA: [parameterA],
    parametersB: [parameterB],
  }
}

function intersectLineCircleReference2(
  start: Point2Input,
  end: Point2Input,
  center: Point2Input,
  radius: number,
  ...
  const parameters = candidates
    .filter(parameter => inDomain(parameter, mode, tolerance))
    .sort((x, y) => x - y)
...
function intersectCircleCircleReference2(
  centerA: Point2Input,
  radiusA: number,
  centerB: Point2Input,
  radiusB: number,
  ...
  if (![resolvedRadiusA, resolvedRadiusB].every(value => Number.isFinite(value) && value >= 0)) {
  ...
  if (
    distance > resolvedRadiusA + resolvedRadiusB + epsilon
...
function orientationReference2(
  a: Point2Input,
  b: Point2Input,
  c: Point2Input,
...
export function intersectLineLine2(
  a0: Point2Input,
  a1: Point2Input,
  b0: Point2Input,
  b1: Point2Input,
...
export function intersectLineCircle2(
  start: Point2Input,
  end: Point2Input,
  center: Point2Input,
  radius: number,
  ...
  return invokeGeometryBackend(
    ...
    [start, end, center, radius, options],
    () => intersectLineCircleReference2(start, end, center, radius, options),
  )
}

export function intersectCircleCircle2(
  centerA: Point2Input,
  radiusA: number,
  centerB: Point2Input,
  radiusB: number,
  ...
  return invokeGeometryBackend(
    ...
    [centerA, radiusA, centerB, radiusB, options],
    () => intersectCircleCircleReference2(centerA, radiusA, centerB, radiusB, options),
  )
}

export function closestPointOnCircle2(
  point: Point2Input,
  center: Point2Input,
  radius: number,
  ...
  const unit: Point2 = tolerance.zero(magnitude) ? [1, 0] : multiply2(radial, 1 / magnitude)
