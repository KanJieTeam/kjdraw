...
export const translation3 = (dx, dy)=>[
        1,
        0,
        0,
        1,
...
export function aroundPoint3(transform, center) {
    ...
    return multiply3(translation3(...resolvedCenter), multiply3(transform, translation3(-resolvedCenter[0], -resolvedCenter[1])));
}
export const rotationAround3 = (angle, center = [
    0,
    0
])=>aroundPoint3(rotation3(Number(angle)), center);
export const scaleAround3 = (sx, sy = sx, center = [
    0,
    0
])=>aroundPoint3(scale3(sx, sy), center);
...
export function similarityScale3(value, tolerance = DEFAULT_TOLERANCE) {
    ...
    if (Math.abs(sx - sy) > threshold || Math.abs(dot2(xAxis, yAxis)) > threshold * Math.max(sx, sy, 1)) {
