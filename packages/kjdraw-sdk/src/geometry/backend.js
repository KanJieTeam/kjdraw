...
export function registerGeometryBackend(backend) {
  ...
  for (const operation of REQUIRED_OPERATIONS) {
    if (typeof backend[operation] !== 'function') throw new KJValidationError(`Geometry backend is missing ${operation}`)
  }
...
export function unregisterGeometryBackend() { activeBackend = null }

export function recordGeometryBackendFailure(error) {
  lastFailure = Object.freeze({ message: error instanceof Error ? error.message : String(error), at: new Date().toISOString() })
  activeBackend = null
}
...
export function getGeometryBackendStatus() {
  return Object.freeze({
    ...
    backend: activeBackend?.identity ?? Object.freeze({ id: 'kjdraw-js-reference', abi: 'kanjie.kjcore.reference.v1', version: '0.2.0', authoritative: false }),
...
export function requireAuthoritativeGeometryBackend() {
  ...
  if (!status.authoritative) throw new KJValidationError('Authoritative KJCore geometry backend is not available')
  return status.backend
}

export function invokeGeometryBackend(operation, args, fallback) {
  const implementation = activeBackend?.[operation]
  return implementation ? implementation(...args) : fallback()
}
