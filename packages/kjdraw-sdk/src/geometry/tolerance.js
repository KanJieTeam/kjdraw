...
export class KJTolerance {
  ...
  equal(a, b) { return Math.abs(a - b) <= this.distanceFor(a, b) }
  zero(value, scale = 1) { return Math.abs(value) <= this.distanceFor(scale) }
  angleEqual(a, b) { return Math.abs(normalizeAngle(a - b)) <= this.angular }
}
