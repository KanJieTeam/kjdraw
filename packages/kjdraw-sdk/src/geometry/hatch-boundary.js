// Generated from hatch-boundary.ts by scripts/build-typescript.mjs. Do not edit directly.
...
export function normalizeHatchSplineEdge(source, label = 'Hatch SPLINE edge') {
    ...
    const definition = normalizeSplineDefinition({
        ...
        controlPoints,
        ...Array.isArray(source.knots) ? {
            knots: source.knots.map(Number)
        } : {},
        ...Array.isArray(source.weights) ? {
            weights: source.weights.map(Number)
        } : {}
    });
    ...
    const tangent = (key)=>source[key] == null ? undefined : point(source[key], `${label} ${key}`);
    const startTangent = tangent('startTangent'), endTangent = tangent('endTangent');
    return {
        type: 'SPLINE',
        degree: definition.degree,
        controlPoints: definition.controlPoints.map((value)=>[
                value[0],
                value[1],
                0
            ]),
        ...
        fitPoints,
        periodic: source.periodic === true || Number(source.periodic) === 1,
        ...startTangent ? {
            startTangent
        } : {},
        ...endTangent ? {
...
export function closedHatchSplineConic(source) {
    let edge;
    try {
        edge = normalizeHatchSplineEdge(source);
    } catch  {
        return null;
    }
    ...
    const expected = [
        ...
        1,
        1
    ], near = (a, b, t = 1e-9)=>Math.abs(a - b) <= t * Math.max(1, Math.abs(a), Math.abs(b));
...
export function sampleHatchSpline(source, stepsPerSpan = 24) {
    const edge = normalizeHatchSplineEdge(source), values = [
        ...new Set(edge.knots.slice(edge.degree, edge.controlPoints.length + 1))
    ], result = [];
