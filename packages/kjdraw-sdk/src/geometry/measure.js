// Generated from measure.ts by scripts/build-typescript.mjs. Do not edit directly.
...
function vertexPoint(vertex) {
    ...
    return vec2(vertex.point, 'polyline vertex');
}
function vertexBulge(vertex) {
    return Array.isArray(vertex) ? 0 : Number(vertex.bulge ?? 0);
}
function polylineVertices(value) {
...
export function polylineLength2(vertices, { closed = false } = {}) {
    ...
    if (vertices.some((vertex)=>Math.abs(vertexBulge(vertex)) >= 1e-15)) {
        return polylineLengthReference2(vertices, {
            closed
        });
    }
    const points = vertices.map(vertexPoint);
    return invokeGeometryBackend('polylineLength2', [
        points,
        {
            closed
        }
    ], ()=>polylineLengthReference2(vertices, {
            closed
        }));
}
...
export function polylineArea2(vertices) {
    ...
    const points = vertices.map(vertexPoint);
    return invokeGeometryBackend('polylineArea2', [
        points
    ], ()=>polylineAreaReference2(vertices));
}
...
export function entityLength2(object) {
    ...
    switch(type){
        case 'LINE':
            return {
                value: distance2(payload.start, payload.end),
                approximate: false
            };
        case 'RAY':
        case 'XLINE':
        ...
        case 'CIRCLE':
            return {
                value: Math.PI * 2 * Number(payload.radius),
                approximate: false
            };
        case 'ARC':
            return {
                value: Math.abs(arcSweep(payload)) * Number(payload.radius),
                approximate: false
            };
        case 'LWPOLYLINE':
        case 'POLYLINE':
        ...
        case 'TRACE':
            return {
                value: polylineLength2(polylineVertices(payload.vertices), {
                    closed: true
                }),
                approximate: false
            };
        case 'SPLINE':
            return {
                value: splineLength2(payload),
                approximate: true,
                algorithm: 'adaptive-rational-bspline'
            };
        case 'ELLIPSE':
            return {
                value: ellipseArcLength2(payload),
                approximate: true,
...
export function entityArea2(object) {
    ...
    switch(type){
        case 'CIRCLE':
            return {
                value: Math.PI * Number(payload.radius) ** 2,
                signed: false,
                approximate: false
            };
        case 'ELLIPSE':
