...
export function projectDimension(payload, style = {}) {
    ...
    if (type === 'ALIGNED' || type === 'ROTATED' || type === 'LINEAR') {
        ...
        if (payload.definitionPoints.some((value)=>!Array.isArray(value) || value.length > 2 && (!Number.isFinite(Number(value[2])) || Math.abs(Number(value[2])) > 1e-12))) return null;
        for (const key of [
            'normal',
            'extrusionDirection'
        ]){
        ...
        if (type === 'ANGULAR') {
            const rays = [
                startAngle,
                positive(startAngle + Math.PI),
                endAngle,
                positive(endAngle + Math.PI)
            ].sort((a, b)=>a - b);
            let selected = false;
        ...
        for (const [origin, direction, q] of [
            [
                origin1,
                u,
                q1
            ...
            lines.push([
                plus(origin, direction, sign * gap),
                plus(q, direction, sign * beyond)
            ]);
        }
        arcs.push({
            center,
        ...
        const mid = startAngle + sweep / 2, radial = [
            Math.cos(mid),
            Math.sin(mid)
        ];
        textPoint = plus(center, radial, radius + height * .65);
