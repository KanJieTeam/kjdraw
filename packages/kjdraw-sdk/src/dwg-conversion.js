// Generated from dwg-conversion.ts by scripts/build-typescript.mjs. Do not edit directly.
...
export const KJ_DWG_CONVERSION_DEFAULT_LIMITS = Object.freeze({
...
function normalizeTarget(value) {
    ...
    if (!TARGETS.has(target)) invalid('DWG conversion target must be DXF or KJD');
    return target;
}
function throwIfAborted(signal, phase) {
    if (signal?.aborted) invalid(`DWG ${phase} aborted`);
}
async function sourceBytes(source, maximum) {
    let bytes;
...
async function firstBytes(source, count) {
...
function report(options, progress) {
    options.onProgress?.(deepFreeze({
        ...progress
    }));
}
function normalizeConversionProgress(value) {
    if (!value || ![
        'validate',
        'upload',
        'convert',
        ...
        'percent',
        'steps'
    ].includes(value.unit)) invalid('Invalid DWG conversion progress');
...
function diagnostics(value, label) {
    ...
    return value.map((item)=>{
        ...
        return boundedText(item, `DWG ${label} item`, MAX_DIAGNOSTIC_LENGTH, {
            allowEmpty: false
        });
    });
}
async function resultBytes(source, maximum) {
    let bytes;
...
function assertResultEnvelope(bytes, target) {
    ...
    for(let index = 0; index < lines.length; index += 2){
        ...
        if (marker === 'SECTION') {
            ...
            sections -= 1;
            if (sections < 0) invalid('DWG converter returned malformed DXF section structure');
        } else if (marker === 'EOF') {
            if (sections !== 0 || index + 2 !== lines.length) invalid('DWG converter returned malformed DXF section structure');
            eof = true;
        }
    }
    if (!sectionCount || sections !== 0 || !eof) invalid('DWG converter returned malformed DXF section structure');
}
function attachProvenance(document, provenance) {
    const state = document.toJSON();
    state.metadata.custom = {
        ...state.metadata.custom,
        kjdrawImport: clone(provenance)
    };
    return KJDocument.open(state);
}
export function getDwgConversionProvenance(document) {
...
export function createDwgConversionFileAdapter(options) {
    ...
    const outputFormats = [
        ...new Set(sourceProvider.outputFormats.map(normalizeTarget))
    ];
    ...
    return defineFileAdapter({
        ...
        formats: {
            DWG: {
                ...
                notes: [
                    'Host-provided DWG conversion to validated editable DXF or KJD; fidelity depends on the selected provider'
                ]
            }
        },
        capabilities: {
        ...
        preservation: {
            ...
            diagnostics: 'document-metadata',
            fidelity: 'provider-reported-and-validated-subset'
        },
        sniff: async (source)=>{
            try {
        ...
        read: async (source, rawOptions = {})=>{
            ...
            const candidateName = readOptions.fileName ?? (source && typeof source === 'object' && 'name' in source ? source.name : undefined) ?? 'drawing.dwg';
            ...
            const conversionProgress = (value)=>{
            ...
            const imported = target === 'DXF' ? await dxf.read?.(convertedBytes, nestedOptions) : await kjd.read?.(convertedBytes, nestedOptions);
            ...
            const provenance = {
                ...
                provider: {
                    id: providerId,
                    version: result.providerVersion == null ? providerVersion : boundedText(result.providerVersion, 'DWG result provider version', MAX_ID_LENGTH),
                    locality
                },
                ...
                targetBytes: convertedBytes.byteLength,
                warnings,
                approximations
            };
