...
function columnLayout(input) {
    ...
    if (value.drawingOrigin != null) {
        if (!Array.isArray(value.drawingOrigin) || value.drawingOrigin.length !== 2) throw new KJValidationError('Geology: column drawing origin needs two coordinates');
        drawingOrigin = [
            projectCoordinate(value.drawingOrigin[0], 'column drawing origin X'),
    ...
    const sourceLocalTextStyle = (raw, label)=>{
        if (!raw || typeof raw !== 'object' || Array.isArray(raw) || Object.keys(raw).sort().join(',') !== 'bigFontFile,dxfFlags,fixedHeight,fontFamily,fontFile,generationFlags,name,obliqueAngleDegrees,widthFactor') throw new KJValidationError(`Geology: ${label} text style needs an exact declarative field-grid schema`);
        const rule = raw;
        ...
        if (fixedHeight !== 0 || widthFactor < 0.5 || widthFactor > 1.5 || obliqueAngleDegrees < -45 || obliqueAngleDegrees > 45 || !Number.isSafeInteger(dxfFlags) || dxfFlags < 0 || dxfFlags > 255 || !Number.isSafeInteger(generationFlags) || generationFlags < 0 || generationFlags > 7) throw new KJValidationError(`Geology: ${label} text style is unreadable or unsafe`);
        return {
            name,
            fontFamily,
            fontFile,
    ...
    if (value.roleTextStyles != null) {
        if (!isFieldGrid || !value.roleTextStyles || typeof value.roleTextStyles !== 'object' || Array.isArray(value.roleTextStyles)) throw new KJValidationError('Geology: role text styles need a declarative field-grid object');
        const roles = Object.keys(value.roleTextStyles);
        if (!roles.length || roles.some((role)=>![
                'layerName',
                'patternLabel'
            ].includes(role))) throw new KJValidationError('Geology: role text styles need at least one supported semantic role');
        const raw = value.roleTextStyles;
        roleTextStyles = Object.fromEntries(roles.map((role)=>[
                role,
                sourceLocalTextStyle(raw[role], `${role} role`)
            ]));
    }
    const styleNames = [
        defaultTextStyle,
        ...Object.values(roleTextStyles ?? {})
    ].filter((item)=>item != null).map((item)=>item.name.toLocaleLowerCase('en-US'));
    ...
    if (value.groundwaterAnnotationStyle != null) {
        ...
        if (!isFieldGrid || !value.groundwaterAnnotationStyle || typeof value.groundwaterAnnotationStyle !== 'object' || Array.isArray(value.groundwaterAnnotationStyle) || ![
            ...
            'dateOffset,fieldRole,gap,guide,guideEndpointOffset,markerHeight,markerOffset,placements,textHeight,textWidthFactor,valueOffset'
        ...
        let guideEndpointOffset;
        if (rule.guideEndpointOffset != null) {
            if (rule.guide !== 'field-top-to-reading' || !Array.isArray(rule.guideEndpointOffset) || rule.guideEndpointOffset.length !== 2) throw new KJValidationError('Geology: groundwater guide endpoint offset needs a declared guide and two coordinates');
            guideEndpointOffset = rule.guideEndpointOffset.map((coordinate, index)=>numeric(coordinate, `groundwater guide endpoint offset ${index + 1}`));
            if (guideEndpointOffset.some((coordinate)=>coordinate < -2 || coordinate > 2)) throw new KJValidationError('Geology: groundwater guide endpoint offset is outside its physical lane');
        }
    ...
    let descriptionPlacements;
    if (value.descriptionPlacements != null) {
        if (!isFieldGrid || !descriptionTextStyle || !Array.isArray(value.descriptionPlacements) || value.descriptionPlacements.length < 1 || value.descriptionPlacements.length > 80) throw new KJValidationError('Geology: description placements need a bounded field-grid knowledge rule');
        descriptionPlacements = value.descriptionPlacements.map((raw, index)=>{
            if (!raw || typeof raw !== 'object' || Array.isArray(raw)) throw new KJValidationError(`Geology: description placement rule ${index + 1} must be an object`);
            const item = raw;
            if (![
                'boundaryRole,groupId,offsetMm',
                'boundaryRole,groupId,offsetMm,precedingBoundaryClearanceMm'
            ].includes(Object.keys(item).sort().join(',')) || ![
                'top',
                'bottom',
                'midpoint'
            ].includes(String(item.boundaryRole))) throw new KJValidationError('Geology: description placement rules need an exact group-boundary schema');
            ...
            let precedingBoundaryClearanceMm;
            if (item.precedingBoundaryClearanceMm != null) {
                const clearance = item.precedingBoundaryClearanceMm;
                if (!clearance || typeof clearance !== 'object' || Array.isArray(clearance) || Object.keys(clearance).sort().join(',') !== 'left,right') throw new KJValidationError('Geology: preceding description boundary clearance needs exact left and right measurements');
                ...
                if (!descriptionBoundaryStyle || left < 0.2 || left > 5 || right < 0.2 || right > 5) throw new KJValidationError('Geology: preceding description boundary clearance is unreadable');
                precedingBoundaryClearanceMm = {
                    left,
                    right
                };
            }
            return {
                groupId,
                boundaryRole: item.boundaryRole,
                offsetMm,
                ...precedingBoundaryClearanceMm ? {
                    precedingBoundaryClearanceMm
                } : {}
        ...
        if (new Set(descriptionPlacements.map((item)=>item.groupId)).size !== descriptionPlacements.length) throw new KJValidationError('Geology: description placement group IDs must be unique');
    }
    let hatchLayerStyle;
    if (value.hatchLayerStyle != null) {
        if (!value.hatchLayerStyle || typeof value.hatchLayerStyle !== 'object' || Array.isArray(value.hatchLayerStyle) || Object.keys(value.hatchLayerStyle).sort().join(',') !== 'color,lineweight') throw new KJValidationError('Geology: hatch layer style needs an exact declarative schema');
        const rule = value.hatchLayerStyle;
        const color = numeric(rule.color, 'hatch layer color'), lineweight = numeric(rule.lineweight, 'hatch layer lineweight');
        const lineweights = new Set([
            -3,
            -2,
            -1,
            0,
        ...
        if (!Number.isSafeInteger(color) || color < 1 || color > 255 || !Number.isSafeInteger(lineweight) || !lineweights.has(lineweight)) throw new KJValidationError('Geology: hatch layer style is outside supported CAD values');
        hatchLayerStyle = {
            color,
            lineweight
        };
...
function sectionLayout(input) {
    ...
    const expectedKeys = [
        ...scalarKeys,
        'footerGrid',
        hasOuterMargins ? 'outerMargins' : 'outerMargin',
        hasInnerMargins ? 'innerMargins' : 'innerMargin',
        ...value.drawingOrigin == null ? [] : [
            'drawingOrigin'
        ],
        ...
        ...value.elevationTickSequence == null ? [] : [
            'elevationTickSequence'
        ],
    ...
    if (value.sectionTextStyle != null) {
        ...
        const textStyleKeys = [
            ...
            'station',
            'stationLabel',
            ...value.sectionTextStyle.holeEndDate == null ? [] : [
                'holeEndDate'
            ]
        ];
        if (Object.keys(value.sectionTextStyle).sort().join(',') !== textStyleKeys.sort().join(',')) throw new KJValidationError('Geology: section text style needs every declared text role');
        const supplied = value.sectionTextStyle;
        const holeIdentifier = supplied.holeIdentifier;
        const holeEndDate = supplied.holeEndDate;
        ...
        const holeEndDatePlacement = holeEndDate == null ? undefined : parseSectionTextPlacement(holeEndDate, 'hole end date', [
            'format',
            ...holeEndDate.labelOverrides == null ? [] : [
                'labelOverrides'
            ]
        ]);
        ...
        let holeEndDateLabelOverrides;
        if (holeEndDate?.labelOverrides != null) {
            if (!Array.isArray(holeEndDate.labelOverrides) || holeEndDate.labelOverrides.length < 1 || holeEndDate.labelOverrides.length > 256) throw new KJValidationError('Geology: section hole end date label overrides need bounded source facts');
            const identities = new Set();
            holeEndDateLabelOverrides = holeEndDate.labelOverrides.map((raw, index)=>{
                if (!raw || typeof raw !== 'object' || Array.isArray(raw) || Object.keys(raw).sort().join(',') !== 'endDate,holeId,placement') throw new KJValidationError(`Geology: section hole end date label override ${index + 1} needs an exact fact schema`);
                const item = raw;
                ...
                return {
                    holeId,
                    endDate,
                    placement: parseSectionTextPlacement(item.placement, `hole end date label override ${index + 1}`)
                };
            });
        }
        ...
        if (collar.labelOverrides != null) {
            ...
            collarLabelOverrides = collar.labelOverrides.map((raw, index)=>{
                if (!raw || typeof raw !== 'object' || Array.isArray(raw)) throw new KJValidationError(`Geology: section collar elevation label override ${index + 1} needs an exact fact schema`);
                const item = raw, hasPlacement = item.placement != null;
                const expectedKeys = [
                    'elevation',
                    'holeId',
                    ...hasPlacement ? [
                ...
                if (Object.keys(item).sort().join(',') !== expectedKeys.sort().join(',')) throw new KJValidationError(`Geology: section collar elevation label override ${index + 1} needs an exact fact schema`);
                ...
                if (elevation < -10000 || elevation > 10000) throw new KJValidationError(`Geology: section collar elevation label override ${index + 1} is out of bounds`);
                return {
                    holeId,
                    elevation,
                    ...hasPlacement ? {
                        placement: parseSectionTextPlacement(item.placement, `collar elevation label override ${index + 1}`)
                    } : {}
                };
            });
        }
        ...
        if (interval.labelOverrides != null) {
            if (interval.format !== 'depth-elevation' || !Array.isArray(interval.labelOverrides) || interval.labelOverrides.length < 1 || interval.labelOverrides.length > 256) throw new KJValidationError('Geology: section interval bottom label overrides need bounded depth-elevation facts');
            const identities = new Set();
            labelOverrides = interval.labelOverrides.map((raw, index)=>{
                if (!raw || typeof raw !== 'object' || Array.isArray(raw)) throw new KJValidationError(`Geology: section interval bottom label override ${index + 1} needs an exact fact schema`);
                const item = raw, hasPlacement = item.placement != null;
                const expectedKeys = [
                    'depth',
                    'elevation',
                ...
                if (Object.keys(item).sort().join(',') !== expectedKeys.sort().join(',')) throw new KJValidationError(`Geology: section interval bottom label override ${index + 1} needs an exact fact schema`);
                ...
                if (depth < 0 || depth > 10000 || elevation < -10000 || elevation > 10000) throw new KJValidationError(`Geology: section interval bottom label override ${index + 1} is out of bounds`);
                return {
                    holeId,
                    intervalId,
                    depth,
                    elevation,
                    ...hasPlacement ? {
                        placement: parseSectionTextPlacement(item.placement, `interval bottom label override ${index + 1}`)
                    } : {}
                };
            });
        }
        sectionTextStyle = {
            ...
            ...holeEndDate && holeEndDatePlacement ? {
                holeEndDate: {
                    ...holeEndDatePlacement,
                    format: holeEndDate.format,
                    ...holeEndDateLabelOverrides ? {
                        labelOverrides: holeEndDateLabelOverrides
                    } : {}
                }
            } : {},
            collarElevation: {
                ...
                ...collarLabelOverrides ? {
                    labelOverrides: collarLabelOverrides
                } : {}
            },
            intervalBottom: {
                ...intervalPlacement,
    ...
    let sourceBackedBoundaryPolylines;
    if (value.sourceBackedBoundaryPolylines != null) {
        if (!Array.isArray(value.sourceBackedBoundaryPolylines) || value.sourceBackedBoundaryPolylines.length < 1 || value.sourceBackedBoundaryPolylines.length > 256) throw new KJValidationError('Geology: section source-backed boundary polylines need 1-256 explicit facts');
        ...
        sourceBackedBoundaryPolylines = value.sourceBackedBoundaryPolylines.map((rawBoundary, boundaryIndex)=>{
            if (!rawBoundary || typeof rawBoundary !== 'object' || Array.isArray(rawBoundary) || Object.keys(rawBoundary).sort().join(',') !== 'points,primitive') throw new KJValidationError('Geology: section source-backed boundary polyline needs an exact primitive and points');
            const boundary = rawBoundary;
            const closed = boundary.primitive === 'closed-polyline';
            if (!closed && boundary.primitive !== 'open-polyline' || !Array.isArray(boundary.points) || boundary.points.length < (closed ? 3 : 2) || boundary.points.length > 32) throw new KJValidationError('Geology: section source-backed boundary polyline geometry is invalid');
            const points = boundary.points.map((rawPoint, pointIndex)=>{
                if (!Array.isArray(rawPoint) || rawPoint.length !== 2) throw new KJValidationError('Geology: section source-backed boundary point needs station and elevation');
                return [
                    numeric(rawPoint[0], `section source-backed boundary ${boundaryIndex + 1} station ${pointIndex + 1}`),
                    numeric(rawPoint[1], `section source-backed boundary ${boundaryIndex + 1} elevation ${pointIndex + 1}`)
                ];
            });
            ...
            if (distinct.size < (closed ? 3 : 2) || closed && area < 1e-8) throw new KJValidationError('Geology: section source-backed boundary polyline is degenerate');
    ...
    let elevationTickSequence;
    if (value.elevationTickSequence != null) {
        if (!value.elevationTickSequence || typeof value.elevationTickSequence !== 'object' || Array.isArray(value.elevationTickSequence) || Object.keys(value.elevationTickSequence).sort().join(',') !== 'maximumElevation,minimumElevation,startElevation,step') throw new KJValidationError('Geology: section elevation tick sequence needs exact start, step and range facts');
        const supplied = value.elevationTickSequence;
        ...
        if (step < 0.1 || step > 100 || minimumElevation > startElevation || startElevation > maximumElevation || maximumElevation - minimumElevation > 1000 || tickCount < 1 || tickCount > 256) throw new KJValidationError('Geology: section elevation tick sequence is out of bounds');
        elevationTickSequence = {
            startElevation,
            step,
            minimumElevation,
    ...
    let sourceBackedBands;
    if (value.sourceBackedBands != null) {
        if (!Array.isArray(value.sourceBackedBands) || value.sourceBackedBands.length < 1 || value.sourceBackedBands.length > 128) throw new KJValidationError('Geology: section source-backed bands need 1-128 explicit facts');
        const seenBands = new Set();
        sourceBackedBands = value.sourceBackedBands.map((rawBand, bandIndex)=>{
            if (!rawBand || typeof rawBand !== 'object' || Array.isArray(rawBand) || Object.keys(rawBand).sort().join(',') !== 'points,sourceHoleId,sourceIntervalId') throw new KJValidationError('Geology: section source-backed band needs an exact source reference and polygon');
            const band = rawBand;
            ...
            if (!Array.isArray(band.points) || band.points.length < 3 || band.points.length > 32) throw new KJValidationError('Geology: section source-backed band polygon needs 3-32 vertices');
            const points = band.points.map((rawPoint, pointIndex)=>{
                if (!Array.isArray(rawPoint) || rawPoint.length !== 2) throw new KJValidationError('Geology: section source-backed band vertex needs station and elevation');
                return [
                    numeric(rawPoint[0], `section source-backed band ${bandIndex + 1} station ${pointIndex + 1}`),
                    numeric(rawPoint[1], `section source-backed band ${bandIndex + 1} elevation ${pointIndex + 1}`)
                ];
            });
            const area = Math.abs(points.reduce((sum, point, index)=>{
                const next = points[(index + 1) % points.length];
    ...
    const parseFrameRule = (raw, label)=>{
        ...
        if (![
            ...
            'top-right',
            'top-left'
        ].includes(rule.startCorner)) throw new KJValidationError(`Geology: section ${label} frame start corner is invalid`);
        ...
        if (rule.primitive === 'line-segments') return {
            primitive: rule.primitive,
            startCorner: rule.startCorner,
            winding: rule.winding
        };
    ...
    if (value.frameStyle != null) {
        if (!value.frameStyle || typeof value.frameStyle !== 'object' || Array.isArray(value.frameStyle) || Object.keys(value.frameStyle).sort().join(',') !== 'inner,outer') throw new KJValidationError('Geology: section frame style needs exact inner and outer rules');
        const supplied = value.frameStyle;
        frameStyle = {
            outer: parseFrameRule(supplied.outer, 'outer'),
            inner: parseFrameRule(supplied.inner, 'inner')
        };
    }
    const parseMargins = (raw, fallback, label)=>{
        if (raw == null) {
        ...
        if (!raw || typeof raw !== 'object' || Array.isArray(raw) || Object.keys(raw).sort().join(',') !== 'bottom,left,right,top') throw new KJValidationError(`Geology: section ${label} margins need exact left, right, bottom and top values`);
        const supplied = raw;
        return {
            left: numeric(supplied.left, `section ${label} left margin`),
            right: numeric(supplied.right, `section ${label} right margin`),
            bottom: numeric(supplied.bottom, `section ${label} bottom margin`),
            top: numeric(supplied.top, `section ${label} top margin`)
        };
    };
    ...
    if (value.drawingOrigin != null) {
        if (!Array.isArray(value.drawingOrigin) || value.drawingOrigin.length !== 2) throw new KJValidationError('Geology: section drawing origin needs two coordinates');
        drawingOrigin = [
            numeric(value.drawingOrigin[0], 'section drawing origin X'),
...
function drawingBuilder(input, templateId, expectedRevision, hatches = {}, defaultTextStyle, roleTextStyles, drawingOrigin = [
    0,
    0
], hatchLayerStyle) {
    ...
    const layers = names.map((name, index)=>({
            ...
            name,
            color: index === 2 && hatchLayerStyle ? hatchLayerStyle.color : [
                7,
                7,
                8,
                7,
            ...
            linetypeId,
            lineweight: index === 2 && hatchLayerStyle ? hatchLayerStyle.lineweight : [
                35,
                35,
                18,
                18,
    ...
    const styleResource = (id, style)=>({
            ...
            payload: {
                ...
                obliqueAngle: style.obliqueAngleDegrees * Math.PI / 180,
    ...
    const roleTextStyleIds = Object.fromEntries(Object.entries(roleTextStyles ?? {}).map(([role])=>[
            role,
            `${prefix}-text-style-${role}`
        ]));
    const textStyles = [
        ...defaultTextStyle ? [
            styleResource(textStyleId, defaultTextStyle)
        ] : [],
        ...Object.entries(roleTextStyles ?? {}).map(([role, style])=>styleResource(roleTextStyleIds[role], style))
    ];
    const textStyleFor = (role)=>role == null ? textStyleId : roleTextStyleIds[role] ?? textStyleId;
    const entities = [];
    ...
    const text = (layer, x, y, value, height = 2.6, centered = false, widthFactor, verticalAlignment, styleRole)=>add('TEXT', layer, {
            position: shifted(x, y),
            text: value,
            height,
            ...textStyleFor(styleRole) ? {
                styleId: textStyleFor(styleRole)
            } : {},
            ...widthFactor == null ? {} : {
                widthFactor
            },
    ...
    const placedText = (layer, x, y, value, height, widthFactor, horizontalAlignment, verticalAlignment, rotation, styleRole)=>add('TEXT', layer, {
            position: shifted(x, y),
            text: value,
            height,
            widthFactor,
...
export function compileGeologyColumn(input) {
    ...
    const { paperHeight: pageHeight, paperWidth: pageWidth, drawingOrigin, left, right, columns, observationColumns, headerDepth, headerRowHeight, fieldHeaderHeight, footerReserve, labels, displayAliases, headerGrid, footerGrid, fieldGrid, sptDisplayCap, titleHeight, titleTextStyle, textFlow, textHeights, intervalDepthTextStyle, majorGroupValueStyle, defaultTextStyle, roleTextStyles, stratigraphicNotationStyle, descriptionTextStyle, sampleMarkerStyle, sampleAnnotationStyle, sampleRangeBaselineStyle, sampleRangeTextFormat, groundwaterAnnotationStyle, patternLabelStyle, titleMarginFacts, frameStyle, descriptionBoundaryStyle, descriptionPlacements, hatchLayerStyle, formTopology, layerNumberStyle, sourceTemplate } = layout;
    ...
    if ((strata.some((layer)=>layer.descriptionPlacement != null) || descriptionPlacements) && !descriptionTextStyle) throw new KJValidationError('Geology: description placement facts need a declared field-grid description text style');
    ...
    const g = drawingBuilder(input, 'borehole-column-engineering', input.expectedRevision, patternDefinitions(input.hatchPack, strata), defaultTextStyle, roleTextStyles, drawingOrigin ?? [
        0,
        0
    ], hatchLayerStyle);
    ...
    const descriptionPlacementRules = new Map(descriptionPlacements?.map((item)=>[
            item.groupId,
            item
        ]) ?? []);
    if (descriptionPlacementRules.size) {
        ...
        for (const rule of descriptionPlacements){
            ...
            if (!group || !group.principal.description) throw new KJValidationError(`Geology: description placement group ${rule.groupId} has no supplied principal description`);
            if (group.principal.descriptionPlacement) throw new KJValidationError(`Geology: description placement group ${rule.groupId} is declared by both facts and knowledge`);
            if (rule.precedingBoundaryClearanceMm && group === groups[0]) throw new KJValidationError('Geology: the first description cannot declare a preceding boundary clearance');
        }
    }
    ...
    if (fieldGrid) {
        ...
        const emitPlacedFieldText = (item, anchorY, value, placement)=>{
            ...
            const horizontalAlignment = placement.horizontalAlignment === 'left' ? 0 : placement.horizontalAlignment === 'center' ? 1 : 2;
            const verticalAlignment = placement.verticalAlignment === 'baseline' ? 0 : 2;
            g.placedText(3, x, y, value, placement.height, placement.textWidthFactor, horizontalAlignment, verticalAlignment, 0, item.role === 'layerName' ? 'layerName' : undefined);
            const textBottom = placement.verticalAlignment === 'middle' ? y - placement.height / 2 : y;
            textBoxes.push({
                role: item.role,
                left: left - 0.25,
        ...
        const writeCore = (id, groupTop, groupBottom, principal)=>{
            ...
            if (principal.description && (grouped || principal.descriptionSource !== 'layer-definition' || definitionAnchors.get(`${principal.code}\u0000${principal.description}`) === principal)) writeGridDescription(principal.description, yTop, yBottom, coreIndex, `major group ${id}`, principal.descriptionPlacement ?? descriptionPlacementRules.get(id));
        };
        for (const layer of strata){
        ...
        if (descriptionBoundaryStyle) {
            ...
            for (const [index, boundary] of majorBoundaries.entries()){
                ...
                const leftDescriptionY = nextDescriptionTop == null ? boundary.y : Math.min(boundary.y, nextDescriptionTop + (sourceClearance?.left ?? descriptionBoundaryStyle.clearance));
                const rightDescriptionY = nextDescriptionTop == null ? boundary.y : Math.min(boundary.y, nextDescriptionTop + (sourceClearance?.right ?? descriptionBoundaryStyle.clearance));
                if (leftDescriptionY < bottom - 1e-9 || leftDescriptionY > boundary.y + 1e-9 || rightDescriptionY < bottom - 1e-9 || rightDescriptionY > boundary.y + 1e-9) throw new KJValidationError('Geology: stepped description boundary exceeds the declared body');
                g.poly(1, [
                    [
                        left,
                        boundary.y
...
export function compileGeologySection(input) {
    ...
    const frame = (margins, rule)=>{
        ...
        const names = rule.winding === 'clockwise' ? [
            'bottom-left',
            'top-left',
            'top-right',
            'bottom-right'
        ...
        const start = names.indexOf(rule.startCorner), ordered = [
            ...names.slice(start),
            ...names.slice(0, start)
    ...
    const holeEndDateLabelOverrides = new Map((holeEndDateRole?.labelOverrides ?? []).map((item)=>[
            item.holeId,
            item
        ]));
    if (holeEndDateRole) {
    ...
    for (const item of holeEndDateRole?.labelOverrides ?? []){
        ...
        if (hole?.endDate == null) throw new KJValidationError('Geology: section hole end date label override references an unknown hole or missing supplied end date');
        if (item.endDate !== visibleHoleEndDate(hole.endDate)) throw new KJValidationError('Geology: section hole end date label override does not match its supplied end date');
    }
    ...
    for (const item of sectionText?.collarElevation.labelOverrides ?? []){
        ...
        if (!supplied) throw new KJValidationError('Geology: section collar elevation label override references an unknown supplied hole');
        if (Math.abs(item.elevation - supplied.collarElevation) > 0.005 + 1e-9) throw new KJValidationError('Geology: section collar elevation label override does not match its supplied collar');
    }
    for (const item of sectionText?.intervalBottom.labelOverrides ?? []){
        ...
        if (!interval) throw new KJValidationError('Geology: section interval bottom label override references an unknown supplied interval');
    ...
    for (const [bandIndex, band] of (layout.sourceBackedBands ?? []).entries()){
        ...
        if (source.patternVisibility === 'boundary-only') throw new KJValidationError(`Geology: source-backed band ${bandIndex + 1} references a boundary-only interval`);
        const points = band.points.map(([station, elevation])=>[
                originX + (station - holes[0].station) * hs,
                layout.plotBottom + (elevation - datum) * vs
            ]);
        if (points.some(([px, py])=>px < layout.innerMargins.left - 1e-9 || px > layout.paperWidth - layout.innerMargins.right + 1e-9 || py < layout.innerMargins.bottom - 1e-9 || py > layout.plotTop + 1e-9)) throw new KJValidationError(`Geology: source-backed band ${bandIndex + 1} leaves the bounded drawing region`);
    ...
    for (const [boundaryIndex, boundary] of (layout.sourceBackedBoundaryPolylines ?? []).entries()){
        ...
        if (points.some(([px, py])=>px < layout.innerMargins.left - 1e-9 || px > layout.paperWidth - layout.innerMargins.right + 1e-9 || py < layout.innerMargins.bottom - 1e-9 || py > layout.plotTop + 1e-9)) throw new KJValidationError(`Geology: source-backed boundary polyline ${boundaryIndex + 1} leaves the bounded section region`);
        const closed = boundary.primitive === 'closed-polyline';
