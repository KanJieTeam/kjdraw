...
function sourceBytes(source: KjpSource): Uint8Array {
...
export class BrowserKjpFileBinding {
  #baselineSha256: string | null = null
  ...
  async write(data: KjpSource): Promise<{ name: string; bytes: number; manifest: KjpOpenResult['manifest'] }> {
    ...
    if (this.#baselineSha256) {
      ...
      if (actualSha256 !== this.#baselineSha256) throw new KJFileConflictError(this.#baselineSha256, actualSha256, { name: handle.name, phase: 'before-write' })
    }
    ...
    if (committedSha256 !== expectedSha256) throw new KJFileConflictError(expectedSha256, committedSha256, { name: handle.name, phase: 'after-write' })
    const reopened = await openKjpPackage(committed)
    this.#baselineSha256 = committedSha256
    return { name: handle.name, bytes: committed.byteLength, manifest: reopened.manifest ?? expectedProject.manifest }
  }
