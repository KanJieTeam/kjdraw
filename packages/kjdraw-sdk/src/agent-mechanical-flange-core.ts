...
const MAX_AUXILIARY_CURVES = 1024
...
export interface KJFlangeSectionHatchSplineEdge {
  kind: 'spline'
  degree: number
  controlPoints: { station: number; offset: number }[]
  knots?: number[]
  weights?: number[]
  fitPoints?: { station: number; offset: number }[]
  periodic?: boolean
  startTangent?: Point2
  endTangent?: Point2
}
...
export type KJFlangeSymbolMember =
  ...
  | { kind: 'instance'; symbolKey: string; position: Point2Or3; scale?: Point2Or3; rotation?: number; role: KJFlangeAuxiliaryLine['role']; entityStyleKey?: string }
...
function validate(document: Document, source: KJAgentMechanicalFlangeCoreInput) {
  ...
  if ((end.outlineSegments as unknown[] | undefined)?.length && (end.outlineSegments as unknown[]).length > 256) throw new KJValidationError('input.endView.outlineSegments exceed their 256-segment budget')
  ...
  if ((side?.outlineSegments as unknown[] | undefined)?.length && (side!.outlineSegments as unknown[]).length > 512) throw new KJValidationError('input.sideViewAxis.outlineSegments exceed their 512-segment budget')
  ...
  const sectionHatches: KJFlangeSectionHatch[] = ((side?.sectionHatches ?? []) as unknown[]).map((value, hatchIndex) => {
    ...
    const loopSource = hasEdges ? [{ edges: hatch.edges, external: false, flags: 0 }] : hatch.boundaryLoops
    ...
    const boundaryLoops = (loopSource as unknown[]).map((value, loopIndex) => {
      const loopLabel = `${label}.boundaryLoops[${loopIndex}]`, loop = plain(value, loopLabel)
      exact(loop, ['edges', 'external', 'flags'], loopLabel)
      ...
      const edges = loop.edges.map((value, edgeIndex) => {
        ...
        if (edge.kind === 'spline') {
          ...
          const fitPoints = (edge.fitPoints ?? []).map((pointValue: unknown, pointIndex: number) => localPoint(pointValue, `${edgeLabel}.fitPoints[${pointIndex}]`))
          ...
          const normalized = normalizeHatchSplineEdge({ type: 'SPLINE', degree,
            controlPoints: controlPoints.map(point => [point.station, point.offset, 0]), knots: edge.knots, weights: edge.weights,
            fitPoints: fitPoints.map(point => [point.station, point.offset, 0]), periodic: edge.periodic === true,
            ...(startTangent == null ? {} : { startTangent: [...startTangent, 0] }), ...(endTangent == null ? {} : { endTangent: [...endTangent, 0] }) }, edgeLabel)
          splinePoints += normalized.controlPoints.length + normalized.fitPoints.length
          return { kind: 'spline' as const, degree: normalized.degree, controlPoints, knots: [...normalized.knots], weights: [...normalized.weights], fitPoints,
            periodic: normalized.periodic, ...(startTangent == null ? {} : { startTangent }), ...(endTangent == null ? {} : { endTangent }) }
        }
        throw new KJValidationError(`${edgeLabel}.kind is invalid`)
      })
      return { external: loop.external === true, flags, edges }
    })
  ...
  const featureControlFrames: KJFlangeFeatureControlFrame[] = ((input.featureControlFrames ?? []) as unknown[]).map((value, index) => {
    const label = `input.featureControlFrames[${index}]`, frame = plain(value, label); exact(frame, ['position', 'rows', 'datumColumnCount', 'trailingRowBreak', 'xAxisDirection', 'styleKey', 'role'], label)
    ...
    const rows = (frame.rows as unknown[]).map((rowValue, rowIndex) => {
      ...
      if ((row.datumReferences as unknown[] | undefined)?.length && (row.datumReferences as unknown[]).length > datumColumnCount) throw new KJValidationError(`${rowLabel}.datumReferences exceed datumColumnCount`)
      ...
      const arrangedDatumReferences = datumReferences.map(datum => {
        ...
        const arranged = { ...datum, slot: nextSlot }
      ...
      return { characteristic: row.characteristic as KJFlangeGeometricCharacteristic, tolerance: row.tolerance, diameterZone: row.diameterZone === true,
        ...(condition == null ? {} : { materialCondition: condition }), datumReferences: arrangedDatumReferences }
    })
    ...
    return { position: point(frame.position, `${label}.position`), rows, datumColumnCount, trailingRowBreak, xAxisDirection, ...(styleKey == null ? {} : { styleKey }), role: frame.role as KJFlangeFeatureControlFrame['role'] }
  })
    ...
    const boundaryLoops = (loopSource as unknown[]).map((value, loopIndex) => {
      ...
      const edges = loop.edges.map((value, edgeIndex) => {
        ...
        if (edge.kind === 'spline') {
          ...
          const fitPoints = (edge.fitPoints ?? []).map((pointValue: unknown, pointIndex: number) => point(pointValue, `${edgeLabel}.fitPoints[${pointIndex}]`))
          const startTangent = edge.startTangent == null ? undefined : point(edge.startTangent, `${edgeLabel}.startTangent`)
          const endTangent = edge.endTangent == null ? undefined : point(edge.endTangent, `${edgeLabel}.endTangent`)
          const degree = finite(edge.degree, `${edgeLabel}.degree`, 1, 10)
          if (!Number.isInteger(degree)) throw new KJValidationError(`${edgeLabel}.degree must be an integer`)
          const normalized = normalizeHatchSplineEdge({ type: 'SPLINE', degree, controlPoints: controlPoints.map(value => [...value, 0]),
            knots: edge.knots, weights: edge.weights, fitPoints: fitPoints.map(value => [...value, 0]), periodic: edge.periodic === true,
            ...(startTangent == null ? {} : { startTangent: [...startTangent, 0] }), ...(endTangent == null ? {} : { endTangent: [...endTangent, 0] }) }, edgeLabel)
          splinePoints += normalized.controlPoints.length + normalized.fitPoints.length
          return { kind: 'spline' as const, degree: normalized.degree, controlPoints, knots: [...normalized.knots], weights: [...normalized.weights], fitPoints,
            periodic: normalized.periodic, ...(startTangent == null ? {} : { startTangent }), ...(endTangent == null ? {} : { endTangent }) }
  ...
  if ((input.auxiliaryLines as unknown[] | undefined)?.length && (input.auxiliaryLines as unknown[]).length > MAX_AUXILIARY_LINES) throw new KJValidationError(`input.auxiliaryLines exceed their ${MAX_AUXILIARY_LINES}-line budget`)
  ...
  if ((input.auxiliarySolids as unknown[] | undefined)?.length && (input.auxiliarySolids as unknown[]).length > 128) throw new KJValidationError('input.auxiliarySolids exceed their 128-solid budget')
  ...
  const symbolDefinitions: KJFlangeSymbolDefinition[] = (symbolSource.definitions as unknown[]).map((value, index) => {
    ...
    const members: KJFlangeSymbolMember[] = (definition.members as unknown[]).map((memberValue, memberIndex) => {
      ...
      if (member.kind === 'instance') {
        ...
        if (!Array.isArray(scaleSource) || ![2, 3].includes(scaleSource.length)) throw new KJValidationError(`${memberLabel}.scale must contain two or three coordinates`)
        const scale: Point3 = [finite(scaleSource[0], `${memberLabel}.scale[0]`, 0.000_001, 1_000_000), finite(scaleSource[1], `${memberLabel}.scale[1]`, 0.000_001, 1_000_000), finite(scaleSource[2] ?? 1, `${memberLabel}.scale[2]`, 0.000_001, 1_000_000)]
  ...
  const symbolInstances: KJFlangeSymbolInstance[] = (symbolSource.instances as unknown[]).map((value, index) => {
    ...
    if (!Array.isArray(scaleSource) || ![2, 3].includes(scaleSource.length)) throw new KJValidationError(`${label}.scale must contain two or three coordinates`)
    const scale: Point3 = [finite(scaleSource[0], `${label}.scale[0]`, 0.000_001, 1_000_000), finite(scaleSource[1], `${label}.scale[1]`, 0.000_001, 1_000_000), finite(scaleSource[2] ?? 1, `${label}.scale[2]`, 0.000_001, 1_000_000)]
  ...
  const styleRole = (value: unknown, label: string): KJFlangeStyleRole => {
    ...
    if (role.linetypePattern != null && (!Array.isArray(role.linetypePattern) || role.linetypePattern.length > 32 || [...role.linetypePattern].some((item: unknown) => typeof item !== 'number' || !Number.isFinite(item) || Math.abs(item) > 1_000) || (role.linetypePattern.length > 0 && !role.linetypePattern.some((item: number) => item !== 0)))) throw new KJValidationError(`${label}.linetypePattern is invalid`)
...
export function buildAgentMechanicalFlangeCore(document: Document, source: KJAgentMechanicalFlangeCoreInput) {
  ...
  const projectSideVector = (station: number, offset: number): Point2 => input.orientation === 'vertical' ? [offset, station] : [station, offset]
    ...
    const boundaryLoops = hatch.boundaryLoops!.map(loop => ({ external: loop.external === true, flags: loop.flags ?? 0, edges: loop.edges.map(edge => edge.kind === 'line'
      ...
      : edge.kind === 'arc' ? { type: 'ARC', center: p3(...projectSidePoint(edge.center.station, edge.center.offset)), radius: edge.radius,
        ...
        : edge.kind === 'ellipse' ? { type: 'ELLIPSE', center: p3(...projectSidePoint(edge.center.station, edge.center.offset)),
          ...
          ...(edge.endTangent == null ? {} : { endTangent: p3(...projectSideVector(...edge.endTangent)) }) } ) }))
    emit('HATCH', { boundaryLoops, patternName: hatch.patternName, solid: hatch.solid, associative: false, patternAngle: 0, patternScale: 1,
      patternLines: hatch.patternLines, patternDefinitionAngle: 0, patternDefinitionScale: 1, layerId: style.layerId }, style.name)
  }
  ...
  const blocks = input.symbolDefinitions.map((definition, definitionIndex) => {
    ...
    const members = definition.members.map((member, memberIndex) => {
      ...
      else { type = 'INSERT'; payload = { blockRecordId: symbolBlockByKey.get(member.symbolKey)!.id, position: p3(member.position[0], member.position[1], member.position[2]), scale: [member.scale?.[0] ?? 1, member.scale?.[1] ?? 1, member.scale?.[2] ?? 1], rotation: member.rotation ?? 0,
  ...
  for (const [instanceIndex, instance] of input.symbolInstances.entries()) {
    ...
    entities.push({ type: 'INSERT', payload: stylePayload({ blockRecordId: block.id, position: p3(instance.position[0], instance.position[1], instance.position[2]), scale: [instance.scale?.[0] ?? 1, instance.scale?.[1] ?? 1, instance.scale?.[2] ?? 1],
  ...
  for (const frame of input.featureControlFrames) {
    const encodedRows = frame.rows.map(row => {
      ...
      const cells = Array<string>(frame.datumColumnCount ?? 4).fill('')
...
export function auditMechanicalDimensionPictures(document: KJDocument): {
  passed: boolean
  dimensions: number
  ...
  const samePoint = (a: unknown, b: unknown): boolean => Array.isArray(a) && Array.isArray(b)
  ...
  const dimensions = document.listEntities({ type: 'DIMENSION', ownerId: document.spaces.modelSpaceId })
  ...
  for (const [index, dimension] of dimensions.entries()) {
    ...
    if (!block || usedBlocks.has(block.id)) { findings.push(`${prefix}:MISSING_OR_SHARED_PICTURE_BLOCK`); continue }
    ...
    if (!projection) { findings.push(`${prefix}:UNPROJECTABLE_DIMENSION`); continue }
    if (payload.measurement != null && !near(payload.measurement, projection.measurement)) findings.push(`${prefix}:MEASUREMENT_MISMATCH`)
    const members = document.listEntities({ ownerId: block.id })
    ...
    if (members.length !== projection.lines.length + projection.arcs.length + projection.arrows.length + 1
      || lines.length !== projection.lines.length || arcs.length !== projection.arcs.length
      || solids.length !== projection.arrows.length || texts.length !== 1) {
      findings.push(`${prefix}:PICTURE_MEMBER_TYPES`)
      continue
    }
    if (!lines.every((member, part) => samePoint(member.payload.start, projection.lines[part]![0])
      && samePoint(member.payload.end, projection.lines[part]![1]))) findings.push(`${prefix}:PICTURE_LINES`)
    if (!arcs.every((member, part) => {
      const expected = projection.arcs[part]!
      return samePoint(member.payload.center, expected.center) && near(member.payload.radius, expected.radius)
        && near(member.payload.startAngle, expected.startAngle) && near(member.payload.endAngle, expected.endAngle)
    ...
    if (!solids.every((member, part) => {
      ...
      return Array.isArray(vertices) && vertices.length === 4 && arrow.length === 3
        && vertices.every((vertex, vertexIndex) => samePoint(vertex, arrow[Math.min(vertexIndex, 2)]))
    ...
    if (payload.dimensionType === 'DIAMETER') {
      const measured = projectDimension({ ...payload, textOverride: null }, style)?.label.text
      if (!measured?.startsWith('⌀')) findings.push(`${prefix}:PICTURE_DIAMETER_LABEL`)
      else {
        const encoded = `%%c${measured.slice(1)}`
        expectedText = payload.textOverride == null || payload.textOverride === ''
          ? encoded : String(payload.textOverride).replaceAll('<>', encoded)
      }
    }
    const decodedText = expectedText.replace(/%%([cdp])/giu, (_, code: string) => ({ c: 'Ø', d: '°', p: '±' })[code.toLowerCase() as 'c' | 'd' | 'p'])
    if (text.text !== decodedText || !samePoint(text.position, projection.label.position)
      || !near(text.height, projection.label.height) || !near(text.rotation ?? 0, projection.label.rotation)
      || text.horizontalAlignment !== 1 || text.verticalAlignment !== 1) findings.push(`${prefix}:PICTURE_TEXT`)
  }
  if (blocks.length !== usedBlocks.size) findings.push('ORPHAN_PICTURE_BLOCK')
  return { passed: findings.length === 0, dimensions: dimensions.length, pictureBlocks: blocks.length, findings }
}