...
export function captureAgentBlockDependencies(document: KJDocument, ids: readonly string[]): readonly KJAgentBlockPreviewDependency[] | undefined {
  ...
  const visit = (entity: KJReadonlyObjectRecord, path: ReadonlySet<string>, depth: number, inheritedMatrix?: AffineMatrix3): void => {
    ...
    // Native attached attributes live in the containing space, outside the block
    // definition graph captured here. Until their geometry and resources have a
    // complete AI review path, reject the sequence before running a draft command.
