...
function fail(message: string): never { throw new KJValidationError(`Road drawing recipe: ${message}`) }

function snapshotRecipe(input: unknown): KJRoadDrawingRecipe {
  ...
  const visit = (value: unknown, depth: number): unknown => {
    ...
    if (typeof value === 'string') { if ((characters += value.length) > 1048576) fail('recipe exceeds the 1 MiB budget'); return value }
    ...
    for (const key of keys) {
      ...
      if (typeof key !== 'string' || ['__proto__', 'prototype', 'constructor'].includes(key)) fail('unsafe JSON property')
  ...
  const fields = ['schema', 'schemaVersion', 'compilerVersion', 'documentId', 'input', 'options']
  ...
  const recipe = copied as KJRoadDrawingRecipe
 ...
 */
export async function restoreRoadDrawingRecipe(document: KJDocument, input: unknown): Promise<ReadonlyDeep<KJRestoredRoadDrawingRecipe>> {
  ...
  const expectedIds = new Set([...drawing.entities.map(entity => entity.options.id), ...drawing.resources.layers.map(layer => layer.id), ...drawing.resources.linetypes.map(type => type.id)])
  ...
  await applyRoadDrawingRevision(branch, drawing, drawing, { expectedRevision: revision })
  if (document.snapshot() !== source || document.revision !== revision) throw new KJRevisionConflictError(revision, document.revision, { documentId: recipe.documentId })
  return deepFreeze({ recipe, drawing, documentId: recipe.documentId, revision })
}
 ...
 */
export async function createRoadDrawingRecipe(document: KJDocument, input: ReadonlyDeep<KJRoadDesignInput>, options: ReadonlyDeep<KJRoadDrawingOptions>): Promise<ReadonlyDeep<KJRoadDrawingRecipe>> {
  const restored = await restoreRoadDrawingRecipe(document, { schema: KJDRAW_ROAD_RECIPE_SCHEMA, schemaVersion: 1, compilerVersion: 1, documentId: document.id, input, options })
