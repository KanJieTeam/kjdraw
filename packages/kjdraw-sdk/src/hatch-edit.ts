...
function polygonLoop(loop: Loop): Point[] | null {
  ...
  try {
    for(const [index,value] of loop.edges.entries()){
      const edge=value as {type?:unknown;start?:unknown;end?:unknown}
      if(String(edge.type).toUpperCase()!=='LINE')return null
      const start=point(edge.start,index),end=point(edge.end,index)
      if(index&&!same(start,point((loop.edges[index-1] as {end?:unknown}).end,index-1)))return null
      result.push(start)
...
function validateIslandPlacement(loops: readonly Loop[],candidate: Loop,ignoredIndex=-1): void {
  ...
  const outerPolygons=loops.filter(loop=>loop.external!==false).map(polygonLoop).filter((value):value is Point[]=>Boolean(value))
...
export function editHatch(document: KJDocument,transaction: KJTransaction,id: unknown,input: KJHatchEditInput): KJObjectRecord {
  ...
  if(input.operation==='add-island'){
    ...
    const candidate=normalizedIsland(input.vertices);validateIslandPlacement(loops,candidate);loops.push(candidate)
  }else if(input.operation==='replace-island'){
    const index=islandIndex(loops,input.loopIndex),candidate=normalizedIsland(input.vertices);validateIslandPlacement(loops,candidate,index);loops[index]=candidate
