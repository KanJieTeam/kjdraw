...
function compareMap(label, expected, actual, findings, severity = 'error') {
  ...
  for (const key of [...keys].sort()) {
    if ((expected?.[key] ?? 0) !== (actual?.[key] ?? 0)) findings.push({ severity, code: `${label.toUpperCase()}_MISMATCH`, path: `${label}.${key}`, expected: expected?.[key] ?? 0, actual: actual?.[key] ?? 0 })
  }
}

export function auditRoundTrip(sourceInput, resultInput, options = {}) {
  const source = sourceInput instanceof KJDocument ? sourceInput : KJDocument.open(sourceInput)
  const result = resultInput instanceof KJDocument ? resultInput : KJDocument.open(resultInput)
  ...
  for (const key of ['layoutCount', 'paperSpaceCount', 'opaquePayloadCount', 'ownerEdgeCount']) {
    if (expected[key] !== actual[key]) findings.push({ severity: key === 'opaquePayloadCount' ? 'error' : 'warning', code: `${key.toUpperCase()}_MISMATCH`, path: key, expected: expected[key], actual: actual[key] })
  }
  ...
  if (strictHandles) {
    ...
    for (const object of sourceObjects) {
      ...
      if (!candidate) { findings.push({ severity: 'error', code: 'HANDLE_LOST', path: `objects.${object.handle}`, expected: object.type, actual: null }); continue }
      if (stableHash(comparableObject(object, { strictHandles })) !== stableHash(comparableObject(candidate, { strictHandles }))) findings.push({ severity: 'error', code: 'OBJECT_CHANGED', path: `objects.${object.handle}`, expected: object.type, actual: candidate.type })
    }
  } else {
