// Generated from agent-road-revision.ts by scripts/build-typescript.mjs. Do not edit directly.
...
const fail = (message)=>{
    throw new KJValidationError(`Road revision proposal: ${message}`);
};
function parse(input) {
    ...
    if (!input || typeof input !== 'object' || Array.isArray(input) || ![
        Object.prototype,
        null
    ].includes(Object.getPrototypeOf(input))) fail('requires a plain input object');
    const keys = Reflect.ownKeys(input), data = {};
    ...
    for (const name of names){
        ...
        if (!d || !('value' in d) || !d.enumerable) fail('accessors and missing fields forbidden');
        data[name] = d.value;
    }
    ...
    for (const key of [
        ...
        'rightWidthDelta',
        'elevationDelta'
    ])if (typeof data[key] !== 'number' || !Number.isFinite(data[key]) || Math.abs(data[key]) > 1e9) fail('deltas must be finite within ±1e9 meters');
...
function counts(receipt) {
    return {
        ...
        removed: receipt.removedIds.length,
        unchanged: receipt.unchangedIds.length
    };
}
export async function buildAgentRoadRevision(document, input, registered) {
    ...
    const previous = restored.drawing, nextInput = structuredClone(restored.recipe.input);
    nextInput.pavement.leftWidth += data.leftWidthDelta;
    nextInput.pavement.rightWidth += data.rightWidthDelta;
    nextInput.profile = nextInput.profile.map((point)=>({
            ...point,
            elevation: point.elevation + data.elevationDelta
        }));
    const recipe = {
        ...structuredClone(restored.recipe),
        input: nextInput
    };
    ...
    if (new TextEncoder().encode(JSON.stringify({
        previous,
        next
    })).length > 4194304) fail('revision working set exceeds 4 MiB');
    const draft = document.fork(), receipt = await applyRoadDrawingRevision(draft, previous, next, {
    ...
    const before = [
        ...receipt.updatedIds,
        ...receipt.removedIds
    ].map((id)=>project(document, id)), after = [
        ...receipt.updatedIds,
        ...receipt.createdIds
    ].map((id)=>project(draft, id));
    ...
    const evidence = {
        ...
        previousDesignParameters: {
            input: structuredClone(restored.recipe.input),
            options: recipe.options
        },
        changes: data,
        changedCounts: counts(receipt),
        previousTotalVolume: previous.calculation.totalVolume,
        totalVolume: next.calculation.totalVolume,
