// Generated from agent-capability-candidates.ts by scripts/build-typescript.mjs. Do not edit directly.
...
const exactRecord = (value, allowed, label)=>{
...
const boundedText = (value, label, maximum = 256)=>{
...
function safeJsonSnapshot(input) {
    let nodes = 0;
...
function ids(value, label, minimum) {
...
function source(value, label) {
    ...
    if (item.toolName !== 'cad_query_topology' || ![
        'seed',
        'related'
    ].includes(String(item.scope)) || !paths.has(String(item.path))) return fail(`${label} is unsupported`);
    return {
...
function validateRules(value) {
    ...
    const result = value.map((raw)=>{
        const item = exactRecord(raw, [
            ...
            'capabilityId',
            'capabilityVersion'
        ], 'Resolved candidate rule');
        const seed = exactRecord(item.seed, [
            'entityTypes'
        ], 'Resolved candidate seed');
        ...
        const predicates = item.predicates.map((rawPredicate)=>{
            const predicate = exactRecord(rawPredicate, [
                'fact',
                'source',
                'operator',
                'compareTo',
            ...
            const fact = predicate.fact, operator = predicate.operator;
            ...
            if ([
                ...
                'same_as',
                'within'
            ].includes(operator) !== (compareTo !== undefined)) return fail('Resolved candidate predicate has invalid value or comparison fields');
            if ([
                'at_least',
                'at_most'
            ].includes(operator) && typeof predicate.value !== 'number') return fail('Resolved candidate numeric predicate requires a number');
            return {
                fact,
                ...
                ...predicate.relation === undefined ? {} : {
                    relation: identifier(predicate.relation, 'Resolved candidate relation')
                },
                ...hasValue ? {
        ...
        if (![
            'always',
            'when-ambiguous'
        ].includes(String(item.confirmation))) return fail('Resolved candidate confirmation is unsupported');
...
function topologyView(value, expectedRevision, expectedTolerance, units, exactIds) {
...
function recordAt(value) {
    return value && typeof value === 'object' && !Array.isArray(value) ? value : null;
}
function boundsAt(value) {
...
function nativeReferences(entity) {
    return recordAt(entity.raw.nativeReferences);
}
function resolveEvidence(entity, path) {
    if (path === 'entities[].ownerId') return {
        kind: 'scalar',
        value: entity.ownerId
    };
    if (path === 'entities[].layer.id') return {
        kind: 'scalar',
    ...
    if (path === 'entities[].nativeReferences.displayExtent.bounds') {
        ...
        return bounds ? {
            kind: 'bounds',
            value: bounds
        } : fail('Topology contains malformed complete display bounds');
    }
    ...
    if (path === 'entities[].nativeReferences.insert.blockRecordId') return typeof insert.blockRecordId === 'string' && insert.blockRecordId ? {
        kind: 'scalar',
        value: insert.blockRecordId
    } : null;
    if (path === 'entities[].nativeReferences.insert.typeCountSignature') return typeof insert.typeCountSignature === 'string' && insert.typeCountSignature ? {
        kind: 'scalar',
        value: insert.typeCountSignature
    } : null;
...
function summary(value) {
    ...
    for (const reference of value.value){
        const status = typeof reference.status === 'string' ? reference.status : 'invalid';
        statuses[status] = (statuses[status] ?? 0) + 1;
    }
    return {
        status: 'available',
        count: value.value.length,
        statuses: Object.fromEntries(Object.entries(statuses).sort(([a], [b])=>stableCompare(a, b)))
    };
}
function evaluatePredicate(predicate, seed, related, tolerance) {
    const entityFor = (scope)=>scope === 'seed' ? seed : related;
    ...
    else if (predicate.operator === 'equals') passed = !!left && left.kind === 'scalar' && Object.hasOwn(predicate, 'value') && equalScalar(left.value, predicate.value);
    ...
    return {
        ...
        observed: {
            source: summary(left),
            ...predicate.compareTo ? {
                compareTo: summary(right)
            } : {}
        }
    };
}
export function evaluateAgentCapabilityCandidates(input) {
    const detached = exactRecord(safeJsonSnapshot(input), [
        'candidateRules',
        'topology',
        'expectedRevision',
        'expectedTolerance',
    ...
    if (!Number.isSafeInteger(detached.expectedRevision) || detached.expectedRevision < 0) return fail('Candidate expectedRevision must be a nonnegative safe integer');
    if (!finite(detached.expectedTolerance) || detached.expectedTolerance <= 0 || detached.expectedTolerance > 1) return fail('Candidate expectedTolerance must be greater than zero and at most 1 drawing unit');
    const expectedRevision = detached.expectedRevision, expectedTolerance = detached.expectedTolerance, units = boundedText(detached.units, 'Candidate units', 64);
    if (!Number.isSafeInteger(detached.maxBytes) || detached.maxBytes < 1024 || detached.maxBytes > 262144) return fail('Candidate maxBytes must be an integer from 1024 through 262144');
    const maxBytes = detached.maxBytes, seedIds = ids(detached.seedIds, 'Candidate seedIds', 1), relatedIds = ids(detached.relatedIds, 'Candidate relatedIds', 0);
    ...
    const rules = validateRules(detached.candidateRules), exactIds = [
        ...seedIds,
        ...relatedIds
    ].sort(), topology = topologyView(detached.topology, expectedRevision, expectedTolerance, units, exactIds);
    let evaluations = 0;
    const bump = ()=>{
        if (++evaluations > MAX_EVALUATIONS) fail(`Candidate predicate evaluation budget exceeded (${MAX_EVALUATIONS})`);
    };
    ...
    for (const rule of rules)for (const seedId of seedIds){
        ...
        const unary = rule.predicates.map((predicate, predicateIndex)=>({
                predicate,
                predicateIndex
            })).filter(({ predicate })=>predicate.source.scope === 'seed' && !predicate.compareTo);
        const paired = rule.predicates.map((predicate, predicateIndex)=>({
                predicate,
                predicateIndex
            })).filter(({ predicate })=>!(predicate.source.scope === 'seed' && !predicate.compareTo));
        ...
        for (const { predicate, predicateIndex } of unary){
            bump();
            const result = evaluatePredicate(predicate, seed, undefined, topology.tolerance);
            unaryPassed = unaryPassed && result.passed;
            predicateEvidence.push({
                predicateIndex,
                fact: predicate.fact,
                operator: predicate.operator,
                source: predicate.source,
                ...predicate.compareTo ? {
                    compareTo: predicate.compareTo
                } : {},
                ...predicate.relation ? {
        ...
        for (const { predicate, predicateIndex } of paired){
            ...
            for (const relatedId of relatedIds){
                bump();
                const result = evaluatePredicate(predicate, seed, topology.entities.get(relatedId), topology.tolerance);
            ...
            predicateEvidence.push({
                ...
                matchingRelatedIds: matching,
                nonMatchingIds: nonMatching
            });
        }
        ...
        if (passed) {
            const matchedIds = [
                seedId,
                ...paired.length ? matchingRelatedIds : []
            ];
            if (matchedIds.some((id)=>[
                    'HATCH',
                    'INSERT'
                ].includes(topology.entities.get(id).type)) && classifiesPatternAsNoiseOrBoundary(rule.candidateKind)) return fail('Capability rules cannot classify HATCH or INSERT candidates as noise or boundary');
            candidates.push({
                capabilityId: rule.capabilityId,
                capabilityVersion: rule.capabilityVersion,
                ruleId: rule.id,
                ...
                relatedIds: paired.length ? matchingRelatedIds : [],
                evidenceCodes: [
                    ...rule.evidenceCodes
                ],
                nonMatchingIds: paired.length ? relatedIds.filter((id)=>!matchingRelatedIds.includes(id)) : [
                    ...relatedIds
                ],
    ...
    const candidateCountByRule = new Map();
    for (const candidate of candidates){
        const key = `${candidate.capabilityId}\0${candidate.capabilityVersion}\0${candidate.ruleId}`;
        candidateCountByRule.set(key, (candidateCountByRule.get(key) ?? 0) + 1);
    }
    for (const candidate of candidates){
        ...
        const containsSemanticPattern = entityIds.some((id)=>[
                'HATCH',
                'INSERT'
            ].includes(topology.entities.get(id).type));
        const key = `${candidate.capabilityId}\0${candidate.capabilityVersion}\0${candidate.ruleId}`;
    ...
    const included = new Set(candidates.flatMap((candidate)=>[
            ...candidate.seedIds,
            ...candidate.relatedIds
        ]));
    const result = {
        ...
        candidates,
        evidence,
        nonMatchingIds: exactIds.filter((id)=>!included.has(id)),
        confirmationRequired: candidates.some((candidate)=>candidate.confirmationRequired),
        limits: {
            maxRules: MAX_RULES,
            maxIds: MAX_IDS,
