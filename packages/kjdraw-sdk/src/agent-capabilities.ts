...
export function validateAgentCapabilityManifest(input: unknown): ReadonlyDeep<KJAgentCapabilityManifest> {
  const source = record(jsonSnapshot(input), ['schema', 'schemaVersion', 'id', 'name', 'version', 'toolApiVersion', 'instructions', 'requiredToolNames', 'requirements'], 'Capability manifest')
  ...
  return deepFreeze({ schema: KJDRAW_AGENT_CAPABILITY_SCHEMA, schemaVersion: KJDRAW_AGENT_CAPABILITY_SCHEMA_VERSION,
...
export class KJAgentCapabilityRegistry {
  ...
  constructor({ toolApiVersion = KJDRAW_AGENT_CAPABILITY_TOOL_API_VERSION }: { toolApiVersion?: number } = {}) {
