...
export const KJDRAW_AGENT_TOOLS: readonly KJAgentToolDefinition[] = deepFreeze([
  ...
  { name: 'cad_propose_drawing_compact', effect: 'propose', description: 'Propose 1–64 native entities in model XY, z=0, drawing units. Common geometry uses compact tuples; optional open splines retain structured control points, knots and weights. Returns geometry without editing; host approval applies one undoable edit.', inputSchema: compactDrawingSchema },
  { name: 'cad_propose_drawing_pattern', effect: 'propose', description: 'Propose 1–64 base entities and up to 16 rectangular arrays, at most 512 total native entities. Uses compact common geometry plus optional open NURBS; array sources are group-local zero-based references such as circles:0, ellipses:0 or splines:0. Full preview, no edit before host approval, one undoable edit.', inputSchema: objectWithOptional({ ...compactDrawingProperties, arrays: { type: 'array', minItems: 0, maxItems: 16, items: object({ sources: collection({ type: 'string', minLength: 6, maxLength: 12 }), rows: patternCount, columns: patternCount, dx: number, dy: number }) } }, ['ellipses', 'splines']) },
...
function styleAnnotatedDrawing(document: KJDocument, input: KJAgentAnnotatedDrawingInput, source: readonly { type: string; payload: Readonly<Record<string, unknown>>; options: { readonly id: string; readonly ownerId: string } }[]): Record<string, unknown> {
  ...
  const keys: string[] = []
  for (const group of ['lines', 'circles', 'arcs', 'ellipses', 'splines', 'polylines'] as const) for (let index = 0; index < (input[group]?.length ?? 0); index++) keys.push(`${group}:${index}`)
        ...
        if (name === 'cad_read_page') value = createDrawingContext(document, { expectedRevision: args.expectedRevision as number, offset: args.offset as number, layerOffset: args.layerOffset as number })
          ...
          if (name === 'cad_propose_road_revision') {
            ...
            if (name === 'cad_propose_component_insert') {
              ...
              let offset = 0
              for (const group of ['lines', 'circles', 'arcs', 'ellipses', 'splines', 'polylines'] as const) {
              ...
              if (['cad_propose_move', 'cad_propose_rotate', 'cad_propose_scale'].includes(name) && byIds === bySelectionSet) throw new KJValidationError('Transform requires exactly one of ids or selectionSetName')
              selectionSet = bySelectionSet ? selectionSetRecord(document, args.selectionSetName) : undefined
              const ids = selectionSet?.memberIds ?? args.ids as string[]
              if (new Set(ids).size !== ids.length) throw new KJValidationError('Object IDs must be unique')
              const context = createDrawingContext(document, { ids, limit: 64, maxBytes: 262144 })
