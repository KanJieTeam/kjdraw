...
const drawingInputSchema = objectWithOptional({ expectedRevision: revision, units: text, lines: drawingGroup(object({ start: point, end: point })), circles: drawingGroup(object({ center: point, radius })), arcs: drawingGroup(object({ center: point, radius, startDegrees: angle, endDegrees: angle })), ellipses: drawingGroup(object({ center: point, majorAxis: point, ratio: { ...number, exclusiveMinimum: 0, maximum: 1 }, startDegrees: angle, endDegrees: angle })), polylines: drawingGroup(object({ vertices: { ...collection(point), minItems: 2 }, closed: { type: 'boolean' } })) }, ['ellipses'])
const numericTuple = (length: number): KJAgentToolSchema => ({ type: 'array', items: number, minItems: length, maxItems: length })
const compactDrawingProperties = { expectedRevision: revision, units: text, lines: drawingGroup(numericTuple(4)), circles: drawingGroup(numericTuple(3)), arcs: drawingGroup(numericTuple(5)), ellipses: drawingGroup(numericTuple(7)), polylines: drawingGroup(object({ points: { ...collection(numericTuple(2)), minItems: 2 }, closed: { type: 'boolean' } })) }
const compactDrawingSchema = objectWithOptional(compactDrawingProperties, ['ellipses'])
...
export const KJDRAW_AGENT_TOOLS: readonly KJAgentToolDefinition[] = deepFreeze([
  ...
  { name: 'cad_propose_drawing_pattern', effect: 'propose', description: 'Propose 1–64 base entities and up to 16 rectangular arrays, at most 512 total native entities. Uses compact drawing groups including optional ellipses; array sources are group-local zero-based references such as circles:0 or ellipses:0. Full preview, no edit before host approval, one undoable edit.', inputSchema: objectWithOptional({ ...compactDrawingProperties, arrays: { type: 'array', minItems: 0, maxItems: 16, items: object({ sources: collection({ type: 'string', minLength: 6, maxLength: 12 }), rows: patternCount, columns: patternCount, dx: number, dy: number }) } }, ['ellipses']) },
...
function styleAnnotatedDrawing(document: KJDocument, input: KJAgentAnnotatedDrawingInput, source: readonly { type: string; payload: Readonly<Record<string, unknown>>; options: { readonly id: string; readonly ownerId: string } }[]): Record<string, unknown> {
  ...
  const keys: string[] = []
  for (const group of ['lines', 'circles', 'arcs', 'ellipses', 'polylines'] as const) for (let index = 0; index < (input[group]?.length ?? 0); index++) keys.push(`${group}:${index}`)
        ...
        if (name === 'cad_read_page') value = createDrawingContext(document, { expectedRevision: args.expectedRevision as number, offset: args.offset as number, layerOffset: args.layerOffset as number })
          ...
          if (name === 'cad_propose_road_revision') {
            ...
            if (name === 'cad_propose_component_insert') {
              ...
              let offset = 0
              for (const group of ['lines', 'circles', 'arcs', 'ellipses', 'polylines'] as const) {
