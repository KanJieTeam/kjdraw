...
export const KJDRAW_AGENT_TOOLS: readonly KJAgentToolDefinition[] = deepFreeze([
  ...
  { name: 'cad_propose_move', effect: 'propose', description: 'Propose an XY displacement of 1–64 visible editable model-space LINE/CIRCLE/ARC/LWPOLYLINE/XLINE/RAY objects identified by exact IDs. Construction-line and ray directions are preserved. Returns before/after geometry; the host must approve before edits apply.', inputSchema: object({ expectedRevision: revision, units: text, ids: collection(text), dx: number, dy: number }) },
...
function styleAnnotatedDrawing(document: KJDocument, input: KJAgentAnnotatedDrawingInput, source: readonly { type: string; payload: Readonly<Record<string, unknown>>; options: { readonly id: string; readonly ownerId: string } }[]): Record<string, unknown> {
  ...
  for (const group of ['texts', 'alignedDimensions', 'rotatedDimensions', 'radiusDimensions', 'diameterDimensions'] as const) for (let index = 0; index < input[group].length; index++) keys.push(`${group}:${index}`)
        ...
        if (name === 'cad_read_page') value = createDrawingContext(document, { expectedRevision: args.expectedRevision as number, offset: args.offset as number, layerOffset: args.layerOffset as number })
          ...
          if (name === 'cad_propose_road_revision') {
            ...
            if (name === 'cad_propose_road_drawing') {
              ...
              if (context.entities.length !== ids.length || context.entities.some(entity => !entity.editable || !['LINE', 'CIRCLE', 'ARC', 'LWPOLYLINE', 'XLINE', 'RAY'].includes(entity.type))) throw new KJValidationError('Move requires visible editable model-space LINE/CIRCLE/ARC/LWPOLYLINE/XLINE/RAY objects')
              command = 'MOVE'
