...
export class KJAgentToolSession {
  ...
  bindTaskProposal(planId: string, input: KJAgentTaskProposalBinding): void {
    ...
    if (!pending || !['CREATEBATCH', 'MOVE', 'ROTATE', 'SCALE', 'LENGTHEN', 'STRETCH', 'PEDIT'].includes(pending.envelope.command)) throw new KJValidationError('Persistent task approval supports an available CREATEBATCH, MOVE, ROTATE, SCALE, LENGTHEN, STRETCH or PEDIT proposal only')
  ...
  async approveTask(planId: string, reviewerId: string, at: string): Promise<KJAgentToolResult> {
    ...
    try {
      ...
      if (!['CREATEBATCH', 'MOVE', 'ROTATE', 'SCALE', 'LENGTHEN', 'STRETCH', 'PEDIT'].includes(command) || pending.definition.id !== command || pending.definition.owner !== '@kanjieteam/kjdraw' || pending.definition.transactional === false) throw new KJValidationError('Persistent task approval is limited to a supported built-in transactional command')
