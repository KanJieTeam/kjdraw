...
export class KJAgentToolSession {
  ...
  get definitions(): readonly KJAgentToolDefinition[] {
    ...
    return deepFreeze(KJDRAW_AGENT_TOOLS.filter(tool => !['cad_propose_manufacturing_sheet', 'cad_propose_architecture_plan', 'cad_propose_cartesian_chart', 'cad_propose_geology_column', 'cad_propose_geology_section'].includes(tool.name) || units === 'millimeter').filter(tool => tool.name !== 'cad_propose_site_plan' || units === 'meter').map(tool => {
