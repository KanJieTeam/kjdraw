// Generated from knowledge-pack.ts by scripts/build-typescript.mjs. Do not edit directly.
import { KJValidationError } from './errors.js';
import { deepFreeze, stableHash } from './utils.js';
export const KJDRAW_KNOWLEDGE_PACK_SCHEMA = 'kjdraw.knowledge-pack.v1';
export const KJDRAW_SEMANTIC_IR_SCHEMA = 'kjdraw.semantic-ir.v1';
const fail = (message)=>{
    throw new KJValidationError(`Knowledge pack: ${message}`);
};
...
export function validateKnowledgePack(source) {
    ...
    const sources = sourcesRaw.map((raw, index)=>{
        ...
        return {
            id: sourceId,
            title: boundedText(row.title, `pack.sources[${index}].title`),
            license: boundedText(row.license, `pack.sources[${index}].license`, 64),
            contentHash,
            ...row.uri == null ? {} : {
                uri: boundedText(row.uri, `pack.sources[${index}].uri`, 512)
            }
        };
    });
    ...
    const result = {
        ...
        id,
        version,
        title: boundedText(pack.title, 'pack.title'),
        domain: boundedText(pack.domain, 'pack.domain', 64),
        license: {
            spdx: boundedText(license.spdx, 'pack.license.spdx', 64),
            redistributable: license.redistributable,
            trainingAllowed: license.trainingAllowed
        },
        sources,
        ontology: {
            objectKinds: stringList(ontology.objectKinds, 'pack.ontology.objectKinds', 512),
            relationKinds: stringList(ontology.relationKinds, 'pack.ontology.relationKinds', 512)
        },
        ...pack.templates == null ? {} : {
            templates: plain(pack.templates, 'pack.templates')
        },
        ...pack.rules == null ? {} : {
            rules: plain(pack.rules, 'pack.rules')
        }
    };
    scan(result, 'pack');
    return deepFreeze(result);
}
export function validateSemanticDrawingIntent(source, pack) {
    ...
    const relations = relationsRaw.map((raw, index)=>{
        ...
        return {
            ...
            to,
            ...row.properties == null ? {} : {
                properties: plain(row.properties, `intent.relations[${index}].properties`)
            }
        };
    });
    const result = {
        ...
        packVersion,
        drawing: {
            kind: boundedText(drawing.kind, 'intent.drawing.kind', 96),
            title: boundedText(drawing.title, 'intent.drawing.title', 256),
            units: drawing.units
        },
        objects,
        relations
...
export class KJKnowledgePackRegistry {
    ...
    get(id, version) {
        const key = version ? `${id}@${version}` : [
            ...this.#packs.keys()
        ].filter((item)=>item.startsWith(`${id}@`)).sort().at(-1);
