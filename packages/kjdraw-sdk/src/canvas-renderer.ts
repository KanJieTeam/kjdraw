...
import { resolvePhysicalPlotPaper, type KJPhysicalPlotPaper } from './plot-settings.js'
...
type PaperSheet = KJPhysicalPlotPaper & { layoutId: string }
...
function toleranceRows(value: unknown): string[][] {
  const glyphs: Readonly<Record<string, string>> = { j: '⌖', r: '◎', i: '≡', f: '∥', b: '⊥', a: '∠', g: '⌭', c: '▱', e: '○', u: '—', d: '⌒', k: '⌢', h: '↗', t: '⇗', n: '⌀', m: 'Ⓜ', l: 'Ⓛ', s: 'Ⓢ', p: 'Ⓟ' }
  return String(value ?? '').split('^J').filter(Boolean).map(row => {
    const cells = row.split('%%v').map(cell => cell.replace(/\{\\Fgdt;([a-z])\}/gu, (_, code: string) => glyphs[code] ?? code))
...
export class KJCanvasRenderer {
  ...
  #paperSheetState: PaperSheet | null = null
  ...
  setDocument(document: KJDocument | null): this {
    ...
    if (document) this.#disposeDocument = document.on('document:change', event => {
      ...
      const pageChanged = operations.some(operation => {
        const value = operation as { before?: { kind?: unknown; payload?: Readonly<Record<string, unknown>> }; after?: { kind?: unknown; payload?: Readonly<Record<string, unknown>> } }
  ...
  fit(): this {
    ...
    if (paper) values.push([0, 0], [paper.width, paper.height])
  ...
  render(): Readonly<KJCanvasRenderReport> {
    ...
    const layers = new Map(document.getTable('layers')?.records.map(layer => [layer.id, layer.payload]) ?? [])
    const drawingTheme: KJCanvasTheme = paper ? 'light' : this.#theme
  ...
  #paperSheet(): PaperSheet | null {
    ...
    const layout = document.spaces.layoutIds
      .map(id => document.getObject(id))
      .find(item => item?.kind === 'layout' && !item.erased && String(item.payload.blockRecordId ?? '') === spaceId)
    ...
    try {
      ...
      return { ...paper, layoutId: layout.id }
    } catch { return null }
  }

  #drawPaperSheet(paper: PaperSheet): void {
    const context = this.context
    const lowerLeft = this.worldToScreen([0, 0]), upperRight = this.worldToScreen([paper.width, paper.height])
    ...
    const printableMinimum = this.worldToScreen([paper.left, paper.bottom])
    const printableMaximum = this.worldToScreen([paper.width - paper.right, paper.height - paper.top])
  ...
  #color(entity: KJReadonlyObjectRecord, layer?: Readonly<Record<string, unknown>>): string {
    const theme: KJCanvasTheme = this.#paperSheetState ? 'light' : this.#theme
  ...
  #drawEntity(entity: KJReadonlyObjectRecord, color: string, depth: number, overrideColor = false, projection?: HatchProjectionIdentity, inherited?: DrawingPropertyInheritance): boolean {
    ...
    if (entity.type === 'LINE') drawn = this.#strokePath(points([payload.start, payload.end]))
      ...
      let unsupportedBoundary = loops.some(loop => Array.isArray(loop.edges) && loop.edges.some((edge: Record<string, unknown>) => !['LINE', 'ARC', 'ELLIPSE', 'SPLINE'].includes(String(edge.type).toUpperCase())))
      const paths = loops.map(loop => {
        const value = loop as Record<string, unknown>
        if (Array.isArray(value.vertices)) {
          const path=polylineSamples({ vertices:value.vertices, closed:true })
        ...
        for (const edge of Array.isArray(value.edges) ? value.edges : []) {
          ...
          if(edgeType==='SPLINE'){
            try{result.push(...sampleHatchSpline(e))}catch{unsupportedBoundary=true;result.length=0;break}
            ...
            if(!center||!axis||!(ratio>0)){unsupportedBoundary=true;result.length=0;break}
            ...
            const line=points([e.start,e.end])
            if(line.length!==2){unsupportedBoundary=true;result.length=0;break}
            result.push(...line)
          } else {unsupportedBoundary=true;result.length=0;break}
        }
      ...
      const position = point2(payload.position), axis = point2(payload.xAxisDirection) ?? [1, 0]
      if (!position || Math.hypot(...axis) <= 1e-12) drawn = false
        ...
        context.translate(screen[0], screen[1]); context.rotate(-rotation)
        let y = 0
      ...
      if (projected) {
        if (projection?.dimensionMatrix) {
          const [a, b, c, d, e, f] = projection.dimensionMatrix as readonly [number, number, number, number, number, number]
          const [x, y] = this.worldToScreen([0, 0]), k = this.camera.scale
          // Conjugate the world transform by worldToScreen. This preserves the
          // local label and maps arcs to true ellipses under general affine maps.
      ...
      if (drawn && entity.type === 'LEADER' && payload.arrowEnabled !== false && values.length >= 2) {
        ...
        if (length > 1e-6) {
          ...
          context.beginPath(); context.moveTo(tip[0], tip[1]); context.lineTo(rearX - uy * half, rearY + ux * half); context.lineTo(rearX + uy * half, rearY - ux * half); context.closePath(); context.fill()
        }
      }
      ...
      const position = point2(source.position), base = point2(block?.payload.basePoint) ?? [0, 0]
      if (!block || !position) drawn = false
      else {
        const inputScale = Array.isArray(source.scale) ? source.scale : [source.scale ?? 1, source.scale ?? 1]
        const factorX = finite(inputScale[0], 1), factorY = finite(inputScale[1], factorX)
        const localMatrix = multiply3(
  ...
  /** Measure in the definition owner, then project graphics through the full block
   * and viewport matrix. General affine maps also transform glyphs and arc shapes. */
  #dimensionInView(entity: KJReadonlyObjectRecord, matrix: readonly number[]): Pick<HatchProjectionIdentity, 'dimension' | 'dimensionMatrix'> {
    const original = projectDimension(entity.payload, this.#document?.getObject(String(entity.payload.styleId ?? ''))?.payload)
    if (!original) return { dimension: null }
    const determinant = matrix[0]! * matrix[3]! - matrix[1]! * matrix[2]!
    const sx = Math.hypot(matrix[0]!, matrix[1]!), sy = Math.hypot(matrix[2]!, matrix[3]!)
    if (!matrix.every(Number.isFinite) || sx === 0 || sy === 0 || Math.abs(determinant) <= 1e-12 * sx * sy) return { dimension: null }
    ...
    if (!similarity) return { dimension: original, dimensionMatrix: matrix }
    ...
    return { dimension: { ...original, arcs, lines: original.lines.map(([a, b]) => [point(a), point(b)]), arrows: original.arrows.map(arrow => arrow.map(point)), label: { ...original.label, position: point(original.label.position), height: original.label.height * Math.hypot(matrix[0]!, matrix[1]!), rotation: original.label.rotation + Math.atan2(matrix[1]!, matrix[0]!) } } }
  }
  ...
  #drawViewport(entity: KJReadonlyObjectRecord, depth: number): boolean {
    ...
    // The current native contract is orthographic XY with a rectangular or verified
    // closed 2D polyline clip. Reject other advanced view controls rather than drawing
    // a false top view.
