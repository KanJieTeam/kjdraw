// Generated from road-drawing.ts by scripts/build-typescript.mjs. Do not edit directly.
import { computeRoadDesign } from './road-design.js';
import { KJValidationError } from './errors.js';
import { deepFreeze } from './utils.js';
const fail = (message)=>{
    throw new KJValidationError(`Road drawing: ${message}`);
};
function object(value, required, optional = []) {
    if (!value || typeof value !== 'object' || Array.isArray(value) || ![
        Object.prototype,
        null
    ].includes(Object.getPrototypeOf(value))) fail('options must be plain data');
    for (const key of Reflect.ownKeys(value)){
        const d = Object.getOwnPropertyDescriptor(value, key);
        if (typeof key !== 'string' || ![
            ...required,
            ...optional
        ].includes(key) || !('value' in d) || !d.enumerable) fail('unsupported fields or accessors');
    }
    if (required.some((key)=>!Object.hasOwn(value, key))) fail('required options missing');
}
...
export function buildRoadDrawing(input, options) {
    object(options, [
        'drawingId',
        'title',
        'profileScale',
    ...
    for (const scale of [
        ...
        positive(scale.horizontal, 'horizontal scale');
        positive(scale.vertical, 'vertical scale');
    }
    ...
    const formats = (value)=>(Math.abs(value) < .5 * 10 ** -precision ? 0 : value).toFixed(precision);
    const sid = (station)=>String(Object.is(station, -0) ? 0 : station);
    const resources = {
        linetypes: [
            {
                id: `${prefix}:linetype:continuous`,
                name: `${options.drawingId}_ROAD_CONTINUOUS`,
                pattern: []
            },
            {
                id: `${prefix}:linetype:ground`,
                name: `${options.drawingId}_ROAD_GROUND`,
                pattern: [
                    3,
                    -1
                ]
        ...
        layers: [
            ...
            'TABLE'
                ...
                id: `${prefix}:layer:${name}`,
                name: `${options.drawingId}_ROAD_${name}`,
                color: [
                    3,
                    8,
                    4,
                ...
                linetypeId: `${prefix}:linetype:${name === 'GROUND' ? 'ground' : 'continuous'}`,
                lineweight: name === 'DESIGN' ? 35 : name === 'FRAME' ? 25 : 18
            }))
    };
    const add = (key, type, payload, layer = 'ANNOTATION')=>{
    ...
    const checkXY = (point)=>{
        if (!point.slice(0, 2).every((n)=>Number.isFinite(n) && Math.abs(n) <= 1e12)) fail('projected output exceeds finite coordinate budget');
    };
    const poly = (key, points, layer = 'DESIGN', closed = false)=>{
        ...
        add(key, 'LWPOLYLINE', ()=>({
                vertices: points.map((point)=>xyz(point[0], point[1])),
                closed
            }), layer);
    };
    const line = (key, a, b, layer = 'AXIS')=>{
        checkXY(a);
        checkXY(b);
    ...
    const text = (key, value, x, y, height = t)=>{
        checkXY([
            x,
            y
        ]);
    ...
    const wrap = (value, width)=>{
        const limit = Math.max(12, Math.floor(width / (1.2 * t))), lines = [];
        let current = '';
    ...
    const edgeSegments = calculation.alignment.flatMap((segment, i)=>{
        ...
        return [
            ...
            [
                ...
                key: `plan/span/${i}/${side}-pavement-edge`,
                points: [
                    segment.start,
                    segment.end
                ].map((p)=>[
                        p[0] - segment.tangent[1] * Number(offset),
                        p[1] + segment.tangent[0] * Number(offset)
                    ])
            }));
    });
    const worldPoints = [
        ...input.alignment,
        ...edgeSegments.flatMap((edge)=>edge.points),
        ...calculation.sections.flatMap((section)=>section.worldDesign.map((p)=>[
                    p[0],
                    p[1]
                ]))
    ];
    ...
    const planBounds = [
        minX - pad,
        minY - 12 * t,
        Math.max(maxX + 24 * t, minX - pad + Math.max(titleWidth, 110 * t)),
        maxY + 12 * t
    ];
    ...
    const profileRows = calculation.sections.map((s)=>({
            key: sid(s.station),
            cells: [
                formats(s.station),
                formats(s.designElevation),
                formats(s.groundCenterElevation),
                formats(s.longitudinalGrade * 100),
                formats(s.areas.cut),
                formats(s.areas.fill)
            ]
        }));
    const quantityRows = calculation.intervals.map((i)=>({
            key: `${sid(i.fromStation)}-${sid(i.toStation)}`,
            cells: [
                formats(i.fromStation),
                formats(i.toStation),
                formats(i.length),
                formats(i.cutVolume),
                formats(i.fillVolume)
            ]
        }));
    quantityRows.push({
        key: 'total',
        cells: [
            ...
            '',
            '',
            formats(calculation.totalVolume.cut),
            formats(calculation.totalVolume.fill)
        ]
    });
    const tableColumns = (headers, rows)=>headers.map((header, i)=>(Math.max(header.length, ...rows.map((row)=>row.cells[i].length)) * 1.2 + 2) * t);
    ...
    const elevations = [
        ...input.profile.map((p)=>p.elevation),
        ...calculation.sections.map((s)=>s.groundCenterElevation)
    ], datum = Math.min(...elevations), elevationMax = Math.max(...elevations);
    ...
    const notices = [
        `Diagram coordinates only: H=${ph} and V=${pv} drawing units per m; vertical exaggeration=${formats(pv / ph)}. Not native distance dimensions.`,
        `Tables rounded to ${precision} decimals; quantities use unrounded calculation. Average-end-area volume range ${formats(calculation.volumeRange[0])} to ${formats(calculation.volumeRange[1])} m.`,
    ...
    const profileCorner = options.origin ?? [
        planBounds[2] + gap,
        planBounds[1]
    ];
    const profileBounds = [
        profileCorner[0],
        profileCorner[1],
        profileCorner[0] + profileWidth,
        profileCorner[1] + profileHeight
    ];
    ...
    const profileOrigin = [
        profileBounds[0] + pad + 10 * t,
        profileBounds[3] - 8 * t - graphHeight
    ];
    const plotProfile = (station, elevation)=>[
            profileOrigin[0] + (station - calculation.startStation) * ph,
            profileOrigin[1] + (elevation - datum) * pv
        ];
    line('profile/datum-axis', profileOrigin, [
        profileOrigin[0] + calculation.length * ph,
        profileOrigin[1]
    ]);
    const legend = (key, x, y)=>{
        line(`${key}/design-sample`, [
            x,
            y
        ], [
            x + 4 * t,
    ...
    const table = (key, headers, rows, widths, top)=>{
        ...
        for(let c = 0; c <= widths.length; c++){
            ...
            if (c < widths.length) {
                text(`${key}/header/${c}`, headers[c], offset + t, top - 2 * t);
                offset += widths[c];
            }
        }
        line(`${key}/top`, [
        ...
        rows.forEach((row, r)=>{
            ...
            row.cells.forEach((value, c)=>{
                if (value) text(`${key}/row/${row.key}/cell/${c}`, value, left + t, top - (r + 1) * rowHeight - 2 * t);
                left += widths[c];
            });
            line(`${key}/row/${row.key}/bottom`, [
                x,
                top - (r + 2) * rowHeight
            ], [
                x + w,
    ...
    const sectionRanges = calculation.sections.map((s)=>{
        const z = [
            ...s.ground,
            ...s.design
        ].map((p)=>p[1]);
    ...
    const sectionLabelWidth = Math.max(...calculation.sections.flatMap((s, i)=>[
            `STA ${formats(s.station)} | Z ${formats(s.designElevation)} m`,
            `Cut ${formats(s.areas.cut)} m2 | Fill ${formats(s.areas.fill)} m2`,
            `H ${sh} / V ${sv}; datum Z ${formats(sectionRanges[i].min)} m`,
            `Offset m: ${formats(offsetMin)} to ${formats(offsetMax)}; positive LEFT`,
    ...
    const sectionBounds = [
        profileBounds[2] + gap,
        profileBounds[1],
        profileBounds[2] + gap + Math.max(titleWidth, usedColumns * cellWidth + (usedColumns - 1) * gap + 2 * pad),
        profileBounds[1] + rows * cellHeight + (rows - 1) * gap + 12 * t
    ];
    ...
    const projections = {
        ...
        profile: {
            horizontal: ph,
            vertical: pv,
            stationDatum: calculation.startStation,
    ...
    calculation.sections.forEach((section, i)=>{
        ...
        const origin = [
            cellX + 12 * t,
            cellTop - cellHeight + 6 * t
        ], elevationDatum = sectionRanges[i].min, key = `sections/station/${sid(section.station)}`;
        const project = (p)=>[
                origin[0] + (p[0] - offsetMin) * sh,
                origin[1] + (p[1] - elevationDatum) * sv
            ];
        ...
        line(`${key}/axis`, project([
            0,
            elevationDatum - t / sv
        ]), project([
            0,
            sectionRanges[i].max + t / sv
        ]), 'AXIS');
        line(`${key}/offset-axis`, project([
            offsetMin,
            elevationDatum
        ]), project([
            offsetMax,
        ...
        line(`${key}/elevation-upper-grid`, project([
            offsetMin,
            upperElevation
        ]), project([
            offsetMax,
        ...
        text(`${key}/elevation-upper-value`, formats(upperElevation), cellX, project([
            0,
            upperElevation
        ])[1] - .4 * t);
        ...
        projections.sections.push({
            station: section.station,
            horizontal: sh,
            vertical: sv,
            offsetDatum: offsetMin,
            elevationDatum,
            origin
        });
    });
    ...
    verify(bounds);
    for (const item of pending)entities.push({
        key: item.key,
        type: item.type,
        payload: {
            ...item.payload(),
            layerId: `${prefix}:layer:${item.layer}`
        },
        options: {
            id: `${prefix}:${item.key}`
        }
    ...
    return deepFreeze({
        ...
        limitations: [
            ...calculation.limitations,
            'Three model-space drawing frames, not paper-space sheets or a certified construction document.',
            'Profile and cross sections use explicitly stretched diagram coordinates; do not measure them as world geometry.',
            'Stable keys allow host-managed revision comparisons; automatic associative update is not implemented.',
            'Ground profile connects supplied section center elevations only; no surveyed terrain between sections is inferred.'
        ]
    });
}
