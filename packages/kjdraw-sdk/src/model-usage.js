// Generated from model-usage.ts by scripts/build-typescript.mjs. Do not edit directly.
import { KJValidationError } from './errors.js';
import { deepFreeze } from './utils.js';
export function extractKJModelUsage(protocol, response, { latencyMs = null } = {}) {
    if (![
        'responses',
        'chat-completions',
        'anthropic-messages',
        'gemini-generate-content'
    ].includes(protocol)) throw new KJValidationError('Unsupported model usage protocol');
    ...
    if (!anthropic && cacheReadInputTokens !== null && reportedInputTokens !== null && cacheReadInputTokens > reportedInputTokens) {
        invalid.add(effectiveReadPath);
        cacheReadInputTokens = null;
    }
    if (!gemini && reasoningOutputTokens !== null && reportedOutputTokens !== null && reasoningOutputTokens > reportedOutputTokens) {
        invalid.add(reasoningPath);
        reasoningOutputTokens = null;
    }
    if (protocol === 'responses' && cacheWriteInputTokens !== null && reportedInputTokens !== null && cacheWriteInputTokens > reportedInputTokens) {
        invalid.add('usage.input_tokens_details.cache_write_tokens');
        cacheWriteInputTokens = null;
    }
    const inputTokens = anthropic ? sum([
        reportedInputTokens,
        cacheReadInputTokens,
        cacheWriteInputTokens
    ], 'normalized.inputTokens') : reportedInputTokens;
    const outputTokens = gemini ? sum([
        reportedOutputTokens,
        reasoningOutputTokens
    ], 'normalized.outputTokens') : reportedOutputTokens;
    const calculatedTotal = sum([
        inputTokens,
        outputTokens
    ], 'normalized.totalTokens');
    let totalTokens = reportedTotalTokens, totalTokensSource = totalTokens === null ? null : 'reported';
    if (invalid.has('normalized.totalTokens')) {
        totalTokens = null;
        totalTokensSource = null;
    } else if (reportedTotalTokens !== null && calculatedTotal !== null && reportedTotalTokens !== calculatedTotal) {
        invalid.add(totalPath);
        totalTokens = null;
        totalTokensSource = null;
    } else if (reportedTotalTokens === null && !invalid.has(totalPath) && calculatedTotal !== null) {
        totalTokens = calculatedTotal;
        totalTokensSource = 'sum-components';
    }
    return deepFreeze({
        protocol,
        inputTokens,
        ...
        toolUsePromptTokens,
        latencyMs,
        latencyScope: latencyMs === null ? null : 'transport-wall',
        invalidFields: [
            ...invalid
        ]
    });
