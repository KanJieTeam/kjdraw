...
export function resolveAgentTransformEntityIds(document, sourceIds) {
    ...
    const result = [], added = new Set(), leaders = document.listEntities({
        type: 'LEADER'
    });
    const add = (id)=>{
        if (!added.has(id)) {
    ...
    for (const id of sourceIds){
        ...
        if (!member || member.kind !== 'entity' || member.erased) {
            add(id);
            continue;
        }
        if (member.type !== 'LEADER' && member.type !== 'MTEXT') {
            add(id);
            continue;
        }
...
function validateMovableAnnotation(document, entity) {
    ...
    if (entity.type === 'LEADER' || entity.type === 'MTEXT') {
        validateTransformGeometry(document, entity);
        return;
    }
...
function validateTransformGeometry(document, entity) {
    ...
    if (entity.type === 'LINE') points = [
        ...
        for (const field of [
            ...
            'blockOffset',
            'annotationOffset'
        ])if (payload[field] != null) points.push(payload[field]);
        ...
        for (const value of loops){
            ...
            if (hasVertices) {
                ...
                if (edges.some((value)=>value && typeof value === 'object' && !Array.isArray(value) && String(value.type).toUpperCase() === 'SPLINE') && edges.length !== 1) throw new KJValidationError('Transform preview requires a verified SPLINE to be the only edge in its closed hatch loop');
                for (const value of edges){
                    ...
                    const edge = value, type = String(edge.type).toUpperCase();
                    if (type === 'LINE') {
                        ...
                        points.push(...edge.controlPoints);
...
function validateStructuralEditPreview(document, args) {
    if (Object.keys(args).some((key)=>![
            ...
            'reconnections',
            'relayer'
        ].includes(key))) throw new KJValidationError('STRUCTURALEDIT preview contains unsupported fields');
    ...
    if (relayer !== undefined) {
        if (!relayer || typeof relayer !== 'object' || Array.isArray(relayer) || Object.keys(relayer).length !== 2 || !Object.hasOwn(relayer, 'ids') || !Object.hasOwn(relayer, 'layerId') || typeof relayer.layerId !== 'string') throw new KJValidationError('STRUCTURALEDIT relayer requires exactly ids and layerId');
        relayerIds = ids(relayer.ids, 'STRUCTURALEDIT relayer ids');
    }
    ...
    for (const raw of reconnections){
        ...
        const item = raw;
        if (typeof item.id !== 'string' || !item.id || item.id.length > 256 || createdIds.has(item.id) || document.getObject(item.id, {
            includeErased: true
        })) throw new KJValidationError('STRUCTURALEDIT reconnection IDs must be unique new bounded IDs');
        if (![
            'LINE',
            'LWPOLYLINE'
        ].includes(String(item.type))) throw new KJValidationError('STRUCTURALEDIT reconnection type must be LINE or LWPOLYLINE');
...
export async function createAgentGeometryPreview(document, command, args, options = {}) {
    ...
    if (command === 'DESIGNCREATE') {
        if (typeof args.id !== 'string' || !args.id.trim() || Object.keys(args).some((key)=>![
                ...
                'name',
                'definition'
            ].includes(key))) throw new KJValidationError('DESIGNCREATE preview requires a stable preallocated ID, name and definition');
        const bindings = args.definition?.bindings;
        ...
        bindingIds = [
            ...new Set(bindings.map((binding)=>String(binding.entityId)))
        ];
    }
    if (command === 'DESIGNUPDATE' && (typeof args.id !== 'string' || Object.keys(args).some((key)=>![
            'id',
            'parameters'
        ].includes(key)))) throw new KJValidationError('DESIGNUPDATE requires a design ID and independent parameter changes');
    const design = command === 'DESIGNUPDATE' ? readDesignRelations(document, [
        String(args.id)
    ])[0] : undefined;
    ...
    if (command === 'CREATEBATCH') {
        ...
        if (command === 'PEDIT') {
            ...
            if (Object.keys(args).some((key)=>![
                    'ids',
                    'patch'
                ].includes(key)) || ids.some((id)=>typeof id !== 'string') || new Set(ids).size !== ids.length) throw new KJValidationError('Relayer preview requires only 1–64 unique entity IDs and one layer patch');
            const patch = args.patch;
            if (!patch || typeof patch !== 'object' || Array.isArray(patch) || ![
                Object.prototype,
                null
            ].includes(Object.getPrototypeOf(patch)) || Object.keys(patch).length !== 1 || !Object.hasOwn(patch, 'payload')) throw new KJValidationError('Relayer preview requires exactly patch.payload.layerId');
            const payload = patch.payload;
            if (!payload || typeof payload !== 'object' || Array.isArray(payload) || ![
                Object.prototype,
                null
            ].includes(Object.getPrototypeOf(payload)) || Object.keys(payload).length !== 1 || !Object.hasOwn(payload, 'layerId') || typeof payload.layerId !== 'string') throw new KJValidationError('Relayer preview requires exactly patch.payload.layerId');
    ...
    if (new TextEncoder().encode(JSON.stringify({
        ...
        workingSet,
        ...design ? {
            design: document.getObject(design.id)
        } : {}
    ...
    const designChange = designId ? {
        id: designId,
        before: design?.definition ?? {
            parameters: [],
            derived: [],
            bindings: [],
            requirements: []
        },
        after: readDesignRelations(draft, [
            designId
        ])[0].definition,
        record: draft.getObject(designId),
        members: ids.map((id)=>draft.getObject(id)),
        dictionary: {
            id: dictionaryId,
            key: dictionaryKey
        }
...
export function agentPreviewMatchesDocument(document, preview) {
    ...
    return document.id === preview.documentId && (!preview.designChange || (()=>{
        ...
        return actual?.kind === 'custom' && !actual.erased && actual.type === 'DESIGN_RELATIONS' && canonicalStringify(actual) === canonicalStringify(preview.designChange.record);
