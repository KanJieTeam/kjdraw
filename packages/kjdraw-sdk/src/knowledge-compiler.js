// Generated from knowledge-compiler.ts by scripts/build-typescript.mjs. Do not edit directly.
import { KJValidationError } from './errors.js';
import { validateKnowledgePack, validateSemanticDrawingIntent } from './knowledge-pack.js';
...
const fail = (message)=>{
    throw new KJValidationError(`Knowledge compiler: ${message}`);
};
...
function getPath(context, source) {
    const path = text(source, 'expression.get', 192);
    const parts = path.split('.');
    if (parts.length < 2 || parts.length > 10 || parts.some((part)=>!safeKey.test(part) || [
            '__proto__',
            'prototype',
            'constructor'
        ].includes(part))) fail(`expression.get path is not allowed: ${path}`);
...
function evaluate(source, context, depth = 0, budget = {
    count: 0
}) {
    ...
    if (Object.hasOwn(expression, 'get')) {
        ...
        if (![
            ...
            'number',
            'boolean'
        ].includes(typeof result)) fail('expression.get must resolve to a string, number or boolean');
        return typeof result === 'number' ? finite(result, 'expression.get result') : result;
    }
    if (Object.hasOwn(expression, 'op')) {
        ...
        if (![
            ...
            'divide',
            'negate'
        ].includes(String(op))) fail(`numeric operation is not supported: ${String(op)}`);
        ...
        const values = expression.args.map((value)=>finite(evaluate(value, context, depth + 1, budget), 'numeric expression argument'));
        if (op === 'negate') {
    ...
    if (Object.hasOwn(expression, 'concat')) {
        ...
        const result = expression.concat.map((value)=>String(evaluate(value, context, depth + 1, budget))).join('');
...
function numberExpression(value, context, label) {
    return finite(evaluate(value, context), label);
}
function point(value, context, label) {
    if (!Array.isArray(value) || value.length !== 2) fail(`${label} must contain two expressions`);
    const values = value;
    return [
        numberExpression(values[0], context, `${label}.x`),
        numberExpression(values[1], context, `${label}.y`),
        0
    ];
}
function selectItems(step, intent, context) {
    if (step.select == null) return [
        context.root
    ];
    ...
    const byId = new Map(intent.objects.map((item)=>[
            item.id,
            item
        ]));
    const selected = [];
...
function validateContinuity(source, items, context) {
...
export function compileKnowledgeDrawing(source) {
    ...
    const programLayers = program.layers;
    const programSteps = program.steps;
    const layers = programLayers.map((raw, index)=>{
        const layer = record(raw, `program.layers[${index}]`);
        exact(layer, [
            'name',
            'color',
            'lineweight'
        ], `program.layers[${index}]`);
    ...
    for (const [stepIndex, rawStep] of programSteps.entries()){
        ...
        for (const item of items)for (const [emitIndex, rawEmit] of emissions.entries()){
            ...
            const primitive = text(emit.primitive, 'emit.primitive', 32), context = {
                ...baseContext,
                item
            };
            if (primitive === 'line') {
                ...
                const vertices = emit.points.map((value, index)=>point(value, context, `polyline.points[${index}]`));
                if (emit.closed && vertices.length < 3) fail('closed polyline requires at least three points');
                add('LWPOLYLINE', emit.layer, {
                    vertices,
                    closed: emit.closed
                });
            } else if (primitive === 'rectangle' || primitive === 'hatch-rectangle') {
                exact(emit, primitive === 'rectangle' ? [
                    'primitive',
                    'layer',
                    'origin',
                    'size'
                ...
                if (primitive === 'rectangle') add('LWPOLYLINE', emit.layer, {
                    vertices,
                    closed: true
                });
                else {
                    ...
                    if (![
                        ...
                        'ANSI37',
                        'CROSS'
                    ].includes(patternName)) fail(`hatch pattern is not supported: ${patternName}`);
                    ...
                    add('HATCH', emit.layer, {
                        ...
                        patternName,
                        solid: patternName === 'SOLID',
                        patternScale,
                        patternAngle: patternAngle * Math.PI / 180
                    });
                }
                ...
                add('TEXT', emit.layer, {
                    position,
                    text: text(value, 'text.value', 512),
                    height,
                    rotation
                });
        ...
        commandArgs: {
            ...
            resources: {
                linetypes: [
                    {
                        id: linetypeId,
                        name: `KP_${stableHash(prefix).toUpperCase()}_CONT`,
                        pattern: []
                    }
                ],
                layers
