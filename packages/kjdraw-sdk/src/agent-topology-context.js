// Generated from agent-topology-context.ts by scripts/build-typescript.mjs. Do not edit directly.
...
function entityFeatures(entity) {
    const payload = entity.payload;
    if (!defaultPlane(payload)) return {
        features: [],
        omittedReason: 'non-default-coordinate-plane'
    };
    ...
    if (entity.type === 'LINE') {
        ...
        return start && end ? {
            ...
            features: [],
            omittedReason: 'unsupported-data'
        };
    }
    if (entity.type === 'ARC') {
        const center = point3(payload.center), radius = payload.radius, startAngle = payload.startAngle, endAngle = payload.endAngle;
        if (!center || !finite(radius) || radius <= 0 || !finite(startAngle) || !finite(endAngle)) return {
            features: [],
            omittedReason: 'unsupported-data'
        };
        if (Math.abs(endAngle - startAngle) >= Math.PI * 2 - 1e-12) return {
            features: [],
            omittedReason: 'closed-curve-has-no-endpoint'
        };
        return {
            features: [
                {
    ...
    if (entity.type === 'ELLIPSE') {
        ...
        if (!center || !axis || axis[2] !== 0 || !finite(ratio) || ratio <= 0 || ratio > 1 || !finite(start) || !finite(end) || Math.hypot(axis[0], axis[1]) <= 1e-12) return {
            features: [],
            omittedReason: 'unsupported-data'
        };
        if (Math.abs(end - start) >= Math.PI * 2 - 1e-12) return {
            features: [],
            omittedReason: 'closed-curve-has-no-endpoint'
        };
        const at = (parameter)=>[
    ...
    if (entity.type === 'LWPOLYLINE' || entity.type === 'POLYLINE') {
        ...
        for(let vertexIndex = 0; vertexIndex < payload.vertices.length; vertexIndex++){
            ...
            const point = point3(raw && typeof raw === 'object' && !Array.isArray(raw) ? raw.point : raw);
            if (!point) return {
                features: [],
                omittedReason: 'unsupported-data'
            };
            features.push({
                ...
                point,
                ownerId: String(entity.ownerId),
                incidentSegmentCount: payload.closed === true || vertexIndex > 0 && vertexIndex < payload.vertices.length - 1 ? 2 : 1
            });
        }
        return {
            features
    ...
    if (entity.type === 'INSERT') return {
        features: [],
        omittedReason: 'block-instance-not-expanded'
    };
    if (entity.type === 'HATCH') return {
        features: [],
        omittedReason: 'region-fill-not-connectivity'
    };
    if (entity.type === 'CIRCLE' || entity.type === 'RAY' || entity.type === 'XLINE') return {
        features: [],
        omittedReason: 'entity-has-no-finite-endpoints'
    };
    return {
        features: [],
        omittedReason: 'unsupported-type'
    };
}
class UnionFind {
    parent;
    ...
    find(index) {
        ...
        return this.parent[index] = this.find(parent);
    }
    union(a, b) {
...
function connectFeatureSites(features, tolerance) {
    const siteByPoint = new Map();
    for (const feature of features){
        const normalized = feature.point.map((value)=>Object.is(value, -0) ? 0 : value);
    ...
    const sites = [
        ...siteByPoint.values()
    ].sort((a, b)=>a.ownerId.localeCompare(b.ownerId) || a.point[0] - b.point[0] || a.point[1] - b.point[1] || a.point[2] - b.point[2]);
    ...
    while(ownerStart < sites.length){
        ...
        const origin = [
            Math.min(...sites.slice(ownerStart, ownerEnd).map((site)=>site.point[0])),
            Math.min(...sites.slice(ownerStart, ownerEnd).map((site)=>site.point[1])),
            Math.min(...sites.slice(ownerStart, ownerEnd).map((site)=>site.point[2]))
        ];
        const cellSize = tolerance / 2;
        const coordinates = sites.slice(ownerStart, ownerEnd).map((site)=>site.point.map((value, axis)=>Math.floor((value - origin[axis]) / cellSize)));
        ...
        if (safeGrid) {
            ...
            for(let localIndex = 0; localIndex < coordinates.length; localIndex++){
                ...
                for(let dx = -2; dx <= 2; dx++)for(let dy = -2; dy <= 2; dy++)for(let dz = -2; dz <= 2; dz++){
                    ...
                    const bucket = buckets.get(key(...candidateCell));
                    if (!bucket) continue;
                    ...
                    for(let axis = 0; axis < 3; axis++){
                        ...
                        minSquared += nearest * nearest;
                        maxSquared += farthest * farthest;
                    }
                    if (minSquared > toleranceSquared) continue;
                    if (maxSquared <= toleranceSquared) {
                        sets.union(siteIndex, bucket[0]);
                        continue;
                    }
    ...
    for(let index = 0; index < sites.length; index++){
        ...
        group.push(...sites[index].features);
...
function hatchLoopSourceHandles(payload) {
    ...
    for(let index = 0; index < tags.length; index++){
        ...
        if (Number(tag.code) === 92) {
            loopIndex++;
            continue;
        }
        if (Number(tag.code) !== 97) continue;
        const count = Number(tag.value);
        ...
        for(let offset = 1; offset <= count; offset++){
            ...
            if (!source || typeof source !== 'object' || Array.isArray(source) || Number(source.code) !== 330) break;
            const handle = String(source.value ?? '').trim().toUpperCase();
            if (/^[0-9A-F]+$/.test(handle)) handles.push(handle);
        }
        result.set(Math.max(loopIndex, 0), [
            ...new Set(handles)
        ].sort());
    }
    return result;
}
export function createAgentTopologyContext(document, query) {
    ...
    const records = [
        ...query.ids
    ].sort().map((id)=>state.objects[id]).map((record, index)=>{
        if (!record || record.erased || record.kind !== 'entity') fail(`Topology query entity does not exist: ${[
            ...query.ids
        ].sort()[index]}`);
        return record;
    });
    ...
    for (const object of Object.values(state.objects))if (!object.erased && object.kind === 'entity') {
        ...
        list.push(object);
        entitiesByHandle.set(object.handle.toUpperCase(), list);
    }
    ...
    const selectionMembership = new Map(), designMembership = new Map();
    const associatedAnnotations = new Map(), annotationDependencies = new Map();
    const omitted = [];
    for (const selection of [
        ...
        if (!Array.isArray(members) || members.length < 1 || members.length > 64 || members.some((id)=>typeof id !== 'string' || !id || id.length > 256) || new Set(members).size !== members.length) {
            if (Array.isArray(members) && members.some((id)=>typeof id === 'string' && selectedIds.has(id))) omitted.push({
                kind: 'selection-set',
                id: selection.id,
        ...
        for (const id of members){
            ...
            selectionMembership.set(id, list);
        }
    }
    for (const relation of [
        ...document.listObjects({
            type: 'DESIGN_RELATIONS'
        })
    ].sort((a, b)=>a.id.localeCompare(b.id))){
        try {
            for (const id of readDesignRelations(document, [
                relation.id
            ])[0]?.entityIds ?? []){
                const list = designMembership.get(id) ?? [];
                list.push(relation.id);
                designMembership.set(id, list);
            }
        } catch  {
            ...
            if (Array.isArray(bindings) && bindings.some((binding)=>binding && typeof binding === 'object' && !Array.isArray(binding) && selectedIds.has(String(binding.entityId ?? '')))) omitted.push({
                kind: 'design',
                id: relation.id,
                reason: 'invalid-design-relation'
            });
        }
    }
    for (const annotation of [
        ...
        if (annotation.type === 'DIMENSION' && Array.isArray(annotation.payload.dimensionAssociations)) {
            try {
                ...
                annotationDependencies.set(annotation.id, dependencies.map((dependency)=>({
                        kind: 'dimension-point',
                        ...dependency
                    })));
                for (const dependency of dependencies){
                    const ids = associatedAnnotations.get(dependency.entityId) ?? new Set();
                    ids.add(annotation.id);
                    associatedAnnotations.set(dependency.entityId, ids);
                }
            } catch  {
                if (selectedIds.has(annotation.id) || annotation.payload.dimensionAssociations.some((dependency)=>dependency && typeof dependency === 'object' && !Array.isArray(dependency) && selectedIds.has(String(dependency.entityId ?? '')))) omitted.push({
                    kind: 'annotation',
                    id: annotation.id,
                    reason: 'invalid-dimension-associations'
                });
            }
        }
        if ((annotation.type === 'LEADER' || annotation.type === 'MLEADER') && typeof annotation.payload.annotationId === 'string' && annotation.payload.annotationId) {
            annotationDependencies.set(annotation.id, [
                {
                    kind: 'leader-annotation',
                    entityId: annotation.payload.annotationId
                }
            ]);
            const ids = associatedAnnotations.get(annotation.payload.annotationId) ?? new Set();
            ids.add(annotation.id);
            associatedAnnotations.set(annotation.payload.annotationId, ids);
        }
    }
    const features = [], connectivity = new Map();
    const notApplicableReasons = new Set([
        'block-instance-not-expanded',
        'closed-curve-has-no-endpoint',
        'entity-has-no-finite-endpoints',
        'region-fill-not-connectivity'
    ]);
    for (const entity of records){
        const extracted = entityFeatures(entity);
        if (extracted.omittedReason) {
            connectivity.set(entity.id, {
                status: notApplicableReasons.has(extracted.omittedReason) ? 'not-applicable' : 'omitted',
                connectionFeatureCount: 0,
                reason: extracted.omittedReason
            });
            omitted.push({
                kind: 'entity',
                id: entity.id,
            ...
            connectivity.set(entity.id, {
                status: 'omitted',
                connectionFeatureCount: 0,
                reason: 'topology-feature-budget'
            });
            omitted.push({
                kind: 'entity',
                id: entity.id,
            ...
            connectivity.set(entity.id, {
                status: 'supported',
                connectionFeatureCount: extracted.features.length
            });
    ...
    const connectionGroups = connected.groups.map((group)=>group.sort((a, b)=>a.entityId.localeCompare(b.entityId) || a.feature.localeCompare(b.feature) || (a.vertexIndex ?? -1) - (b.vertexIndex ?? -1))).sort((a, b)=>a[0].ownerId.localeCompare(b[0].ownerId) || a[0].point[0] - b[0].point[0] || a[0].point[1] - b[0].point[1] || a[0].point[2] - b[0].point[2] || a[0].entityId.localeCompare(b[0].entityId)).map((group, index)=>({
            id: `connection:${String(index + 1).padStart(4, '0')}`,
            ownerId: group[0].ownerId,
            point: group[0].point,
            members: group.map(({ entityId, feature, vertexIndex, incidentSegmentCount })=>({
                    entityId,
                    feature,
    ...
    const entityIndex = new Map(entityIds.map((id, index)=>[
            id,
            index
        ])), entitySets = new UnionFind(entityIds.length);
    for (const group of connectionGroups){
        const ids = [
            ...new Set(group.members.map((member)=>member.entityId))
        ];
        for(let index = 1; index < ids.length; index++)entitySets.union(entityIndex.get(ids[0]), entityIndex.get(ids[index]));
    }
    const componentMap = new Map();
    for (const id of entityIds){
        const root = entitySets.find(entityIndex.get(id)), list = componentMap.get(root) ?? [];
        list.push(id);
        componentMap.set(root, list);
    }
    const components = [
        ...componentMap.values()
    ].map((ids)=>ids.sort()).sort((a, b)=>a[0].localeCompare(b[0])).map((ids, index)=>({
            id: `component:${String(index + 1).padStart(4, '0')}`,
            entityIds: ids
        }));
    const branchNodes = connectionGroups.map((group)=>({
            connectionId: group.id,
            entityIds: [
                ...new Set(group.members.map((member)=>member.entityId))
            ].sort(),
            degree: group.members.reduce((sum, member)=>sum + member.incidentSegmentCount, 0)
        })).filter((branch)=>branch.degree >= 3);
    const entities = records.map((entity)=>{
        ...
        let hatchBoundarySources;
        const resolveHatchSource = (handle)=>{
            ...
            if (source.ownerId !== entity.ownerId) return {
                handle,
        ...
        if (hatchLoopSources) hatchBoundarySources = [
            ...hatchLoopSources.values()
        ].flatMap((handles)=>handles.map(resolveHatchSource));
        let insertReference;
        if (entity.type === 'INSERT') {
            ...
            insertReference = {
                ...
                definitionEntityCount: memberIds.length,
                resolvedEntityCount: resolvedCount,
                definitionTypes,
                typeCountSignature: stableHash({
                    definitionEntityCount: memberIds.length,
                    definitionTypes
                }),
                position: point3(entity.payload.position),
                scale: Array.isArray(entity.payload.scale) ? entity.payload.scale.slice(0, 3) : null,
                rotation: finite(entity.payload.rotation) ? entity.payload.rotation : 0,
                repeat: {
                    source: 'native',
                    scope: 'document',
                    sameDefinitionInstanceCount: blockRecordId ? insertCounts.get(blockRecordId) ?? 0 : 0
                }
            };
        }
        ...
        return {
            ...
            layer: {
                ...
                editable: layerVisible && layer?.payload.locked !== true
            },
            visible,
            editable: visible && layer?.payload.locked !== true,
            selectionSetIds: [
                ...selectionMembership.get(entity.id) ?? []
            ].sort(),
            designIds: [
                ...designMembership.get(entity.id) ?? []
            ].sort(),
            associatedAnnotationIds: [
                ...associatedAnnotations.get(entity.id) ?? []
            ].sort(),
            annotationDependencies: annotationDependencies.get(entity.id) ?? [],
            connectivity: connectivity.get(entity.id),
            ...hatchBoundarySources === undefined && insertReference === undefined ? {} : {
                nativeReferences: {
                    structuralRole: entity.type === 'HATCH' ? 'region-fill' : 'block-instance',
                    patternCandidate: {
                        eligible: true,
                        kind: entity.type === 'HATCH' ? 'region-fill' : 'symbol-candidate',
                        source: 'native-entity-type',
                        inferredRole: null,
                    ...
                    displayExtent: displayExtent ? {
                        ...
                        status: 'complete',
                        bounds: displayExtent
                    } : {
                        source: 'native-projection',
                        status: 'unavailable',
                        bounds: null
                    },
                    ...hatchBoundarySources === undefined ? {} : {
                        hatch: {
                            source: 'native',
                            patternName: typeof entity.payload.patternName === 'string' ? entity.payload.patternName : null,
                            solid: entity.payload.solid === true,
                            associative: entity.payload.associative === true,
                            patternScale: finite(entity.payload.patternScale) ? entity.payload.patternScale : null,
                            patternAngle: finite(entity.payload.patternAngle) ? entity.payload.patternAngle : null,
                            loops: Array.from({
                                length: Array.isArray(entity.payload.boundaryLoops) ? entity.payload.boundaryLoops.length : 0
                            }, (_, loopIndex)=>({
                                    loopIndex,
                                    boundarySources: (hatchLoopSources?.get(loopIndex) ?? []).map(resolveHatchSource)
                                }))
                        }
                    },
    ...
    const result = {
        ...
        connectivityOmissions: omitted.filter((item)=>item.kind === 'entity'),
        omitted: omitted.filter((item)=>item.kind !== 'entity').sort((a, b)=>a.kind.localeCompare(b.kind) || a.id.localeCompare(b.id) || a.reason.localeCompare(b.reason)),
        limits: {
            maxIds: 64,
            maxTopologyFeatures: MAX_TOPOLOGY_FEATURES,
            maxComparisons: MAX_TOPOLOGY_COMPARISONS,
            comparisons: connected.comparisons,
    ...
    if (jsonBytes({
        ok: true,
        value: result
    }) > query.maxBytes) fail('Topology query result exceeds maxBytes; query fewer IDs or increase maxBytes');
