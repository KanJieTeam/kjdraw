// Generated from dimension-associations.ts by scripts/build-typescript.mjs. Do not edit directly.
...
export function normalizeDimensionAssociations(value) {
    ...
    const result = value.map((candidate, offset)=>{
        ...
        if (typeof item.feature !== 'string' || !FEATURES.has(item.feature)) return fail(`Dimension association ${offset} feature is unsupported`);
        ...
        return {
            ...
            feature,
            ...feature === 'vertex' ? {
                vertexIndex: Number(item.vertexIndex)
            } : {},
            ...feature === 'curve' ? {
...
export function migratePolylineDimensionAssociations(transaction, sourceId, edit) {
    ...
    for (const readonlyDimension of Object.values(transaction._draft().objects)){
        ...
        if (stableHash(migrated) !== stableHash(associations)) {
            updated.push(transaction.updateObject(readonlyDimension.id, {
                payload: {
...
export function migrateBreakDimensionAssociations(transaction, sourceId, trailingId, polylineVertexMap) {
    const updated = [];
    for (const readonlyDimension of Object.values(transaction._draft().objects)){
        if (readonlyDimension.kind !== 'entity' || readonlyDimension.type !== 'DIMENSION' || readonlyDimension.erased || !Array.isArray(readonlyDimension.payload.dimensionAssociations)) continue;
        const associations = normalizeDimensionAssociations(readonlyDimension.payload.dimensionAssociations);
...
const normalizedAngle = (value)=>(value % (2 * Math.PI) + 2 * Math.PI) % (2 * Math.PI);
const breakFeatureAngle = (association)=>{
...
const strictlyInsideArc = (entity, angle)=>{
...
export function migrateCircleBreakDimensionAssociations(transaction, sourceId, pieces) {
...
function curvePoint(entity, angle) {
    if (![
        'CIRCLE',
        'ARC'
    ].includes(entity.type)) return fail(`Dimension curve association requires CIRCLE or ARC: ${entity.id}`);
...
function resolvePoint(transaction, dimension, association) {
    ...
    switch(association.feature){
        ...
        case 'center':
            if (![
                'CIRCLE',
                'ARC'
            ].includes(source.type)) return fail('Dimension center association requires CIRCLE or ARC');
            return point3(payload.center, `Dimension source ${source.id} center`);
        case 'left':
            return curvePoint(source, Math.PI);
        case 'right':
            return curvePoint(source, 0);
        case 'top':
            return curvePoint(source, Math.PI / 2);
        case 'bottom':
            return curvePoint(source, 3 * Math.PI / 2);
        case 'curve':
            return curvePoint(source, association.angle);
        case 'vertex':
            {
...
export function refreshAssociativeDimensions(transaction, changedEntityIds) {
    ...
    for (const readonlyDimension of dimensions){
        ...
        const definitionPoints = Array.isArray(dimension.payload.definitionPoints) ? clone(dimension.payload.definitionPoints) : [];
        ...
        updated.push(transaction.updateObject(dimension.id, {
            payload: {
                definitionPoints,
                measurement: null,
                blockName: null
