// Generated from component-library.ts by scripts/build-typescript.mjs. Do not edit directly.
...
const LICENSE = {
    spdx: 'Apache-2.0',
    source: 'KJDraw original component catalog',
    sourceUrl: 'https://github.com/KanJieTeam/kjdraw'
};
...
const DEFINITIONS = deepFreeze([
    {
        id: 'org.kjdraw.mechanical.four-hole-flange',
        version: '1.0.0',
        category: 'mechanical',
        title: {
            en: 'Four-hole mounting flange',
            zh: '四孔安装法兰'
        },
        description: {
            en: 'Parametric circular flange with bore and equally spaced mounting holes.',
            zh: '带中心孔和等距安装孔的参数化圆形法兰。'
        },
        keywords: {
            en: [
                'flange',
    ...
    {
        id: 'org.kjdraw.architecture.single-swing-door',
        version: '1.0.0',
        category: 'architecture',
        title: {
            en: 'Single swing door',
            zh: '单扇平开门'
        },
        description: {
            en: 'Plan symbol with jambs, door leaf and exact swing arc.',
            zh: '包含门垛、门扇和精确开启弧线的平面符号。'
        },
        keywords: {
            en: [
                'door',
        ...
        build (p, factor) {
            ...
            return [
                line(0, -wall / 2, 0, wall / 2),
                line(width, -wall / 2, width, wall / 2),
    ...
    {
        id: 'org.kjdraw.electrical.duplex-receptacle',
        version: '1.0.0',
        category: 'electrical',
        title: {
            en: 'Duplex receptacle',
            zh: '双联电源插座'
        },
        description: {
            en: 'General electrical plan symbol for a duplex receptacle.',
            zh: '用于电气平面图的通用双联插座符号。'
        },
        keywords: {
            en: [
        ...
        build (p, factor) {
            ...
            return [
                circle(0, 0, radius),
                line(-radius * .55, -radius * .55, -radius * .55, radius * .55),
                line(radius * .55, -radius * .55, radius * .55, radius * .55),
...
const PUBLIC_CATALOG = deepFreeze(DEFINITIONS.map(({ build: _build, ...entry })=>entry));
const BY_ID = new Map(DEFINITIONS.map((entry)=>[
        entry.id,
        entry
    ]));
const DRAWING_UNITS_PER_MILLIMETER = Object.freeze({
    millimeter: 1,
    mm: 1,
    centimeter: .1,
    cm: .1,
    ...
    inch: 1 / 25.4,
    foot: 1 / 304.8,
    'us-survey-foot': 3937 / 1_200_000
});
function fail(message) {
    throw new KJValidationError(`COMPONENT: ${message}`);
}
function finite(value, label, minimum = -1e12, maximum = 1e12) {
...
function exactKeys(value, allowed, label) {
    ...
    const result = value, unknown = Object.keys(result).find((key)=>!allowed.includes(key));
    if (unknown) fail(`${label} contains unsupported field: ${unknown}`);
    return result;
}
export function listComponentCatalog() {
    return PUBLIC_CATALOG;
}
export function searchComponentCatalog(input = {}) {
    const query = String(input.query ?? '').trim();
    if (query.length > 128) fail('search query must contain at most 128 characters');
    const locale = input.locale == null ? 'en' : String(input.locale);
    if (![
        'en',
        'zh-CN'
    ].includes(locale)) fail('locale must be en or zh-CN');
    const category = input.category == null || String(input.category).trim() === '' ? null : String(input.category).toLowerCase();
    if (category !== null && ![
        'mechanical',
        'architecture',
        'electrical'
    ].includes(category)) fail('category must be mechanical, architecture or electrical');
    ...
    return deepFreeze({
        catalogVersion: KJDRAW_COMPONENT_CATALOG_VERSION,
        query,
        category,
        locale,
...
function resolveParameters(definition, input) {
    const supplied = exactKeys(input, definition.parameters.map((parameter)=>parameter.name), 'parameters'), values = {};
...
function point3(value) {
    ...
    return [
        finite(value[0], 'position x'),
        finite(value[1], 'position y'),
        value.length === 3 ? finite(value[2], 'position z') : 0
    ];
}
function definitionName(definition, units, values) {
    const slug = definition.id.split('.').at(-1).replace(/[^a-z0-9]+/gi, '_').toUpperCase().slice(0, 40);
    return `KJCOMP_${slug}_${stableHash([
        definition.id,
        definition.version,
        units,
        values
...
function componentMetadata(definition, units, values, entityCount) {
    return {
        contractVersion: 1,
        catalogVersion: KJDRAW_COMPONENT_CATALOG_VERSION,
        ...
        units,
        parameters: clone(values),
        license: clone(definition.license),
        entityCount
    };
}
function componentIdentity(value, entityCount) {
    ...
    if (new Set([
        ...
        insertId,
        ...memberIds
    ]).size !== entityCount + 2) fail('identity IDs must be unique');
    return deepFreeze({
        definitionId,
        memberIds: [
            ...memberIds
...
export function createCatalogComponentInsertIdentity(document, input) {
    ...
    const memberIds = existing?.kind === 'block-record' && !existing.erased && Array.isArray(existing.payload.entityIds) ? existing.payload.entityIds.map(String) : Array.from({
        length: count
    }, ()=>createId('entity'));
    return deepFreeze({
        definitionId: existing?.id ?? createId('block'),
        memberIds,
        insertId: createId('entity')
    });
}
export function insertCatalogComponent(_document, transaction, input) {
    const definition = resolveComponent(input), state = transaction._draft(), units = String(input.units ?? '');
    if (!units || units !== state.header.units) fail('units must exactly match drawing units');
    const factor = DRAWING_UNITS_PER_MILLIMETER[normalizeName(units).toLowerCase()];
    if (!(factor != null && factor > 0)) fail(`drawing unit is not supported for physical components: ${units}`);
    const values = resolveParameters(definition, input.parameters), position = point3(input.position ?? [
        0,
        0,
        0
    ]);
    ...
    const specs = definition.build(values, factor).map((spec)=>({
            type: spec.type,
            payload: {
                ...clone(spec.payload),
                layerId: zeroLayerId
    ...
    if (existing) {
        ...
        block = transaction.upsertTableRecord('blockRecords', {
            ...identity ? {
            ...
            payload: {
                ...
                component: expectedMetadata
            }
        });
        for (const [index, spec] of specs.entries())transaction.createEntity(spec.type, spec.payload, {
            ownerId: block.id,
            ...identity ? {
                id: identity.memberIds[index]
            } : {}
        });
    }
    const insert = transaction.createEntity('INSERT', {
        ...
        ownerId,
        ...identity ? {
            id: identity.insertId
        } : {}
    });
    return deepFreeze({
        catalogVersion: KJDRAW_COMPONENT_CATALOG_VERSION,
        component: PUBLIC_CATALOG.find((entry)=>entry.id === definition.id),
        parameters: values,
        units,
        definitionId: block.id,
        definitionName: name,
        definitionReused: reused,
        definitionEntityCount: specs.length,
        insert
    });
}
