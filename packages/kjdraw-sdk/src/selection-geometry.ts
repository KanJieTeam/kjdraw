...
function project(entity: KJReadonlyObjectRecord, document: KJDocument, depth = 0, inheritedMatrix?: readonly number[]): Projection {
  ...
  if (inheritedMatrix && ['DIMENSION','TEXT','MTEXT','ATTRIB','ATTDEF'].includes(entity.type)) {
    ...
    const vector = (p: Point): Point => [matrix[0]! * p[0] + matrix[2]! * p[1], matrix[1]! * p[0] + matrix[3]! * p[1]]
    result.parts = result.parts.map(part => part.kind === 'curve' ? { ...part, center: transform(part.center), u: vector(part.u), v: vector(part.v) } : part.kind === 'point' ? { ...part, point: transform(part.point) } : { ...part, a: transform(part.a), b: transform(part.b) })
  ...
  for (const parts of loops) for (let index = 0; index < parts.length; index++) {
    ...
    if(!circular&&Math.abs(part.sweep)>=TAU-1e-10){
      ...
      if(radius>0&&Math.abs(dy)<radius-1e-12){
        ...
        for(const angle of [phase-offset,phase+offset])if(curveAt(part,angle)[0]>p[0])inside=!inside
      }
      continue
    }
