// Generated from agent-design-relations.ts by scripts/build-typescript.mjs. Do not edit directly.
import { readDesignRelations } from './design-relations.js';
import { KJValidationError } from './errors.js';
export function createAgentDesignContext(document, offset, limit, maxBytes) {
    ...
    const rows = result.designs;
    let consumed = offset;
    for (const record of records.slice(offset, offset + limit)){
        const design = readDesignRelations(document, [
            record.id
        ])[0];
        const row = {
            id: design.id,
            name: design.name,
            units: design.units,
            parameters: design.definition.parameters,
            derived: Object.fromEntries(design.definition.derived.map((parameter)=>[
                    parameter.name,
                    design.values[parameter.name]
                ])),
            entityIds: design.entityIds,
            driftedEntityIds: design.driftedEntityIds
        };
        rows.push(row);
    ...
    if (new TextEncoder().encode(JSON.stringify({
        ok: true,
        value: result
    })).length > maxBytes) throw new KJValidationError('Design context metadata exceeds the byte budget');
    return result;
}
