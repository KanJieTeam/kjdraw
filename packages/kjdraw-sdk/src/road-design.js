// Generated from road-design.ts by scripts/build-typescript.mjs. Do not edit directly.
import { KJValidationError } from './errors.js';
import { deepFreeze } from './utils.js';
const fail = (message)=>{
    throw new KJValidationError(`Road design: ${message}`);
};
...
function points(value, label) {
    ...
    for (const point of value){
        list(point, 2, 2, label);
        finite(point[0], label);
        finite(point[1], label);
    }
}
function increasing(value, previous, label) {
...
function interpolation(points, x) {
    ...
    while(hi - lo > 1){
        ...
        if (points[middle][0] <= x) lo = middle;
        else hi = middle;
    }
...
export function computeRoadDesign(input) {
    record(input, [
        'units',
        'startStation',
        'alignment',
    ...
    for (const [key, value] of Object.entries(input.slopes)){
        finite(value, key);
        if (!(value > 0)) fail(`${key} must be positive`);
    }
    ...
    for (const [index, item] of input.profile.entries()){
        ...
        finite(item.station, 'profile station');
        finite(item.elevation, 'profile elevation');
    ...
    for (const [index, section] of input.sections.entries()){
        ...
        finite(section.station, 'section station');
        points(section.ground, 'ground');
    ...
    const grades = input.profile.slice(1).map((point, index)=>{
        ...
        return {
            fromStation: before.station,
            toStation: point.station,
            fromElevation: before.elevation,
            toElevation: point.elevation,
            grade: (point.elevation - before.elevation) / (point.station - before.station)
        };
    });
    const profile = input.profile.map((point)=>[
            point.station,
            point.elevation
        ]);
    const sections = input.sections.map((section)=>{
        ...
        const pavement = [
            [
                -rightWidth,
                elevation + rightWidth * rightCrossfall
            ],
            [
                0,
                elevation
        ...
        const offsets = [
            ...new Set([
                ...design.map((point)=>point[0]),
                ...ground.filter((point)=>point[0] > right.point[0] && point[0] < left.point[0]).map((point)=>point[0])
            ])
        ].sort((a, b)=>a - b);
        ...
        const strips = [];
        const addStrip = (from, to, a, b)=>{
            const signedArea = (to - from) * (a + b) / 2;
            strips.push({
                fromOffset: from,
                toOffset: to,
                cutArea: Math.max(0, -signedArea),
                fillArea: Math.max(0, signedArea)
            });
        };
        for(let index = 1; index < offsets.length; index++){
            ...
            if (a < 0 && b > 0 || a > 0 && b < 0) {
                ...
                addStrip(from, zero, a, 0);
                addStrip(zero, to, 0, b);
            } else addStrip(from, to, a, b);
        }
        return {
            station,
            center,
            tangent: copyPoint(segment.tangent),
            designElevation: elevation,
            groundCenterElevation: interpolation(ground, 0),
            longitudinalGrade: grade.grade,
            ground: ground.map(copyPoint),
            pavement,
            design,
            daylight: {
                left,
            ...
            worldDesign: design.map(([offset, z])=>[
                    center[0] - segment.tangent[1] * offset,
                    center[1] + segment.tangent[0] * offset,
                    z
                ]),
            strips,
            areas: {
                cut: strips.reduce((sum, strip)=>sum + strip.cutArea, 0),
                fill: strips.reduce((sum, strip)=>sum + strip.fillArea, 0)
            }
        };
    });
    const intervals = sections.slice(1).map((section, index)=>{
        ...
        return {
            fromStation: before.station,
            toStation: section.station,
            length,
            cutVolume: length * (before.areas.cut + section.areas.cut) / 2,
            fillVolume: length * (before.areas.fill + section.areas.fill) / 2
        };
    });
    const result = {
        units: 'meter',
        ...
        endStation,
        length: endStation - input.startStation,
        alignment,
        grades,
        sections,
        ...
        volumeRange: [
            sections[0].station,
            sections.at(-1).station
        ],
        intervals,
        totalVolume: {
            cut: intervals.reduce((sum, interval)=>sum + interval.cutVolume, 0),
            fill: intervals.reduce((sum, interval)=>sum + interval.fillVolume, 0)
        },
        limitations: [
            'This calculation does not certify supplied ground survey data.',
            'Piecewise-linear alignment and profile; no horizontal or vertical curves.',
            'Pavement is the earthwork comparison surface; no pavement thickness, topsoil, shrink or swell adjustments.',
            'Average end-area volumes cover supplied section intervals only and do not correct for curvature, overlap, or unknown terrain between sections.',
            'No road-standard compliance or construction certification is implied.'
        ]
    };
    const verifyFinite = (value)=>{
