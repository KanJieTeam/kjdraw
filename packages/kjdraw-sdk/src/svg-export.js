...
export function exportDrawingSvg(document, options) {
    ...
    const text = (entity, value, position, textHeight, angle, anchor = 'start', baseline = 'alphabetic', stretch = [
        1,
        1,
        0
    ...
    const primitive = (entity)=>{
        ...
        if (entity.type === 'TEXT' || entity.type === 'ATTRIB' || entity.type === 'ATTDEF') {
            ...
            const stretch = [
                factor * ((flags & 2) !== 0 || p.mirrored === true ? -1 : 1),
                (flags & 4) !== 0 ? -1 : 1,
                shear
            ];
            ...
            if (Array.isArray(p.patternLines) && p.patternLines.length) {
                ...
                for (const line of patternLines){
                    ...
                    if (collinear && line.dashes.length) {
                        ...
                        if (!(cycle > 0) || Math.abs(tangentShift / cycle - Math.round(tangentShift / cycle)) > 1e-9) fail('custom PAT family has no readable row spacing');
                    }
    ...
    const render = (entity, frozen, depth = 0, ancestors = [], inheritedLayer, inheritedColor, inViewport = false, geometryScale = scale, inheritedLineweight, inheritedLinetypeId)=>{
        ...
        try {
            ...
            if (p.visible === false || lp.visible === false || lp.frozen === true || lp.plottable === false && entity.type !== 'VIEWPORT' || frozen.has(layer.id)) {
                report.hidden++;
                return '';
            }
