...
export function resolveDxfPlotSource(document, layoutId, settings, isModel) {
    ...
    const center = finitePoint(payload.center, `Named view ${name} center`), target = finitePoint(payload.target ?? [
        0,
        0,
        0
    ], `Named view ${name} target`);
    const direction = finitePoint(payload.direction ?? [
        0,
        0,
        1
    ], `Named view ${name} direction`);
    const directionLength = Math.hypot(...direction), width = Number(payload.width), height = Number(payload.height);
    if (!(width > 0) || !(height > 0) || ![
        width,
        height
    ].every(Number.isFinite)) throw new KJValidationError(`Named view ${name} must have finite positive width and height`);
    ...
    if (![
        ...
        renderMode,
        ucsAssociated
    ].every(Number.isFinite)) throw new KJValidationError(`Named view ${name} has invalid projection metadata`);
