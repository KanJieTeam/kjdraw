// Generated from agent-cartesian-chart.ts by scripts/build-typescript.mjs. Do not edit directly.
import { KJValidationError } from './errors.js';
import { stableHash } from './utils.js';
export const KJDRAW_CARTESIAN_CHART_VERSION = '1.0.0';
const INPUT_KEYS = [
    'version',
    'expectedRevision',
    'units',
...
function point2(value, label) {
    ...
    return [
        boundedNumber(value[0], `${label}[0]`, -1_000_000, 1_000_000),
        boundedNumber(value[1], `${label}[1]`, -1_000_000, 1_000_000)
    ];
}
...
function formatTick(value, minimum, step) {
...
function validateInput(document, source) {
    ...
    const ids = new Set();
    const series = input.series.map((source, index)=>{
        ...
        if ([
            ...
            'GRID',
            'TEXT'
        ].includes(id.toUpperCase())) throw new KJValidationError(`input.series[${index}].id is reserved for a chart layer`);
        ...
        return {
            id,
            name: boundedString(item.name, `input.series[${index}].name`, 64),
            kind: item.kind,
            values,
            color: item.color == null ? DEFAULT_COLORS[index] : boundedInteger(item.color, `input.series[${index}].color`, 1, 255)
        };
    });
    const values = series.flatMap((item)=>item.values);
    const hasBars = series.some((item)=>item.kind === 'bar');
    let minimum, maximum, tick;
    if (input.yAxis != null) {
    ...
    return {
        ...
        origin: input.origin == null ? [
            0,
            0
        ] : point2(input.origin, 'input.origin'),
...
export function buildAgentCartesianChart(document, source) {
    ...
    const idPrefix = `chart-${stableHash({
    ...
    const seriesLayers = input.series.map((series, index)=>({
            id: `${idPrefix}-layer-series-${index + 1}`,
            name: `CHART_${series.id.toUpperCase()}`,
            color: series.color,
            linetypeId: continuousId,
            lineweight: series.kind === 'line' ? 35 : 25
        }));
    const add = (type, layerId, payload)=>{
    ...
    const line = (x1, y1, x2, y2, layerId = layers.axis.id)=>add('LINE', layerId, {
            start: p3(x1, y1),
            end: p3(x2, y2)
        });
    const text = (x, y, value, height = input.textHeight)=>add('TEXT', layers.text.id, {
            position: p3(x, y),
            text: value,
            height
        });
    const rectangle = (x, y, width, height, layerId)=>add('LWPOLYLINE', layerId, {
            vertices: [
                p3(x, y),
                p3(x + width, y),
                p3(x + width, y + height),
    ...
    const mapY = (value)=>plotY + (value - axis.minimum) / (axis.maximum - axis.minimum) * plotHeight;
    const categoryX = (index)=>plotX + plotWidth * (index + 0.5) / input.categories.length;
    ...
    return {
        commandArgs: {
            ...
            resources: {
                linetypes: [
                    {
                        id: continuousId,
                        name: `KJ_${idPrefix.slice(6, 18)}_CONT`,
                        pattern: []
                    },
                    {
                        id: gridId,
                        name: `KJ_${idPrefix.slice(6, 18)}_GRID`,
                        pattern: [
                            2,
                            -2
                        ]
        ...
        evidence: {
            ...
            limitations: [
                'Cartesian category charts only',
                'At most 8 series and 160 data values',
                'Text labels are placed deterministically without font-metric collision solving'
            ]
        }
    };
}
