...
export const KJDRAW_1_0_READINESS_PROFILE = Object.freeze({
  ...
  requiredCommands: Object.freeze([
    ...
    'NEAREST', 'ORTHO', 'SNAPSETTINGS', 'BLOCKCREATE', 'BLOCKINSERT', 'GROUP',
  ...
  requiredFormats: Object.freeze([
    ...
    { format: 'DXF', operation: 'read', versions: ['R14', '2000', '2004', '2010', '2013', '2018', '2024'], certification: 'golden-corpus' },
    { format: 'DXF', operation: 'write', versions: ['R14', '2000', '2004', '2010', '2013', '2018', '2024'], certification: 'golden-corpus' },
    { format: 'DWG', operation: 'read', versions: ['R14', '2000', '2004', '2010', '2013', '2018', '2024'], certification: 'golden-corpus' },
    { format: 'DWG', operation: 'write', versions: ['R14', '2000', '2004', '2010', '2013', '2018', '2024'], certification: 'golden-corpus' },
  ]),
...
export function auditSDKReadiness(sdk, profile = KJDRAW_1_0_READINESS_PROFILE) {
  ...
  const findings = []
  for (const id of profile.requiredCommands ?? []) if (!manifest.commandIds.includes(id)) {
    findings.push({ severity: 'error', code: 'COMMAND_MISSING', capability: id })
  }
  for (const type of profile.requiredEntityTypes ?? []) if (!manifest.entityTypes.includes(type)) {
    findings.push({ severity: 'error', code: 'ENTITY_TYPE_MISSING', capability: type })
  }
...
export function buildSDKCapabilityManifest(sdk) {
  ...
  const commands = sdk.commands.list().map(commandDescriptor).sort((a, b) => a.id.localeCompare(b.id))
...
export function auditCommandBindings(sdk, bindings = []) {
  ...
  for (const source of bindings ?? []) {
    ...
    if (!binding || !['sdk', 'host'].includes(binding.kind)) {
    ...
    if (binding.kind === 'sdk' && !sdk.commands.resolve(binding.command)) {
      findings.push({ id, code: 'sdk-command-missing', message: `${id} requires unavailable SDK command ${binding.command}` })
    }
