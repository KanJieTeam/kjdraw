...
export interface KJRoadDrawingRevisionOptions { expectedRevision: number }
...
function fail(message: string): never { throw new KJValidationError(`Road drawing update: ${message}`) }
...
function dataOnly(value: unknown): void {
  ...
  const visit = (item: unknown, depth: number): void => {
    ...
    if (typeof item === 'number') { if (!Number.isFinite(item)) fail('nonfinite input'); return }
    if (typeof item === 'string') { characters += item.length; if (characters > 32_000_000) fail('input string budget exceeded'); return }
    ...
    } else if (![Object.prototype, null].includes(Object.getPrototypeOf(item))) fail('inputs must be plain data')
    path.add(item)
    for (const key of Reflect.ownKeys(item)) {
      ...
      if (typeof key !== 'string' || ['__proto__', 'constructor', 'prototype'].includes(key) || !('value' in descriptor) || !descriptor.enumerable) fail('accessors and non-data properties are not allowed')
...
function fields(value: unknown, names: string[]): void {
...
function prepare(result: KJRoadDrawingResult, modelSpaceId: string): { drawingId: string; entities: Map<string, KJObjectRecord>; resources: Map<string, KJObjectRecord> } {
  ...
  const drawingId = match[1]!, prefix = `road:${drawingId}`, resources = new Map<string, KJObjectRecord>()
  for (const role of ['continuous', 'ground']) {
    const type = linetypes.find(value => value.id === `${prefix}:linetype:${role}`)
    if (!type) fail('linetype identity mismatch')
    fields(type, ['id', 'name', 'pattern'])
    if (type.name !== `${drawingId}_ROAD_${role.toUpperCase()}` || !equal(type.pattern, role === 'continuous' ? [] : [3, -1])) fail('linetype does not match the compiler semantic resource')
    resources.set(type.id, createObjectRecord({ id: type.id, kind: 'table-record', type: 'LINETYPE', name: type.name, payload: { description: '', pattern: type.pattern, totalPatternLength: type.pattern.reduce((sum, segment) => sum + Math.abs(segment), 0), dxfFlags: 0 } }))
  }
  const roles = ['DESIGN', 'GROUND', 'AXIS', 'ANNOTATION', 'FRAME', 'TABLE']
  for (const [index, role] of roles.entries()) {
    const layer = layers.find(value => value.id === `${prefix}:layer:${role}`)
    if (!layer) fail('layer identity mismatch')
    fields(layer, ['id', 'name', 'color', 'linetypeId', 'lineweight'])
    if (layer.name !== `${drawingId}_ROAD_${role}` || layer.color !== [3, 8, 4, 7, 7, 7][index] || layer.lineweight !== (role === 'DESIGN' ? 35 : role === 'FRAME' ? 25 : 18) || layer.linetypeId !== `${prefix}:linetype:${role === 'GROUND' ? 'ground' : 'continuous'}`) fail('layer does not match the compiler semantic resource')
    resources.set(layer.id, createObjectRecord({ id: layer.id, kind: 'table-record', type: 'LAYER', name: layer.name, payload: { color: layer.color, linetypeId: layer.linetypeId, linetypeName: resources.get(layer.linetypeId)!.name, lineweight: layer.lineweight, visible: true, frozen: false, locked: false, plottable: true } }))
  }
  ...
  for (const spec of result.entities) {
    fields(spec, ['key', 'type', 'payload', 'options']); fields(spec.options, ['id'])
    ...
    const payloadFields = spec.type === 'LINE' ? ['start', 'end', 'layerId'] : spec.type === 'LWPOLYLINE' ? ['vertices', 'closed', 'layerId'] : spec.type === 'TEXT' ? ['text', 'position', 'height', 'rotation', 'layerId'] : null
  ...
  return { drawingId, entities, resources }
}

function sameRecord(actual: unknown, expected: KJObjectRecord): boolean {
  if (!actual || typeof actual !== 'object') return false
  const record = actual as KJObjectRecord
  // Handles are allocated by the target document; every other field is compiler-owned data.
  return equal({ ...record, handle: '' }, expected)
}
...
export async function applyRoadDrawingRevision(document: KJDocument, previous: ReadonlyDeep<KJRoadDrawingResult>, next: ReadonlyDeep<KJRoadDrawingResult>, options: KJRoadDrawingRevisionOptions): Promise<ReadonlyDeep<KJRoadDrawingRevisionReceipt>> {
  ...
  await document.transact('Update road drawing', native => {
    ...
    const refersToRemoved = (value: unknown): boolean => typeof value === 'string' ? removed.has(value) : !!value && typeof value === 'object' && Object.values(value).some(refersToRemoved)
    if (removed.size) for (const object of Object.values(state.objects)) {
      ...
      if (refersToRemoved([object.ownerId, payload, object.extension, object.source])) fail(`another object refers to a removed road object: ${object.id}`)
    }
    for (const id of updatedIds) tx.updateObject(id, { payload: after.entities.get(id)!.payload })
    for (const id of removedIds) tx.eraseObject(id, { hard: true })
    for (const id of createdIds) { const spec = after.entities.get(id)!; tx.createEntity(spec.type, spec.payload, { id, ownerId: modelSpaceId }) }
    scope.validate()
  }, { expectedRevision, source: 'road-drawing-update', metadata: { drawingId: before.drawingId } })
  return deepFreeze({ documentId: document.id, drawingId: before.drawingId, previousRevision: expectedRevision, revision: expectedRevision + 1, updatedIds, createdIds, removedIds, unchangedIds })
}
