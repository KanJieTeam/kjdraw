...
function validateObjectGraph(state, issues, previousState) {
    ...
    for (const [id, object] of Object.entries(state.objects)){
        ...
        if (object?.kind === 'entity' && object.type === 'DIMENSION' && Array.isArray(object.payload?.dimensionAssociations)) {
            for (const [index, candidate] of object.payload.dimensionAssociations.entries()){
                ...
                const association = candidate, source = typeof association.entityId === 'string' ? state.objects[association.entityId] : null;
                const associationPath = `objects.${id}.payload.dimensionAssociations.${index}`;
                if (!source || source.kind !== 'entity' || source.id === id) issues.push({
                    path: `${associationPath}.entityId`,
                    message: `Dimension association source is missing or invalid: ${association.entityId}`
                });
                else {
                    if (source.ownerId !== object.ownerId) issues.push({
                        path: `${associationPath}.entityId`,
                        message: 'Dimension association source must share its drawing space'
                    });
                    const feature = String(association.feature);
                    const expected = feature === 'vertex' ? [
                        'LWPOLYLINE'
                    ] : [
                        'start',
                        'end'
                    ...
                    if (!expected.includes(source.type)) issues.push({
                        path: `${associationPath}.feature`,
                        message: `Dimension association ${feature} is incompatible with ${source.type}`
                    });
                }
            }
        }
    ...
    if (typeof pointDisplayMode !== 'number' || !Number.isInteger(pointDisplayMode) || pointDisplayMode < 0 || pointDisplayMode > 100 || (pointDisplayMode & 31) > 4 || ![
        0,
        32,
        64,
        96
    ].includes(pointDisplayMode & ~31)) issues.push({
        path: 'header.systemVariables.PDMODE',
        message: 'PDMODE must be a legal point-display mode'
    });
    const pointDisplaySize = state.header.systemVariables?.PDSIZE ?? 0;
    if (typeof pointDisplaySize !== 'number' || !Number.isFinite(pointDisplaySize) || pointDisplaySize < -100 || pointDisplaySize > 1_000_000) issues.push({
        path: 'header.systemVariables.PDSIZE',
        message: 'PDSIZE must be finite from -100 to 1000000'
    });
}
