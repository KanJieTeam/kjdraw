...
export function createEmptyDocumentState(options = {}) {
  ...
  objects[nodId].payload.entries.ACAD_LAYOUT = [modelLayoutId, paperLayoutId]
...
function validateObjectGraph(state, issues) {
  ...
  for (const [id, object] of Object.entries(state.objects)) {
    ...
    if (!KJ_OBJECT_KINDS.includes(object?.kind)) issues.push({ path: `objects.${id}.kind`, message: `Unsupported object kind: ${object?.kind}` })
    ...
    try {
      ...
      issues.push({ path: `objects.${id}.handle`, message: error.message })
    }
    ...
    if (object?.kind === 'entity' && isStandardEntityType(object.type)) {
      ...
      catch (error) { issues.push({ path: `objects.${id}.payload`, message: error.message }) }
    }
  ...
  try {
    ...
    issues.push({ path: 'header.handseed', message: error.message })
  }
}
...
function validateSpaces(state, issues) {
...
export function validateDocumentState(input, { throwOnError = true } = {}) {
...
function normalizeCurrentEntityContracts(state) {
  state.resources ??= {}
  for (const collection of ['fonts', 'images', 'hatches', 'materials', 'binaries', 'externalReferences', 'plotStyles']) state.resources[collection] ??= {}
  for (const object of Object.values(state.objects ?? {})) {
    ...
    try { object.payload = normalizeLegacyEntityPayload(object.type, object.payload) }
      ...
      object.payload = normalizeStandardEntityPayload('PROXY_ENTITY', { originalType, originalPayload, importError: error.message })
    }
  }
  return state
}
...
export function importLegacyScene(legacy = {}) {
  const state = createEmptyDocumentState({ documentId: legacy.id, title: legacy.title ?? legacy.id ?? 'Imported drawing', sourceFormat: 'KJDraw legacy scene' })
  const layerIds = new Map([['0', state.tables.layers.currentId]])
  for (const layer of legacy.layers ?? []) {
    ...
    state.objects[id] = createObjectRecord({ id, handle: allocateHandle(state), kind: 'table-record', type: 'LAYER', name, payload: { ...clone(layer), name: undefined, id: undefined } })
  ...
  for (const source of legacy.entities ?? []) {
    ...
    try { normalizedPayload = normalizeLegacyEntityPayload(type, { ...payload, layerId: layerIds.get(layerName) ?? state.tables.layers.currentId }) }
    catch (error) {
      normalizedPayload = normalizeStandardEntityPayload('PROXY_ENTITY', { originalType: type, originalPayload: payload, importError: error.message, layerId: layerIds.get(layerName) ?? state.tables.layers.currentId })
      type = 'PROXY_ENTITY'
    }
    ...
    state.objects[state.spaces.modelSpaceId].payload.entityIds.push(id)
  }
