...
function boundedDraftPoint(value, label) {
...
function normalizedDraftLine(value, label, tolerance) {
...
function pointCounts(tool, options) {
    ...
    if (tool === 'circle') {
        const count = options.circleMode === 'tangent-tangent-radius' ? 1 : options.circleMode === '3-point' ? 3 : 2;
        return {
            minimum: count,
            maximum: count
        };
