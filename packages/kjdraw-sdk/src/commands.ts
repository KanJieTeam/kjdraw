...
export interface KJEntityBatchAttributeSequence {
  attributes: { id: string; payload: KJObjectPayload }[]
  sequenceEnd: { id: string; dxfOwnerMode: 'insert' | 'space'; layerId?: string }
}
...
export const KJ_CORE_COMMAND_CAPABILITIES = deepFreeze({
  ...
  DESIGNDELETE: { domain: 'design-relations', atomic: true, preservesGeometry: true },
  HATCH: { domain: 'entity', entityType: 'HATCH', boundaryModes: ['polyline', 'line-arc-edges', 'full-ellipse-edge', 'verified-closed-spline-edge'] },
  HATCHEDIT: { domain: 'entity', entityType: 'HATCH', operations: ['update-pattern', 'add-island', 'replace-island', 'remove-island'], exactSourceTypes: ['CIRCLE', 'ELLIPSE', 'SPLINE', 'LINE', 'ARC'], openEllipseArcBoundary: false, splineBoundaryContract: 'closed-rational-quadratic-conic', stableIdentity: true },
...
export function registerCoreCommands(registry: KJCommandRegistry): () => void {
  ...
  disposers.push(registry.register({
    id: 'DESIGNDELETE', title: 'Remove design parameters and keep current geometry',
...
interface KJOwnedLeaderPairSelection {
  readonly ids: readonly string[]
  readonly pairIds: ReadonlySet<string>
  readonly pairs: readonly Readonly<{ leaderId: string; annotationId: string }>[]
}

function effectiveEntityLayerId(document: KJDocument, entity: KJReadonlyObjectRecord): string {
  return String(entity.payload.layerId ?? document.getTable('layers')?.currentId ?? '')
}

function assertOwnedLeaderPairWritable(document: KJDocument, entity: KJReadonlyObjectRecord, command: string): void {
  const layerId = effectiveEntityLayerId(document, entity)
  const layer = document.getObject(layerId)
  const protectedRecord = entity.payload.locked === true ? { reason: 'locked' }
    : entity.payload.frozen === true ? { reason: 'frozen' }
      : entity.payload.visible === false ? { reason: 'hidden' }
        : layer?.payload.locked === true ? { reason: 'locked' }
          : layer?.payload.frozen === true ? { reason: 'frozen' }
            : layer?.payload.visible === false ? { reason: 'hidden' } : null
  if (!protectedRecord) return
  throw new KJValidationError(
    `${command}: owned LEADER annotation pair contains ${protectedRecord.reason} geometry`,
    { policy: 'layer-editability', commandId: command, layerId, entityId: entity.id, reason: protectedRecord.reason },
  )
}

function leaderAnnotationPositionMatches(leader: KJReadonlyObjectRecord, annotation: KJReadonlyObjectRecord): boolean {
  ...
  const values = [Number(first[0]), Number(first[1]), Number(first[2] ?? 0), Number(second[0]), Number(second[1]), Number(second[2] ?? 0)]
  return values.every(Number.isFinite) && Math.hypot(values[0]! - values[3]!, values[1]! - values[4]!, values[2]! - values[5]!) <= 1e-9
}

function resolveOwnedLeaderPairSelection(document: KJDocument, inputIds: readonly string[], command: string): KJOwnedLeaderPairSelection {
  const ids = new Set(inputIds)
  const pairs = new Map<string, Readonly<{ leaderId: string; annotationId: string }>>()
  const leaders = document.listEntities({ type: 'LEADER' })
  const inspect = (member: KJReadonlyObjectRecord): void => {
    let candidates: KJReadonlyObjectRecord[] = []
    ...
    ids.add(leader.id); ids.add(annotation.id)
    pairs.set(leader.id, { leaderId: leader.id, annotationId: annotation.id })
  }
  for (const id of [...ids]) {
    const member = document.getObject(id)
    if (member?.kind === 'entity') inspect(member)
  }
  const pairIds = new Set([...pairs.values()].flatMap(pair => [pair.leaderId, pair.annotationId]))
  return { ids: [...ids], pairIds, pairs: [...pairs.values()] }
}
...
function eraseEntities({ document, transaction }: KJCommandContext, args: KJCommandArguments): KJObjectRecord[] {
  ...
  const impact = createEraseImpact(document, {
    expectedRevision: document.revision, units: state.header.units, operation: 'erase', ids: entityIds(args), tolerance: 1e-9, maxBytes: 1024,
  }, { maxIds: 4096, maxObjectsLimit: Math.max(1, Object.keys(state.objects).length), allowCompoundRecords: true, analyzeConnectivity: false, mode: 'decision' })
  if (!impact.canErase) {
    const blocker = impact.blockers[0]!
    if (blocker.kind === 'protected-entity') throw new KJValidationError(blocker.message, { policy: 'layer-editability', commandId: 'ERASE', entityId: blocker.sourceId, reason: blocker.reason })
...
function createBatchResources(document: KJDocument, transaction: KJTransaction, resources: KJEntityBatchResources, modelSpecs: readonly KJEntityBatchSpec[]): KJObjectRecord[] {
  ...
  for (const type of resources.linetypes) {
    ...
    // Native simple DXF linetypes store an ordered segment sequence: positive
    // values are dashes, negative values are gaps, and zero is a point. Do not
    // force an artificial positive/negative pairing that would erase point
    // segments or reject valid repeated dash/gap elements.
  ...
  const validateExplicitEntityId = (spec: KJEntityBatchSpec, label: string, required: boolean): void => {
    ...
    if (typeof id !== 'string' || !id.trim() || id !== id.trim() || id.length > 256 || /[\u0000-\u001f\u007f]/.test(id) || ['__proto__', 'constructor', 'prototype'].includes(id)) throw new KJValidationError(`${label} requires a bounded entity id`)
...
interface KJValidatedBatchLayout {
  readonly id: string
  readonly blockRecordId: string
  readonly name: string
  readonly settings: KJDxfPlotSettings & {
    paperWidth: number
    paperHeight: number
    marginLeft: number
    marginRight: number
  ...
  readonly viewport: {
    ...
    readonly height: number
    readonly viewCenter: Point3
    readonly viewHeight: number
    readonly twistAngle: number
  }
}

function validateBatchLayout(document: KJDocument, input: KJEntityBatchLayout | undefined, args: KJCommandArguments): KJValidatedBatchLayout | null {
  ...
  const layout = exact(input, ['id', 'blockRecordId', 'name', 'dxfPlotSettings', 'viewport'], 'CREATEBATCH layout')
  ...
  const settingKeys = ['paperWidth', 'paperHeight', 'marginLeft', 'marginBottom', 'marginRight', 'marginTop', 'originX', 'originY', 'scaleNumerator', 'scaleDenominator', 'flags', 'paperUnits', 'rotation', 'plotType']
  ...
  const settings = clone(settingRecord) as unknown as KJValidatedBatchLayout['settings']
  ...
  const paperValues = [settings.paperWidth, settings.paperHeight, settings.marginLeft, settings.marginRight, settings.marginTop, settings.marginBottom]
  if (!paperValues.every(value => typeof value === 'number' && Number.isFinite(value)) || !(settings.paperWidth > 0) || !(settings.paperHeight > 0)
    || settings.marginLeft < 0 || settings.marginRight < 0 || settings.marginTop < 0 || settings.marginBottom < 0
    || settings.marginLeft + settings.marginRight >= settings.paperWidth || settings.marginTop + settings.marginBottom >= settings.paperHeight) {
    throw new KJValidationError('CREATEBATCH physical layout paper and margins must define a positive printable area')
  }

  const viewportRecord = exact(layout.viewport, ['id', 'center', 'width', 'height', 'viewCenter', 'viewHeight', 'twistAngle', 'modelUnits', 'scaleDenominator'], 'CREATEBATCH layout.viewport')
  const layoutId = String(layout.id ?? ''), blockRecordId = String(layout.blockRecordId ?? ''), id = String(viewportRecord.id ?? '')
  const validId = (value: string) => Boolean(value.trim()) && value === value.trim() && value.length <= 256 && !/[\u0000-\u001f\u007f]/.test(value) && !['__proto__', 'constructor', 'prototype'].includes(value)
  if (!validId(layoutId) || !validId(blockRecordId) || !validId(id) || new Set([layoutId, blockRecordId, id]).size !== 3) throw new KJValidationError('CREATEBATCH layout and viewport require distinct bounded object IDs')
  const occupiedIds = new Set([
    ...
    ...(args.resources?.blocks ?? []).flatMap(block => [block.id, ...block.entities.map(entity => String(entity.options?.id ?? ''))]),
    ...(args.entities ?? []).map(entity => String(entity.options?.id ?? '')).filter(Boolean),
  ])
  if ([layoutId, blockRecordId, id].some(value => occupiedIds.has(value))) throw new KJValidationError('CREATEBATCH layout and viewport IDs must be globally unique')
  ...
  if (![width, height, viewHeight].every(value => Number.isFinite(value) && value > 0) || !Number.isFinite(twistAngle)) throw new KJValidationError('CREATEBATCH layout viewport dimensions and view must be finite and positive')
  ...
  if (center[0] - width / 2 < printableMinimumX - 1e-9 || center[0] + width / 2 > printableMaximumX + 1e-9
    || center[1] - height / 2 < printableMinimumY - 1e-9 || center[1] + height / 2 > printableMaximumY + 1e-9) {
  ...
  const millimetersPerUnit: Readonly<Record<string, number>> = { millimeter: 1, meter: 1_000, inch: 25.4, foot: 304.8 }
  ...
  return { id: layoutId, blockRecordId, name, settings, viewport: { id, center, width, height, viewCenter, viewHeight, twistAngle } }
}

function createBatchLayout(transaction: KJTransaction, layout: KJValidatedBatchLayout): KJObjectRecord {
  const record = transaction.createLayout({
    id: layout.id,
    blockRecordId: layout.blockRecordId,
    name: layout.name,
    paper: { width: layout.settings.paperWidth, height: layout.settings.paperHeight, unit: 'mm' },
  ...
  const viewport = transaction.createEntity('VIEWPORT', {
    ...
    frozenLayerIds: [],
  }, { id: layout.viewport.id, ownerId })
  transaction.updateObject(record.id, { payload: { viewportIds: [viewport.id] } })
  return viewport
}

interface KJValidatedBatchAttributeSequence {
  attributes: { id: string; payload: KJObjectPayload }[]
  sequenceEnd: { id: string; dxfOwnerMode: 'insert' | 'space'; layerId?: string }
}

function validateBatchAttributeSequences(document: KJDocument, args: KJCommandArguments, specs: readonly KJEntityBatchSpec[]): Map<KJEntityBatchSpec, KJValidatedBatchAttributeSequence> {
  const result = new Map<KJEntityBatchSpec, KJValidatedBatchAttributeSequence>()
  ...
  const validId = (value: unknown, label: string): string => {
    ...
    if (!id.trim() || id !== id.trim() || id.length > 256 || /[\u0000-\u001f\u007f]/u.test(id) || ['__proto__', 'constructor', 'prototype'].includes(id)) throw new KJValidationError(`${label} requires a bounded object id`)
    return id
  }
  const occupied = new Set(Object.keys(document.snapshot().objects))
  const reserve = (value: unknown, label: string): string => {
  ...
  for (const resource of [...(args.resources?.linetypes ?? []), ...(args.resources?.layers ?? []), ...(args.resources?.textStyles ?? []), ...(args.resources?.dimensionStyles ?? [])]) reserve(resource.id, 'CREATEBATCH resource id')
  ...
  if (args.layout) {
    reserve(args.layout.id, 'CREATEBATCH layout id'); reserve(args.layout.blockRecordId, 'CREATEBATCH layout block id'); reserve(args.layout.viewport.id, 'CREATEBATCH viewport id')
  }
  const layers = new Set([...document.getTable('layers')!.records.filter(item => !item.erased).map(item => item.id), ...(args.resources?.layers ?? []).map(item => item.id)])
  const linetypes = new Set([...document.getTable('linetypes')!.records.filter(item => !item.erased).map(item => item.id), ...(args.resources?.linetypes ?? []).map(item => item.id)])
  const textStyles = new Set([...document.getTable('textStyles')!.records.filter(item => !item.erased).map(item => item.id), ...(args.resources?.textStyles ?? []).map(item => item.id)])
  for (const [index, spec] of specs.entries()) {
    ...
    const suppliedIds = spec.payload?.attributeIds as unknown[] | undefined
    ...
    const sequence = exact(spec.attributeSequence, ['attributes', 'sequenceEnd'], label)
    ...
    const attributes = sequence.attributes.map((value, attributeIndex) => {
      const attributeLabel = `${label}.attributes[${attributeIndex}]`, attribute = exact(value, ['id', 'payload'], attributeLabel)
      const id = reserve(attribute.id, `${attributeLabel}.id`), payload = plain(attribute.payload, `${attributeLabel}.payload`) as KJObjectPayload
      ...
      return { id, payload: clone(payload) }
    })
    const end = plain(sequence.sequenceEnd, `${label}.sequenceEnd`), endKeys = Object.keys(end)
    if (!Object.hasOwn(end, 'id') || !Object.hasOwn(end, 'dxfOwnerMode') || endKeys.some(key => !['id', 'dxfOwnerMode', 'layerId'].includes(key))) throw new KJValidationError(`${label}.sequenceEnd fields do not match the declared format`)
    if (!['insert', 'space'].includes(String(end.dxfOwnerMode))) throw new KJValidationError(`${label}.sequenceEnd.dxfOwnerMode must be insert or space`)
    ...
    result.set(spec, { attributes, sequenceEnd: { id: reserve(end.id, `${label}.sequenceEnd.id`), dxfOwnerMode: end.dxfOwnerMode as 'insert' | 'space', ...(end.layerId == null ? {} : { layerId: String(end.layerId) }) } })
  }
  return result
}

function validateBatchPointDisplay(value: unknown): { PDMODE?: number; PDSIZE?: number } | undefined {
  if (value === undefined) return undefined
  if (!value || typeof value !== 'object' || Array.isArray(value) || ![Object.prototype, null].includes(Object.getPrototypeOf(value))) throw new KJValidationError('CREATEBATCH systemVariables must be a plain object')
  const source = value as Record<string, unknown>, keys = Object.keys(source)
  if (!keys.length || keys.some(key => !['PDMODE', 'PDSIZE'].includes(key))) throw new KJValidationError('CREATEBATCH systemVariables accept only PDMODE and PDSIZE')
  const result: { PDMODE?: number; PDSIZE?: number } = {}
  if (Object.hasOwn(source, 'PDMODE')) {
    const mode = source.PDMODE
    if (typeof mode !== 'number' || !Number.isInteger(mode) || mode < 0 || mode > 100 || (mode & 31) > 4 || ![0, 32, 64, 96].includes(mode & ~31)) throw new KJValidationError('CREATEBATCH PDMODE must be a legal point-display mode')
    result.PDMODE = mode
  }
...
function createEntityBatch({ document, transaction }: KJCommandContext, args: KJCommandArguments = {}): KJObjectRecord[] {
  ...
  if (Object.hasOwn(args, 'resources')) {
    ...
    for (const spec of specs) {
      ...
      if (normalizeName(String(spec?.type ?? '')) === 'INSERT') {
        ...
        if ((spec?.payload?.attributeIds as unknown[] | undefined)?.length || spec?.payload?.sequenceEndId != null || spec?.payload?.parentInsertId != null) throw new KJValidationError('CREATEBATCH INSERT cannot supply attached entity relationships')
      }
    }
  }
  ...
  for (const spec of specs) {
    ...
    if (!sequence) { created.push(entity); continue }
    const attributes = sequence.attributes.map(attribute => transaction.createEntity('ATTRIB', { ...attribute.payload, parentInsertId: entity.id }, { id: attribute.id, ownerId: entity.ownerId }))
    const sequenceLayerId = sequence.sequenceEnd.layerId ?? entity.payload.layerId
    const sequenceEnd = transaction.createObject({ id: sequence.sequenceEnd.id, kind: 'custom', type: 'SEQEND', ownerId: entity.id,
      payload: { dxfOwnerMode: sequence.sequenceEnd.dxfOwnerMode, ...(sequenceLayerId == null ? {} : { layerId: sequenceLayerId }) } })
    const updated = transaction.updateObject(entity.id, { payload: { attributeIds: attributes.map(attribute => attribute.id), sequenceEndId: sequenceEnd.id } })
...
function copyEntities({ document, transaction }: KJCommandContext, args: KJCommandArguments, matrix: AffineMatrix3Input, command: string): KJObjectRecord[] {
  ...
  const leaderSelection = command === 'COPY'
    ? resolveOwnedLeaderPairSelection(document, requestedIds, command)
    : { ids: requestedIds, pairIds: new Set<string>(), pairs: [] }
  ...
  const copies = sourceIds.map((id, sourceIndex) => {
    ...
    const copied = transaction.createEntity(entity.type, { ...transformEntityPayload(entity.type, entity.payload, matrix), ...clone(args.payloadPatch ?? {}), ...(attributed ? { attributeIds: [], sequenceEndId: null } : {}) }, {
      ...(resultIds == null ? {} : { id: resultIds[sourceIndex] as string }),
  ...
  const retargeted = copies.map((copy, index) => {
    ...
    if (source.type === 'LEADER' && leaderSelection.pairIds.has(source.id)) {
      ...
      return transaction.updateObject(copy.id, { payload: { annotationId } })
    }
  ...
  if (leaderSelection.pairIds.size) appendCopiedMemberships(transaction, new Map([...replacements].filter(([id]) => leaderSelection.pairIds.has(id))))
...
function appendCopiedMemberships(transaction: KJTransaction, replacements: ReadonlyMap<string, string>): void {
  for (const group of Object.values(transaction._draft().objects)) {
    if (group.erased || group.kind !== 'group' || !['GROUP', 'SELECTION_SET'].includes(group.type)) continue
    const members = group.payload.memberIds
    if (!Array.isArray(members) || !members.some(id => replacements.has(id))) continue
    const memberIds = [...new Set(members.flatMap(id => replacements.has(id) ? [id, replacements.get(id)!] : [id]))]
    transaction.updateObject(group.id, { payload: { memberIds } })
  }
}
