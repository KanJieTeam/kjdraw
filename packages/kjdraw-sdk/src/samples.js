...
const sampleCatalog = [
    {
        id: 'sample-site-plan',
        title: 'Riverside research park / site plan',
        titleZh: '滨河研发园 / 场地总图',
        discipline: 'CIVIL',
        build: buildSitePlan
    },
    ...
    {
        id: 'sample-road-profile',
        title: 'Hill route C2 / longitudinal profile',
        titleZh: '山区道路 C2 / 纵断面',
        discipline: 'TRANSPORTATION',
        build: buildRoadProfile
    },
    {
        id: 'sample-mechanical',
        title: 'Bearing bracket / manufacturing drawing',
        titleZh: '轴承支架 / 制造工程图',
...
function draftingKit(entities) {
    ...
    const circle = (center, radius, layer)=>add('CIRCLE', {
            center,
            radius
        }, layer);
    const arc = (center, radius, startAngle, endAngle, layer)=>add('ARC', {
            center,
            radius,
            startAngle,
            endAngle
        }, layer);
    const text = (position, value, height, layer = 'ANNO', rotation = 0)=>add('TEXT', {
            position,
            text: value,
            height,
            rotation
        }, layer);
    const poly = (points, layer, closed = true)=>add('LWPOLYLINE', {
            vertices: points.map((point)=>({
                    point
                })),
            closed
        }, layer);
    const rect = (x, y, width, height, layer)=>poly([
            [
                x,
                y
            ],
    ...
    const arrow = (at, direction, size, layer)=>{
        const angle = Math.atan2(direction[1], direction[0]), wing = Math.PI * .82;
        line(at, [
            at[0] + Math.cos(angle + wing) * size,
            at[1] + Math.sin(angle + wing) * size
        ], layer);
        line(at, [
            at[0] + Math.cos(angle - wing) * size,
            at[1] + Math.sin(angle - wing) * size
    ...
    const dimH = (x1, x2, y, offset, value, layer = 'DIMS')=>{
        ...
        text([
            (x1 + x2) / 2 - String(value).length * .65,
            dimensionY + dy * 1.1
        ], String(value), 1.5, layer);
    };
    const dimV = (x, y1, y2, offset, value, layer = 'DIMS')=>{
        ...
        text([
            dimensionX + dx * 1.1,
            (y1 + y2) / 2 - String(value).length * .65
        ], String(value), 1.5, layer, Math.PI / 2);
    };
    const sheet = ({ x = -8, y = -12, width = 256, height = 164, title, number, scale, discipline })=>{
        rect(x, y, width, height, 'FRAME');
        rect(x + 3, y + 3, width - 6, height - 6, 'FRAME');
        const blockX = x + width - 112, blockY = y + 3;
        rect(blockX, blockY, 109, 18, 'FRAME');
        line([
            blockX + 58,
            blockY
        ], [
        ...
        text([
            blockX + 3,
            blockY + 5
        ], discipline, 1.35, 'TITLE');
        text([
            blockX + 61,
            blockY + 12.5
        ], `SCALE  ${scale}`, 1.15, 'TITLE');
        text([
            blockX + 61,
            blockY + 4.5
        ], 'REV  A', 1.15, 'TITLE');
...
async function createDrawing(sdk, sample) {
    ...
    const document = sdk.createDocument({
        ...
        tags: [
            'sample',
            sample.discipline.toLowerCase()
        ],
        metadata: {
            synthetic: true,
            sampleId: sample.id,
            discipline: sample.discipline,
            titleZh: sample.titleZh
        }
    });
    const entities = [];
    const kit = draftingKit(entities);
    const layers = sample.layers ?? [];
    for (const [name, color] of layers)await sdk.executeCommand('LAYERNEW', {
        name,
        color
    }, {
        document
...
export async function createIndustrySample(sdk, id) {
...
export async function createIndustrySamples(sdk) {
    const drawings = [];
...
function buildSitePlan({ line, circle, text, poly, rect, cross, dimH, dimV, sheet }) {
    sheet({
        title: 'RIVERSIDE RESEARCH PARK',
        number: 'C-101',
        scale: '1:500',
        discipline: 'SITE DEVELOPMENT PLAN'
    });
    poly([
        [
            4,
    ...
    for(let i = 0; i < 17; i++){
        const y = 25 + i * 6.2;
        const points = [];
        for(let j = 0; j <= 20; j++)points.push([
            8 + j * 11,
            y + Math.sin(i * .38 + j * .48) * 2.3
        ]);
        poly(points, 'C-TOPO', false);
        if (i % 4 === 0) text([
            13,
            y + 2.8
        ], `${108 + i}.00`, 1.1, 'C-TOPO');
    ...
    rect(38, 68, 58, 35, 'A-BUILD');
    rect(40, 70, 54, 31, 'A-BUILD');
    text([
        52,
        86
    ], 'LAB 01', 3, 'ANNO');
    rect(113, 72, 72, 43, 'A-BUILD');
    rect(115, 74, 68, 39, 'A-BUILD');
    text([
        133,
        94
    ], 'LAB 02', 3, 'ANNO');
    rect(193, 75, 27, 25, 'A-BUILD');
    text([
        198,
        87
    ], 'UTILITY', 1.8, 'ANNO');
    ...
    text([
        18,
        61
    ], 'DN200 WATER', 1.25, 'C-UTIL');
    for(let i = 0; i < 38; i++){
        const x = 20 + i % 19 * 11, y = i < 19 ? 119 : 127;
        circle([
            x,
            y
        ], 1.5, 'L-PLANT');
    ...
    dimH(38, 96, 68, -7, '58.00 m');
    dimH(113, 185, 72, -7, '72.00 m');
    dimV(185, 72, 115, 7, '43.00 m');
    line([
        229,
        112
    ], [
    ...
    text([
        9,
        146
    ], 'GENERAL NOTES  ·  ALL DIMENSIONS IN METERS  ·  COORDINATE DATUM: LOCAL DEMO GRID', 1.15, 'TITLE');
}
...
function buildArchitecture({ line, circle, arc, text, poly, rect, dimH, dimV, sheet }) {
    sheet({
        title: 'INNOVATION HUB · LEVEL 01',
        number: 'A-101',
        scale: '1:100',
        discipline: 'ARCHITECTURAL FLOOR PLAN'
    });
    ...
    for(let i = 0; i < 4; i++){
        ...
        text([
            x0 - 11.7,
            y - .7
        ], String.fromCharCode(65 + i), 1.5, 'A-GRID');
    }
    rect(x0, y0, x1 - x0, y1 - y0, 'A-WALL');
    rect(x0 + 1.8, y0 + 1.8, x1 - x0 - 3.6, y1 - y0 - 3.6, 'A-WALL');
    for (const x of [
        55,
        88,
        121,
    ...
    const doorOpenings = bayStarts.map((bayStart)=>[
            bayStart + 6,
            bayStart + 14
        ]);
    const horizontalWallWithOpenings = (y)=>{
        let cursor = x0 + 2;
    ...
    horizontalWallWithOpenings(67);
    horizontalWallWithOpenings(98);
    for (const x of [
        22,
        55,
        88,
    ...
    for(let i = 0; i < bayStarts.length; i++){
        ...
        text([
            bayStart + 15,
            60.5
        ], lowerRooms[i], 1.35, 'ANNO');
        text([
            bayStart + 10,
            93.5
        ], middleRooms[i], 1.25, 'ANNO');
        text([
            bayStart + 12,
            124
        ], upperRooms[i], 1.25, 'ANNO');
        for (const deskX of [
            bayStart + 5,
            bayStart + 18
        ])for (const deskY of [
    ...
    dimH(x0, x1, y0, -10, '19800');
    for(let i = 0; i < 6; i++)dimH(x0 + i * 33, x0 + (i + 1) * 33, y1, 9, '3300');
    dimV(x0, y0, y1, -10, '9400');
    text([
        9,
        146
    ], 'GROUND FLOOR · MODEL UNITS: MILLIMETERS · ORIGINAL SAMPLE DRAWING', 1.15, 'TITLE');
}
...
function buildRoadProfile({ line, circle, text, poly, rect, dimH, sheet }) {
    sheet({
        title: 'HILL ROUTE C2 · LONGITUDINAL PROFILE',
        number: 'C-301',
        scale: 'H 1:1000 / V 1:100',
        discipline: 'ROAD ENGINEERING'
    });
    ...
    for(let i = 0; i <= 50; i++){
        const x = left + i * (right - left) / 50;
        existing.push([
            x,
            64 + Math.sin(i * .38) * 9 + Math.sin(i * .13) * 7 + i * .42
        ]);
        grade.push([
            x,
            68 + i * .37 + Math.sin(i * .11) * 2.2
        ]);
    }
    poly(existing, 'C-GROUND', false);
    poly(grade, 'C-GRADE', false);
    for(let i = 0; i < grade.length; i += 5){
        const point = grade[i];
        line([
            point[0],
            bottom
        ], point, 'C-GRADE');
        circle(point, 1.1, 'C-GRADE');
        text([
            point[0] - 2.5,
            point[1] + 3
        ], `PVI ${i * 20}`, 1.05, 'ANNO');
    }
    for (const [station, y, label] of [
        [
            73,
            70,
            'BOX CULVERT 2.0 × 1.5'
        ...
        rect(station - 3, y - 3, 6, 3, 'C-STRUCT');
        text([
            station + 3,
            y - 1
        ], label, 1.15, 'C-STRUCT');
    ...
    text([
        20,
        55
    ], 'DRAINAGE INVERT', 1.1, 'C-DRAIN');
    dimH(left, right, bottom, -11, 'CHAINAGE 0+000 — 1+000');
    text([
        20,
        125
    ], 'EXISTING GROUND', 1.4, 'C-GROUND');
    line([
        55,
        126
    ], [
    ...
    text([
        9,
        146
    ], 'DATUM 90.00 m  ·  DESIGN SPEED 60 km/h  ·  ALL LEVELS ARE SYNTHETIC', 1.15, 'TITLE');
}
...
function buildMechanical({ line, circle, arc, text, poly, rect, cross, dimH, dimV, sheet }) {
    sheet({
        title: 'BEARING BRACKET · MACHINING',
        number: 'M-2047',
        scale: '1:1',
    ...
    for (const x of [
        ...
        cross(x, 55, 7, 'M-CENTER');
    }
    cross(69, 88, 25, 'M-CENTER');
    line([
        51,
        88
    ], [
    ...
    rect(139, 47, 70, 62, 'M-OBJECT');
    rect(154, 62, 40, 31, 'M-OBJECT');
    circle([
        174,
        77.5
    ], 10, 'M-OBJECT');
    cross(174, 77.5, 34, 'M-CENTER');
    for(let x = 141; x < 208; x += 5){
        line([
            x,
            49
        ], [
    ...
    arc([
        190,
        128
    ], 6, 0, Math.PI * 1.5, 'M-OBJECT');
    cross(190, 128, 13, 'M-CENTER');
    text([
        204,
        132
    ], 'DETAIL B', 1.5, 'ANNO');
    ...
    text([
        38,
        25
    ], 'Ra 3.2 UNLESS NOTED', 1.15, 'TITLE');
    text([
        80,
        34
    ], 'TOLERANCE', 1, 'TITLE');
...
function buildBoreholeLog({ line, text, rect, poly, dimV, sheet }) {
    sheet({
        title: 'LOESS BOREHOLE LOG · ZK01',
        number: 'G-101',
        scale: 'V 1:200',
    ...
    let cursor = top;
    for (const [name, metres] of units){
        const height = metres * depthStep;
        rect(left + 43, cursor - height, 21, height, 'GEO-BOUNDARY');
        for(let x = left + 45; x < left + 63; x += 5)for(let y = cursor - 3; y > cursor - height + 2; y -= 5)line([
            x,
            y
        ], [
            x + 3,
    ...
    for(let depth = 0; depth <= 30; depth += 3){
        const y = top - depth * depthStep;
        line([
            left - 3,
            y
        ], [
        ...
        text([
            left - 12,
            y - .5
        ], String(depth), 1.15, 'GEO-DEPTH');
        text([
            right + 7,
            y - .5
        ], (300 - depth).toFixed(2), 1.1, 'GEO-ELEV');
    }
    text([
        left + 2,
        top + 7
    ...
    text([
        left + 46,
        top + 7
    ], 'LITHOLOGY', 1.25, 'TITLE');
    for(let i = 1; i <= 9; i++)text([
        left + 2,
        top - i * 3.33 - 1
    ], String(i), 1, 'ANNO');
    line([
    ...
    text([
        left + 66,
        33
    ], 'GWL 18.40 m', 1.2, 'GEO-WATER');
    dimV(left + 73, top, bottom, 6, '30.00 m');
    text([
        left,
        21
