...
import { computeRoadDesign, type KJRoadDesignInput } from './road-design.js'
...
export const KJDRAW_ROAD_INPUT_ASSET_SCHEMA = 'com.kanjie.kjdraw.road-design-input@1' as const
export interface KJAgentInputAssetRegistration {
  assetId: string
  schema: typeof KJDRAW_ROAD_INPUT_ASSET_SCHEMA
  data: KJRoadDesignInput
}
export interface KJAgentInputAssetReference { assetId: string; sha256: string }
export interface KJAgentInputAssetDescriptor extends KJAgentInputAssetReference {
  ...
  units: 'meter'
  byteLength: number
  counts: { alignment: number; profile: number; sections: number; groundPoints: number }
...
function fail(message: string): never { throw new KJValidationError('Input asset: ' + message) }

// Snapshot before validation or hashing; no accessor, toJSON or conversion hook is
// invoked. These are data-only assets, not a path resolver or executable skills.
function snapshot(input: unknown): unknown {
  let nodes = 0, characters = 0
  const active = new Set<object>()
  const visit = (value: unknown, depth: number): unknown => {
    if (++nodes > 30000 || depth > 12) fail('structural budget exceeded')
    if (typeof value === 'string') { if ((characters += value.length) > 1048576) fail('text budget exceeded'); return value }
    ...
    for (const key of keys) {
      ...
      if (typeof key !== 'string' || ['__proto__', 'prototype', 'constructor'].includes(key)) fail('unsafe property')
...
export async function createAgentInputAsset(input: unknown): Promise<ReadonlyDeep<KJAgentInputAsset>> {
  ...
  if (Object.keys(item).length !== 3 || !['assetId', 'schema', 'data'].every(key => Object.hasOwn(item, key))) fail('registration requires exactly assetId, schema and data')
  ...
  // The domain/schema is part of the digest; names do not change data identity.
  const canonical = canonicalizeAgentPlanBinding({ schema: item.schema, data: item.data })
  const bytes = new TextEncoder().encode(canonical)
  if (bytes.byteLength > 1048576) fail('data exceeds the 1 MiB asset budget')
  const data = (JSON.parse(canonical) as { data: KJRoadDesignInput }).data
  if (!data || typeof data !== 'object' || Array.isArray(data)) fail('road data must be an object')
  const bounded = (value: unknown, maximum: number): value is unknown[] => Array.isArray(value) && value.length >= 2 && value.length <= maximum
  ...
  // Reuse the deterministic native validator, including exact station coverage,
  // ordered offsets and unique daylight intersections. Do not infer missing data.
  ...
  return deepFreeze({ descriptor: {
    assetId: item.assetId, sha256, schema: KJDRAW_ROAD_INPUT_ASSET_SCHEMA,
    canonicalization: KJ_AGENT_PLAN_BINDING_CANONICALIZATION, units: 'meter',
    byteLength: bytes.byteLength,
    counts: { alignment: data.alignment.length, profile: data.profile.length, sections: data.sections.length, groundPoints },
    stationRange: [calculation.startStation, calculation.endStation] as const,
  }, data })
}
