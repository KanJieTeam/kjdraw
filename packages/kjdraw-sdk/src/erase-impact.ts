...
export interface KJEraseImpactOptions { maxIds?: number; maxBytesLimit?: number; maxObjectsLimit?: number; allowCompoundRecords?: boolean; analyzeConnectivity?: boolean; mode?: 'diagnostic' | 'decision' }
type Point3 = [number, number, number]
type Endpoint = { entityId: string; ownerId: string; point: Point3 }

export const KJDRAW_ERASE_IMPACT_LIMITS = Object.freeze({ maxObjects: 200000, maxReferences: 500000, maxConnectivityFeatures: 32768, maxConnectivityComparisons: 500000 })
...
function endpointGeometry(entity: KJReadonlyObjectRecord): Endpoint[] {
  const payload = entity.payload, add = (value: unknown): Endpoint | null => {
    const point = point3(value); return point ? { entityId: entity.id, ownerId: String(entity.ownerId), point } : null
  }
  if (entity.type === 'LINE') return [add(payload.start), add(payload.end)].filter((item): item is Endpoint => item !== null)
  if (entity.type === 'ARC') {
    ...
    return [start, end].map(angle => ({ entityId: entity.id, ownerId: String(entity.ownerId), point: [center[0] + radius * Math.cos(angle), center[1] + radius * Math.sin(angle), center[2]] }))
  }
  if (entity.type === 'ELLIPSE') {
    ...
    return [start, end].map(parameter => ({ entityId: entity.id, ownerId: String(entity.ownerId), point: [center[0] + axis[0] * Math.cos(parameter) - axis[1] * ratio * Math.sin(parameter), center[1] + axis[1] * Math.cos(parameter) + axis[0] * ratio * Math.sin(parameter), center[2]] }))
  }
  if (entity.type === 'LWPOLYLINE' || entity.type === 'POLYLINE') {
    if (!Array.isArray(payload.vertices) || payload.vertices.length < 2 || payload.vertices.length > 4096) return []
    const points = payload.vertices.map(vertex => point3(vertex && typeof vertex === 'object' && !Array.isArray(vertex) ? (vertex as { point?: unknown }).point : vertex))
    if (points.some(point => point === null)) return []
    return points.map(point => ({ entityId: entity.id, ownerId: String(entity.ownerId), point: point! }))
  }
  return []
}

function connectivityImpact(objects: readonly KJReadonlyObjectRecord[], erased: ReadonlySet<string>, tolerance: number): Readonly<Record<string, unknown>> {
  ...
  const entityIds = [...new Set(endpoints.map(endpoint => endpoint.entityId))].sort(), adjacency = new Map(entityIds.map(id => [id, new Set<string>()]))
  ...
  while (ownerStart < endpoints.length) {
    ...
    for (let left = ownerStart; left < ownerEnd; left++) for (let right = left + 1; right < ownerEnd && endpoints[right]!.point[0] - endpoints[left]!.point[0] <= tolerance; right++) {
      ...
      if ((a[1] - b[1]) ** 2 + (a[2] - b[2]) ** 2 + (a[0] - b[0]) ** 2 <= tolerance ** 2) {
        ...
        if (first !== second) { adjacency.get(first)!.add(second); adjacency.get(second)!.add(first) }
      }
    }
    ownerStart = ownerEnd
  }
  const visited = new Set<string>(), beforeComponents: string[][] = []
  for (const id of entityIds) {
    if (visited.has(id)) continue
    const queue = [id], component: string[] = []; visited.add(id)
    while (queue.length) { const current = queue.shift()!; component.push(current); for (const next of adjacency.get(current)!) if (!visited.has(next)) { visited.add(next); queue.push(next) } }
    component.sort(); if (component.some(member => erased.has(member))) beforeComponents.push(component)
  }
  const afterComponents: string[][] = [], disconnectCandidates: Record<string, unknown>[] = []
  for (const before of beforeComponents) {
    const retained = new Set(before.filter(id => !erased.has(id))), fragments: string[][] = []
    while (retained.size) {
      const seed = [...retained].sort()[0]!, queue = [seed], fragment: string[] = []; retained.delete(seed)
      while (queue.length) { const current = queue.shift()!; fragment.push(current); for (const next of adjacency.get(current) ?? []) if (retained.has(next)) { retained.delete(next); queue.push(next) } }
      fragment.sort(); fragments.push(fragment); afterComponents.push(fragment)
    }
    fragments.sort((a, b) => compareText(a[0] ?? '', b[0] ?? ''))
    if (fragments.length > 1) disconnectCandidates.push({ condition: 'selected-entities-bridge-retained-endpoint-connectivity', beforeEntityIds: before, removedEntityIds: before.filter(id => erased.has(id)), afterComponents: fragments, requiresCapabilityConfirmation: true })
  }
  afterComponents.sort((a, b) => compareText(a[0] ?? '', b[0] ?? ''))
  return { semanticInference: 'none', before: { affectedComponents: beforeComponents }, after: { retainedComponents: afterComponents }, disconnectCandidates, comparisons, omittedNativeTypes: ['HATCH', 'INSERT'] }
}

function sourceHandles(hatch: KJReadonlyObjectRecord, count: (amount?: number) => void): { handle: string; reference: 'raw-97-330' | 'boundary-loop-sourceHandles' }[] {
  const found = new Map<string, 'raw-97-330' | 'boundary-loop-sourceHandles'>(), tags = hatch.payload.rawTags
  if (Array.isArray(tags)) for (let index = 0; index < tags.length; index++) {
    count()
    const tag = tags[index]
    if (!tag || typeof tag !== 'object' || Array.isArray(tag) || Number((tag as { code?: unknown }).code) !== 97) continue
    const nativeCount = Number((tag as { value?: unknown }).value)
    const next = tags[index + 1]
    // HATCH spline edges also use code 97 for fit-point count. A native source
    // list is distinguishable because its first following tag is code 330.
    ...
    for (let offset = 1; offset <= nativeCount; offset++) {
      ...
      if (!source || typeof source !== 'object' || Array.isArray(source) || Number((source as { code?: unknown }).code) !== 330) fail(`HATCH ${hatch.id} has an incomplete native 97/330 source list`)
  ...
  if (Array.isArray(hatch.payload.boundaryLoops)) for (const loop of hatch.payload.boundaryLoops) {
    count()
    const handles = loop && typeof loop === 'object' && !Array.isArray(loop) ? (loop as { sourceHandles?: unknown }).sourceHandles : undefined
    if (!Array.isArray(handles)) continue
    for (const value of handles) { count(); const handle = String(value ?? '').trim().toUpperCase(); if (handle && !found.has(handle)) found.set(handle, 'boundary-loop-sourceHandles') }
  }
  return [...found].sort(([a], [b]) => compareText(a, b)).map(([handle, reference]) => ({ handle, reference }))
}

function protectedReason(document: KJDocument, entity: KJReadonlyObjectRecord): 'locked' | 'frozen' | 'hidden' | null {
...
function matchingLeaderPosition(leader: KJReadonlyObjectRecord, annotation: KJReadonlyObjectRecord): boolean {
...
export function createEraseImpact(document: KJDocument, query: KJEraseImpactQuery, options: KJEraseImpactOptions = {}): Readonly<KJEraseImpact> {
  if (!query || typeof query !== 'object' || Array.isArray(query)) fail('Erase impact query must be an object')
  const allowed = ['expectedRevision', 'units', 'operation', 'ids', 'tolerance', 'maxBytes']
  ...
  let references = 0
  const count = (amount = 1): void => { references += amount; if (references > KJDRAW_ERASE_IMPACT_LIMITS.maxReferences) fail(`Erase impact exceeds ${KJDRAW_ERASE_IMPACT_LIMITS.maxReferences} references`) }
  const requestedIds = [...query.ids].sort(), requested = requestedIds.map(id => state.objects[id]).map((object, index) => {
  ...
  const blockers: KJEraseImpactBlocker[] = [], blockerKeys = new Set<string>()
  ...
  for (const object of objects) {
    ...
    if (object.kind === 'entity' && object.type === 'LEADER' && object.payload.annotationId != null) {
      ...
      values.push(object.id); attributesByInsert.set(insertId, values); count()
      ...
      values.push(object.id); sequenceEndsByInsert.set(insertId, values); count()
    }
    if (object.kind === 'entity' && object.type === 'INSERT' && object.payload.blockRecordId != null) {
      ...
      values.push(object.id); instancesByDefinition.set(definitionId, values); count()
    }
  }
  ...
  for (const member of requested) {
    ...
    if (!valid) {
      block({ kind: 'owned-leader-annotation', sourceId: member.id, dependentId: annotationId || null, message: 'ERASE rejects a broken or ambiguous owned LEADER + MTEXT annotation pair' }); continue
    }
    rootIds.add(leader.id); rootIds.add(annotation.id); effectiveIds.add(leader.id); effectiveIds.add(annotation.id)
    leaderPairs.push({ leaderId: leader.id, annotationId: annotation.id, condition: 'owned-native-annotation-erased-atomically', resolvedBySameErase: true })
  }
  for (const id of rootIds) {
    const entity = state.objects[id]
    if (!entity || entity.kind !== 'entity') continue
    const reason = protectedReason(document, entity)
    if (reason) block({ kind: 'protected-entity', sourceId: id, dependentId: null, reason, message: `ERASE cannot modify ${reason} entity ${id}` })
  }
  const insertAttachments: Record<string, unknown>[] = []
  for (const id of [...rootIds]) {
    const entity = state.objects[id]!
    if ((entity.type === 'ATTRIB' && entity.payload.parentInsertId) || entity.type === 'SEQEND') {
      ...
      if (!rootIds.has(parentId)) block({ kind: 'attached-insert-record', sourceId: id, dependentId: parentId, message: `ERASE must include parent INSERT ${parentId} when erasing attached ATTRIB ${id}` })
    }
    ...
    if (!complete) block({ kind: 'attached-insert-record', sourceId: entity.id, dependentId: null, message: `ERASE cannot safely resolve attached ATTRIB/SEQEND records for INSERT ${entity.id}` })
    for (const childId of attributeIds) effectiveIds.add(childId)
    if (sequenceEndId) effectiveIds.add(sequenceEndId)
    insertAttachments.push({ insertId: entity.id, attributeIds, sequenceEndId, complete, condition: 'attached-records-erased-with-parent-insert', expandedGeometry: false, resolvedBySameErase: complete })
  }
  for (const id of [...rootIds]) {
  ...
  const designRelations: Record<string, unknown>[] = []
  let designViews: Map<string, ReturnType<typeof readDesignRelations>[number]> | null = null
  try { designViews = new Map(readDesignRelations(document).map(view => [view.id, view])) } catch { designViews = null }
  for (const relation of designRecords) {
    let entityIds: string[] = designViews?.get(relation.id)?.entityIds ?? [], valid = designViews?.has(relation.id) === true
    if (!valid) {
      ...
      entityIds = Array.isArray(bindings) ? bindings.map(binding => String(binding && typeof binding === 'object' && !Array.isArray(binding) ? (binding as { entityId?: unknown }).entityId ?? '' : '')).filter(Boolean) : []
    }
    count(entityIds.length)
    const affected = [...new Set(entityIds.filter(id => effectiveIds.has(id)))].sort()
    if (!affected.length) continue
    designRelations.push({ id: relation.id, name: relation.name, boundEntityIds: [...new Set(entityIds)].sort(), affectedEntityIds: affected, valid, condition: valid ? 'would-dangle-design-binding' : 'cannot-verify-design-binding', requiresCapabilityConfirmation: true, resolvedBySameErase: false })
    block({ kind: 'design-relation', sourceId: affected[0]!, dependentId: relation.id, message: `ERASE must remove design relation ${relation.id} before erasing one of its bound entities` })
  }
  const dimensions: Record<string, unknown>[] = []
  for (const dimension of dimensionsByAssociation) {
    const raw = dimension.payload.dimensionAssociations
    if (!Array.isArray(raw)) continue
    let associations: { entityId: string }[] = [], valid = true
    try { associations = normalizeDimensionAssociations(raw) } catch { valid = false; associations = raw.map(value => ({ entityId: String(value && typeof value === 'object' && !Array.isArray(value) ? (value as { entityId?: unknown }).entityId ?? '' : '') })).filter(value => value.entityId) }
    count(associations.length)
    const affected = [...new Set(associations.map(value => value.entityId).filter(id => effectiveIds.has(id)))].sort()
    if (!affected.length) continue
    const resolved = effectiveIds.has(dimension.id)
    dimensions.push({ id: dimension.id, affectedSourceIds: affected, associationCount: associations.length, valid, condition: valid ? 'would-dangle-dimension-association' : 'cannot-verify-dimension-association', requiresCapabilityConfirmation: true, resolvedBySameErase: resolved })
    if (!resolved) block({ kind: 'dimension-association', sourceId: affected[0]!, dependentId: dimension.id, message: `ERASE must include dimension ${dimension.id} when erasing one of its referenced sources` })
  }
  const hatchSourceReferences: Record<string, unknown>[] = []
  ...
  for (const id of effectiveIds) {
    ...
    const members = definitionMembers.get(owner.id) ?? []; members.push(id); definitionMembers.set(owner.id, members)
  }
  for (const [definitionId, memberIds] of [...definitionMembers].sort(([a], [b]) => compareText(a, b))) {
    if (!diagnostic) continue
    const definition = state.objects[definitionId]!, instanceIds = instancesByDefinition.get(definitionId) ?? []
    blockDefinitionInstances.push({ blockRecordId: definitionId, definitionName: definition.name, affectedMemberIds: memberIds.sort(compareText), instanceIds, condition: 'instances-display-definition-after-member-removal', requiresCapabilityConfirmation: instanceIds.length > 0, expandedInstances: false })
  }
  const nativeCandidates = diagnostic ? requested.filter(entity => entity.kind === 'entity' && (entity.type === 'HATCH' || entity.type === 'INSERT')).map(entity => ({ id: entity.id, type: entity.type, structuralRole: entity.type === 'HATCH' ? 'region-fill' : 'block-instance', patternCandidate: { eligible: true, kind: entity.type === 'HATCH' ? 'region-fill' : 'symbol-candidate', inferredRole: null, boundaryRole: 'unassigned' }, expandedGeometry: false })) : []
  ...
  const result: KJEraseImpact = {
    ...
    limits: { maxIds, maxObjects, scannedObjects: objects.length, maxReferences: KJDRAW_ERASE_IMPACT_LIMITS.maxReferences, references, maxConnectivityFeatures: KJDRAW_ERASE_IMPACT_LIMITS.maxConnectivityFeatures, maxConnectivityComparisons: KJDRAW_ERASE_IMPACT_LIMITS.maxConnectivityComparisons, maxBytes: query.maxBytes },
  }
  if (diagnostic && jsonBytes({ ok: true, value: result }) > query.maxBytes) fail('Erase impact result exceeds maxBytes; query fewer IDs or increase maxBytes')
