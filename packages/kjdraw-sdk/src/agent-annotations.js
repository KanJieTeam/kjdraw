// Generated from agent-annotations.ts by scripts/build-typescript.mjs. Do not edit directly.
...
function jsonSnapshot(input, maxBytes) {
    ...
    const visit = (value, depth)=>{
        ...
        if (typeof value === 'string') {
            ...
            if (characters > maxBytes) fail('Annotation input exceeds its byte budget');
            return value;
        }
        ...
        for (const key of keys){
            ...
            if (typeof key !== 'string' || [
                ...
                'constructor',
                'prototype'
            ].includes(key) || array && !/^(0|[1-9]\d*)$/.test(key)) fail('Unsafe annotation input property');
        ...
        if (array && keys.length - 1 !== value.length) fail('Annotation arrays cannot contain holes');
        active.delete(value);
        return copy;
    };
...
function number(value, label, min = -1e12, max = 1e12) {
...
export function buildAgentAnnotationEntities(document, input, options = {}) {
    const source = record(jsonSnapshot(input, 65536), [
        'expectedRevision',
        'units',
        'texts',
        'dimensions'
    ...
    const bases = opts.baseEntities === undefined ? {} : record(opts.baseEntities, Object.keys(opts.baseEntities), []);
    ...
    const reference = (value, withFeature = false)=>{
        const ref = record(value, withFeature ? [
            'source',
            'id',
            'feature',
            'vertexIndex'
        ...
        if (![
            'document',
            'proposal'
        ].includes(ref.source)) fail('Unknown annotation reference source');
    ...
    const feature = (value)=>{
        ...
        if ([
            ...
            'top',
            'bottom'
        ].includes(ref.feature)) {
            if (![
                'CIRCLE',
                'ARC'
            ].includes(object.type)) fail('Quadrant references require CIRCLE/ARC entities');
            const angles = {
                right: 0,
                top: Math.PI / 2,
                left: Math.PI,
    ...
    const omitUndefined = (value)=>{
        ...
        if (value && typeof value === 'object') return Object.fromEntries(Object.entries(value).filter(([, child])=>child !== undefined).map(([key, child])=>[
                key,
                omitUndefined(child)
            ]));
        return value;
    ...
    for (const value of source.texts){
        ...
        append('TEXT', {
            ...
            height: number(item.height, 'Text height', 1e-6, 1e6),
    ...
    for (const value of source.dimensions){
        ...
        if (type === 'ALIGNED' || type === 'ROTATED') {
            ...
            if (type === 'DIAMETER' ? object.type !== 'CIRCLE' : ![
                'CIRCLE',
                'ARC'
            ].includes(object.type)) fail('Radius requires a CIRCLE/ARC; diameter requires a CIRCLE');
            const center = nativePoint(object.payload.center), radius = number(object.payload.radius, 'Referenced radius', 1e-12, 1e12);
