// Generated from project-package.ts by scripts/build-typescript.mjs. Do not edit directly.
...
export const KJP_DEFAULT_READ_LIMITS = Object.freeze({
    maxEntries: 10_000,
...
function normalizeReadLimits(overrides = {}) {
    const limits = {
...
function normalizeZipEntries(input) {
    ...
    else if (Array.isArray(input)) source = input.map((row)=>[
            row.path,
            row.data
        ]);
...
export function encodeZip64(input) {
    ...
    for (const entry of entries){
        ...
        const localExtra = zip64Extra([
            size,
            size
        ]);
        const { data: local, view } = header(30);
        u32(view, 0, ZIP_SIGNATURE.local);
        u16(view, 4, 45);
        u16(view, 6, 0x0800);
        u16(view, 8, 0);
        u16(view, 10, 0);
        ...
        u32(view, 18, 0xffffffff);
        u32(view, 22, 0xffffffff);
        u16(view, 26, entry.name.length);
        u16(view, 28, localExtra.length);
    ...
    for (const entry of central){
        ...
        u32(view, 0, ZIP_SIGNATURE.central);
        u16(view, 4, 45);
        u16(view, 6, 45);
        u16(view, 8, 0x0800);
        u16(view, 10, 0);
        ...
        u32(view, 20, 0xffffffff);
        u32(view, 24, 0xffffffff);
    ...
    u32(z64, 0, ZIP_SIGNATURE.zip64End);
    u64(z64, 4, 44);
    u16(z64, 12, 45);
    u16(z64, 14, 45);
    u32(z64, 16, 0);
    u32(z64, 20, 0);
    u64(z64, 24, entries.length);
    u64(z64, 32, entries.length);
    u64(z64, 40, centralSize);
    u64(z64, 48, centralOffset);
    const { data: locator, view: loc } = header(20);
    u32(loc, 0, ZIP_SIGNATURE.zip64Locator);
    u32(loc, 4, 0);
    u64(loc, 8, zip64EndOffset);
    u32(loc, 16, 1);
    const { data: end, view: eocd } = header(22);
    u32(eocd, 0, ZIP_SIGNATURE.end);
    u16(eocd, 4, 0xffff);
    u16(eocd, 6, 0xffff);
    u16(eocd, 8, 0xffff);
    u16(eocd, 10, 0xffff);
...
export function decodeZip64(source, inputLimits = {}, signal) {
...
function drawingSourceId(source) {
...
function normalizedDrawings(input) {
    let rows;
    if (input instanceof Map) rows = [
        ...input
    ];
    else if (Array.isArray(input)) rows = input.map((row)=>[
            row.id,
            row.document ?? row.data ?? row
        ]);
    ...
    return rows.map(([rawId, source])=>{
        ...
        return {
            id,
            document,
            path: `drawings/${id}.kjd`,
            data: bytes(document.serialize())
        };
...
export async function createKjpPackage(options = {}) {
    ...
    const entries = new Map(drawings.map((row)=>[
            row.path,
            row.data
        ]));
    ...
    const manifest = {
        ...
        activeDrawing,
        drawings: drawings.map((row)=>({
                id: row.id,
                path: row.path,
                revision: row.document.revision,
                sha256: contentHashes[row.path]
            })),
        contentHashes,
        application: {
            name: 'KJDraw',
            minReaderVersion: KJDRAW_VERSION,
            writerVersion: String(options.writerVersion ?? KJDRAW_VERSION)
        },
...
function jsonEntry(entries, path) {
    ...
    try {
        ...
        throw new KJValidationError(`KJP ${path} 不是有效 UTF-8 JSON`, {
            path,
            cause: error
        });
    }
...
function manifestFrom(value) {
    ...
    const drawings = value.drawings.map((row, index)=>{
    ...
    return {
        ...
        drawings,
        contentHashes: Object.fromEntries(objectEntries(value.contentHashes).map(([path, hash])=>[
                path,
                String(hash)
            ])),
        application: {
            name: 'KJDraw',
            minReaderVersion: String(application.minReaderVersion ?? ''),
            writerVersion: String(application.writerVersion ?? '')
        },
...
export async function openKjpPackage(source, options = {}) {
    ...
    for (const row of manifest.drawings){
        ...
        try {
            ...
            throw new KJValidationError(`KJP 图纸不是有效 KJD：${path}`, {
                path,
                cause: error
            });
        }
    ...
    const commands = historyText.split(/\r?\n/).filter(Boolean).map((line, index)=>{
        try {
            ...
            throw new KJValidationError(`KJP 命令日志第 ${index + 1} 行损坏`, {
                line: index + 1,
                cause: error
            });
        }
