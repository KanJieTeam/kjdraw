// Generated from file-adapters.ts by scripts/build-typescript.mjs. Do not edit directly.
...
export class KJFileAdapterRegistry {
    ...
    find({ format, version = null, operation = 'read', adapterId = null } = {}) {
        ...
        const candidates = adapterId ? [
            this.get(adapterId)
        ].filter((adapter)=>Boolean(adapter)) : this.list();
    ...
    async read(source, inputOptions = {}) {
        ...
        if (!adapter?.read) throw new KJAdapterError(`No reader for ${format ?? 'unknown format'} ${options.version ?? ''}`.trim(), {
            format,
        ...
        try {
            ...
            throw new KJAdapterError(`Adapter ${adapter.id} failed to read ${format}`, {
                adapterId: adapter.id,
                format
            }, error);
        }
    ...
    async write(document, options = {}) {
        ...
        if (!adapter?.write) throw new KJAdapterError(`No writer for ${options.format} ${options.version ?? ''}`.trim(), {
            format: options.format,
            version: options.version
        });
        try {
            return await adapter.write(document, {
                ...options,
                format: normalizeName(options.format),
                adapter
            });
        } catch (error) {
            throw new KJAdapterError(`Adapter ${adapter.id} failed to write ${options.format}`, {
                adapterId: adapter.id,
                format: options.format
            }, error);
        }
