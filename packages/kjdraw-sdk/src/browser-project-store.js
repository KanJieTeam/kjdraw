...
function requireKjpName(name) {
  ...
  if (!name.toLowerCase().endsWith('.kjp')) throw new KJValidationError('本地工程文件必须使用 .kjp 扩展名')
  return name
}

function fileBytes(file) { return file.arrayBuffer().then(value => new Uint8Array(value)) }
...
async function recoveryRoot() {
  if (!globalThis.navigator?.storage?.getDirectory) throw new KJValidationError('当前运行时不支持本地崩溃恢复存储')
...
export class BrowserKjpFileBinding {
  constructor(handle = null) { this.handle = handle }

  static supported() { return typeof globalThis.showOpenFilePicker === 'function' && typeof globalThis.showSaveFilePicker === 'function' }

  static async chooseOpen(options = {}) {
    if (!this.supported()) throw new KJValidationError('当前浏览器不能绑定本地工程文件，请使用 KJDraw 桌面版或 Chromium 内核')
  ...
  get bound() { return Boolean(this.handle) }
  ...
  async write(data) {
    ...
    try { await writable.write(data); await writable.close() } catch (error) { await writable.abort?.().catch(() => {}); throw error }
  ...
  async writeRecovery(projectId, data) {
    await openKjpPackage(data)
    const root = await recoveryRoot(), handle = await root.getFileHandle(recoveryName(projectId), { create: true }), writable = await handle.createWritable()
    try { await writable.write(data); await writable.close() } catch (error) { await writable.abort?.().catch(() => {}); throw error }
    return { projectId: String(projectId), bytes: data.byteLength }
  }

  async inspectRecovery(projectId) {
    try {
      const root = await recoveryRoot(), handle = await root.getFileHandle(recoveryName(projectId)), data = await fileBytes(await handle.getFile()), project = await openKjpPackage(data)
      return { available: true, data, manifest: project.manifest }
    } catch (error) {
      if (error?.name === 'NotFoundError') return { available: false }
      throw error
    }
  }

  async clearRecovery(projectId) {
    try { await (await recoveryRoot()).removeEntry(recoveryName(projectId)); return true } catch (error) { if (error?.name === 'NotFoundError') return false; throw error }
  }
}
