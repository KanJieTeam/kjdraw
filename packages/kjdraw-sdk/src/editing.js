...
function joinEllipsePayloads(entities, primaryIndex, tolerance) {
    const paths = entities.map((entity, index)=>({
            id: String(entity.id ?? `join-source-${index}`),
            geometry: ellipseEditGeometry(entity)
        }));
    ...
    const parameterDistance = (first, second)=>{
        const delta = Math.abs(positiveTurn(first - second));
        return Math.min(delta, TURN - delta);
    };
    for (const path of paths){
        const geometry = path.geometry;
        if (distance3(geometry.center, reference.center) > tolerance || distance3(geometry.majorAxis, reference.majorAxis) > tolerance || Math.abs(geometry.ratio - reference.ratio) * reference.majorLength > tolerance) {
    ...
    const starts = predecessorCount.flatMap((count, index)=>count === 0 ? [
            index
        ] : []);
    ...
    while(!used.has(current)){
        used.add(current);
        ordered.push(paths[current]);
        const end = paths[current].geometry.start + paths[current].geometry.span;
        const next = paths.flatMap((path, index)=>!used.has(index) && parameterDistance(path.geometry.start, end) <= parameterTolerance ? [
                index
            ] : []);
    ...
    return {
        type: 'ELLIPSE',
        payload,
        sourceIds: ordered.map((path)=>path.id),
        closed
    };
}
