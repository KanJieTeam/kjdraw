...
export class KJSelectionSet {
  ...
  get ids() { return Object.freeze([...this.#ids]) }
  get objects() { return Object.freeze(this.#ids.map(id => this.#document.getObject(id)).filter(Boolean)) }
  has(value) { return this.#ids.includes(String(value?.id ?? value)) }
  onChange(listener, options) { return this.#events.on('change', listener, options) }
  ...
  remove(values, { silent = false } = {}) {
    const targets = new Set((Array.isArray(values) ? values : [values]).map(value => String(value?.id ?? value)))
  ...
  toggle(value) { return this.has(value) ? this.remove(value) : this.add(value) }
...
export class KJSelectionManager {
  ...
  dispose() { this.#disposeDocumentChange?.(); this.#disposeDocumentChange = null }
