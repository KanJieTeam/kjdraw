...
import { exportDrawingPng, type KJDrawingPngExport, type KJDrawingPngOptions } from './drawing-image.js'
...
import type { KJComponentCatalogEntry, KJComponentSearchResult } from './component-library.js'
import {
...
const copy = {
  en: {
    ...
    showLayer: 'Show layer', hideLayer: 'Hide layer', lockLayer: 'Lock layer', unlockLayer: 'Unlock layer', freezeLayer: 'Freeze layer', thawLayer: 'Thaw layer', newLayer: 'New layer', editLayer: 'Layer settings', currentLayer: 'Current layer', setCurrentLayer: 'Set current', layerName: 'Layer name', layerColor: 'Color index (1–255)', layerLinetype: 'Linetype', layerLineweight: 'Lineweight', layerVisible: 'Visible', layerLocked: 'Locked', layerFrozen: 'Frozen', makeCurrent: 'Make current', byLayer: 'ByLayer', entityColor: 'Color', entityLinetype: 'Linetype', entityLineweight: 'Lineweight',
  ...
  'zh-CN': {
    ...
    showLayer: '显示图层', hideLayer: '隐藏图层', lockLayer: '锁定图层', unlockLayer: '解锁图层', freezeLayer: '冻结图层', thawLayer: '解冻图层', newLayer: '新建图层', editLayer: '图层设置', currentLayer: '当前图层', setCurrentLayer: '设为当前', layerName: '图层名称', layerColor: '颜色索引（1–255）', layerLinetype: '线型', layerLineweight: '线宽', layerVisible: '可见', layerLocked: '锁定', layerFrozen: '冻结', makeCurrent: '设为当前图层', byLayer: '随层', entityColor: '颜色', entityLinetype: '线型', entityLineweight: '线宽',
...
const DRAFT_COMMAND_TO_TOOL = new Map<string, KJDraftTool>([
  ...
  ['DIM', 'dimension'], ['DIMENSION', 'dimension'], ['LE', 'leader'], ['LEADER', 'leader'],
])
...
const draftToolText: Readonly<Record<KJDraftTool, KJLocalizedControlText>> = Object.freeze({
  ...
  ray: { en: 'Ray', zh: '射线' }, xline: { en: 'Construction line', zh: '构造线' }, spline: { en: 'Spline', zh: '样条曲线' }, hatch: { en: 'Hatch', zh: '填充' }, dimension: { en: 'Dimension', zh: '标注' }, leader: { en: 'Leader', zh: '引线' },
})
...
const draftPointText: Readonly<Record<KJDraftPointRole, KJLocalizedControlText>> = Object.freeze({
  ...
  arrowPoint: { en: 'Specify the arrow point', zh: '指定箭头点' }, leaderVertex: { en: 'Specify the next leader vertex', zh: '指定下一引线顶点' },
})
...
export class KJDrawWorkbench {
  ...
  async exportPng(options: KJWorkbenchPngOptions = {}): Promise<KJDrawingPngExport> {
    if (this.#abort.signal.aborted) throw new Error('KJDraw workbench has been disposed')
    const drawing = this.document
    if (!drawing) throw new Error('No active KJDraw document')
    ...
    const output = await exportDrawingPng(drawing, {
      layoutId,
      ...(options.maxEdge === undefined ? {} : { maxEdge: options.maxEdge }),
  ...
  #bind(): void {
    ...
    query<HTMLButtonElement>(this.root, '[data-action="save-png"]').addEventListener('click', () => void this.#run(() => this.exportPng()), { signal })
  ...
  async #runCommand(): Promise<void> {
    ...
    const result = await this.#run(async () => {
      ...
      if (command === 'BR' || command === 'BREAK') {
        ...
        const twoPoint = target?.type === 'CIRCLE' || (['LWPOLYLINE', 'POLYLINE'].includes(target?.type ?? '') && target?.payload.closed === true)
  ...
  #syncDraftActions(): void {
    ...
    const variableLength = state?.maximumPoints === null, closable = Boolean(state && ['polyline', 'spline', 'hatch'].includes(state.tool))
    ...
    if (close) { close.hidden = !closable; close.disabled = !(state?.canClose) }
  ...
  async #commitDraft(gesture: KJWorkbenchDraftGesture, spec: KJDraftEntitySpec): Promise<void> {
    ...
    const receipt = await this.#run(() => spec.type === 'LEADER'
      ? this.execute('LEADER', { ...spec.payload, ...(spec.options?.ownerId ? { ownerId: spec.options.ownerId } : {}) }, { expectedRevision: gesture.revision })
      : this.execute('CREATE', { type: spec.type, payload: spec.payload, ...(spec.options ? { options: spec.options } : {}) }, { expectedRevision: gesture.revision }))
  ...
  async #pointerUp(event: PointerEvent): Promise<void> {
    ...
    const rawWorld = this.renderer.screenToWorld(location)
    const directPick = ['PEDIT', 'BREAK'].includes(this.#modificationGesture?.definition.command ?? '')
    const snapped = (this.#draftGesture !== null || this.#modificationGesture !== null || ['text', 'measure', 'move', 'copy'].includes(this.#tool)) && !directPick ? this.#snapAt(rawWorld) : null
  ...
  #drawOverlay(): void {
    ...
    if (modification) {
      ...
      try {
        if (['PEDIT', 'BREAK'].includes(modification.definition.command) && cursor) {
          ...
          const directPreview = modification.definition.command === 'PEDIT'
            ? [{ type: target.type, payload: editPolylinePayload(target, request.arguments) }]
  ...
  #refreshLayers(): void {
    ...
    host.replaceChildren()
    const toolbar = document.createElement('div'); toolbar.className = 'layer-toolbar'
    const create = document.createElement('button'); create.type = 'button'; create.dataset.action = 'new-layer'; create.textContent = `+ ${this.#t('newLayer')}`; create.disabled = this.#readOnly === true
    create.addEventListener('click', () => this.#openLayerEditor(), { signal: this.#abort.signal }); toolbar.append(create); host.append(toolbar)
    ...
    for (const layer of drawing.getTable('layers')?.records ?? []) {
      ...
      const current = document.createElement('button'); current.type = 'button'; current.className = 'layer-state layer-current'; current.dataset.layerCurrent = layer.id
      const isCurrent = drawing.getTable('layers')?.currentId === layer.id; current.textContent = isCurrent ? '●' : '○'; current.disabled = this.#readOnly === true || isCurrent
      current.title = `${this.#t(isCurrent ? 'currentLayer' : 'setCurrentLayer')}: ${layer.name ?? '0'}`; current.setAttribute('aria-label', current.title); current.setAttribute('aria-pressed', String(isCurrent))
      current.addEventListener('click', () => void this.#run(() => this.execute('LAYERCURRENT', { id: layer.id })), { signal: this.#abort.signal })
      ...
      const swatch = document.createElement('i'); swatch.className = 'layer-swatch'; swatch.style.background = layer.payload.trueColor == null ? aciColor(layer.payload.color ?? 7, this.#options.theme) : `#${Number(layer.payload.trueColor).toString(16).padStart(6, '0')}`
      ...
      const edit = document.createElement('button'); edit.type = 'button'; edit.className = 'layer-state'; edit.dataset.layerEdit = layer.id; edit.textContent = '⋯'; edit.disabled = this.#readOnly === true
      edit.title = `${this.#t('editLayer')}: ${layer.name ?? '0'}`; edit.setAttribute('aria-label', edit.title)
      edit.addEventListener('click', () => this.#openLayerEditor(layer), { signal: this.#abort.signal })
      row.append(current, label, ...controls, edit, count); host.append(row)
    }
  }
  ...
  async #openComponentLibrary(): Promise<void> {
    ...
    const dialog = document.createElement('dialog'); dialog.className = 'modify-dialog component-dialog'; dialog.dataset.componentDialog = ''
    ...
    const queryInput = query<HTMLInputElement>(dialog, '[data-component-query]'), category = query<HTMLSelectElement>(dialog, '[data-component-category]')
    const results = query<HTMLSelectElement>(dialog, '[data-component-results]'), description = query<HTMLElement>(dialog, '[data-component-description]')
    const license = query<HTMLElement>(dialog, '[data-component-license]'), parameterHost = query<HTMLElement>(dialog, '[data-component-parameters]')
    const previous = query<HTMLButtonElement>(dialog, '[data-component-previous]'), next = query<HTMLButtonElement>(dialog, '[data-component-next]')
    let page: KJComponentSearchResult | null = null, cursor: string | null = null, cursors: Array<string | null> = []
    const item = (): KJComponentCatalogEntry | null => page?.items.find(candidate => candidate.id === results.value) ?? null
    const syncItem = (): void => {
      const selected = item(); parameterHost.replaceChildren()
      if (!selected) { description.textContent = ''; license.textContent = ''; return }
      ...
      for (const parameter of selected.parameters) {
        ...
        const label = document.createElement('span'); label.textContent = `${zh ? parameter.label.zh : parameter.label.en} (${parameter.unit})`
        const input = document.createElement('input'); input.type = 'number'; input.step = parameter.integer ? '1' : 'any'; input.min = String(parameter.minimum); input.max = String(parameter.maximum); input.value = String(parameter.default); input.dataset.componentParameter = parameter.name
        field.append(label, input); parameterHost.append(field)
      }
    }
    const load = async (targetCursor: string | null, remember = true): Promise<void> => {
      const result = await this.sdk.executeCommand<KJComponentSearchResult>('COMPONENTSEARCH', { query: queryInput.value, category: category.value, locale: this.#locale, limit: 3, ...(targetCursor === null ? {} : { cursor: targetCursor }) }, { document: drawing })
      page = result; cursor = targetCursor
      if (remember && cursors.at(-1) !== targetCursor) cursors.push(targetCursor)
      results.replaceChildren(...result.items.map(entry => { const option = document.createElement('option'); option.value = entry.id; option.textContent = `${this.#locale === 'zh-CN' ? entry.title.zh : entry.title.en} · ${entry.license.spdx}`; return option }))
      if (result.items.length) results.value = result.items[0]!.id
      previous.disabled = cursors.length < 2; next.disabled = result.nextCursor === null; syncItem()
    }
    const close = (): void => { if (dialog.open) dialog.close(); else dialog.remove() }
    dialog.addEventListener('close', () => dialog.remove(), { once: true }); query<HTMLButtonElement>(dialog, '[data-component-cancel]').addEventListener('click', close)
    results.addEventListener('change', syncItem)
    query<HTMLButtonElement>(dialog, '[data-component-search]').addEventListener('click', () => void this.#run(async () => { cursors = []; await load(null) }))
    next.addEventListener('click', () => void this.#run(async () => { if (page?.nextCursor) await load(page.nextCursor) }))
    previous.addEventListener('click', () => void this.#run(async () => { if (cursors.length > 1) { cursors.pop(); await load(cursors.at(-1) ?? null, false) } }))
    query<HTMLButtonElement>(dialog, '[data-component-insert]').addEventListener('click', () => void this.#run(async () => {
      const selected = item(); if (!selected) throw new Error(this.#t('componentResults'))
      const coordinates = query<HTMLInputElement>(dialog, '[data-component-position]').value.split(/[ ,]+/).filter(Boolean).map(Number)
      if (coordinates.length !== 2 || coordinates.some(value => !Number.isFinite(value))) throw new Error(this.#t('componentPosition'))
      const parameters = Object.fromEntries([...parameterHost.querySelectorAll<HTMLInputElement>('[data-component-parameter]')].map(input => [input.dataset.componentParameter!, Number(input.value)]))
      await this.execute('COMPONENTINSERT', { componentId: selected.id, version: selected.version, units: drawing.snapshot().header.units, parameters,
        position: coordinates, scale: Number(query<HTMLInputElement>(dialog, '[data-component-scale]').value), rotation: Number(query<HTMLInputElement>(dialog, '[data-component-rotation]').value) * Math.PI / 180 }, { expectedRevision: revision })
      close()
    }))
  ...
  #openHatchEditor(entity: KJReadonlyObjectRecord, sourceIds: readonly string[] = []): void {
    ...
    const sources = sourceIds.map(id => drawing.getObject(id)).filter((value): value is KJReadonlyObjectRecord => value?.kind === 'entity' && !value.erased)
    ...
    for (const [value, label, disabled] of [
      ...
      ['add-selected', this.#t('hatchAddSelected'), sources.length === 0],
      ['replace-selected', this.#t('hatchReplaceSelected'), sources.length === 0 || innerIndexes.length === 0],
    ...
    query<HTMLButtonElement>(dialog, '[data-hatch-apply]').addEventListener('click', () => void this.#run(async () => {
      ...
      const command: Record<string, unknown> = { id: entity.id, operation: operation.value.replace('-selected', '-island'), patternScale: Number(query<HTMLInputElement>(dialog, '[data-hatch-scale]').value), patternAngle: Number(query<HTMLInputElement>(dialog, '[data-hatch-angle]').value) * Math.PI / 180 }
...
#openLayerEditor(layer?: KJReadonlyObjectRecord): void {
    const drawing = this.document
    if (!drawing || this.#readOnly) return
    const dialog = document.createElement('dialog'); dialog.className = 'modify-dialog'; dialog.dataset.layerDialog = layer?.id ?? 'new'
    const form = document.createElement('form'); form.className = 'modify-form'; form.method = 'dialog'
    const head = document.createElement('div'); head.className = 'modify-head'
    const heading = document.createElement('h2'); heading.textContent = this.#t(layer ? 'editLayer' : 'newLayer'); head.append(heading)
    const body = document.createElement('div'); body.className = 'modify-body modify-fields'
    const addField = (key: keyof typeof copy.en, control: HTMLElement) => { const label = document.createElement('label'); label.className = control instanceof HTMLInputElement && control.type === 'checkbox' ? 'check' : 'field'; const span = document.createElement('span'); span.textContent = this.#t(key); label.append(span, control); body.append(label) }
    const name = document.createElement('input'); name.name = 'name'; name.value = String(layer?.name ?? ''); name.required = true; name.maxLength = 255; name.dataset.layerName = ''; addField('layerName', name)
    const color = document.createElement('input'); color.name = 'color'; color.type = 'number'; color.min = '1'; color.max = '255'; color.step = '1'; color.value = String(layer?.payload.color ?? 7); color.dataset.layerColor = ''; addField('layerColor', color)
    const linetype = document.createElement('select'); linetype.name = 'linetypeId'; linetype.dataset.layerLinetype = ''
    for (const item of drawing.getTable('linetypes')?.records ?? []) { const option = document.createElement('option'); option.value = item.id; option.textContent = String(item.name ?? 'CONTINUOUS'); option.selected = item.id === (layer?.payload.linetypeId ?? drawing.getTable('linetypes')?.currentId); linetype.append(option) }
    addField('layerLinetype', linetype)
    const lineweight = document.createElement('select'); lineweight.name = 'lineweight'; lineweight.dataset.layerLineweight = ''
    for (const value of LAYER_LINEWEIGHTS) { const option = document.createElement('option'); option.value = String(value); option.textContent = value === -1 ? this.#t('byLayer') : value === 0 ? '0.00 mm' : `${(value / 100).toFixed(2)} mm`; option.selected = value === Number(layer?.payload.lineweight ?? -1); lineweight.append(option) }
    addField('layerLineweight', lineweight)
    const checks = ([['visible', 'layerVisible', layer?.payload.visible !== false], ['locked', 'layerLocked', layer?.payload.locked === true], ['frozen', 'layerFrozen', layer?.payload.frozen === true], ['current', 'makeCurrent', drawing.getTable('layers')?.currentId === layer?.id]] as const)
    for (const [fieldName, key, checked] of checks) { const input = document.createElement('input'); input.type = 'checkbox'; input.name = fieldName; input.checked = checked; input.dataset.layerProperty = fieldName; addField(key, input) }
    const actions = document.createElement('div'); actions.className = 'modify-actions'
    const cancel = document.createElement('button'); cancel.type = 'button'; cancel.textContent = this.#t('cancel'); cancel.addEventListener('click', () => dialog.close())
    const apply = document.createElement('button'); apply.type = 'submit'; apply.className = 'confirm'; apply.textContent = this.#t('apply'); actions.append(cancel, apply)
    form.append(head, body, actions); dialog.append(form); this.root.append(dialog)
    dialog.addEventListener('close', () => dialog.remove(), { once: true })
    form.addEventListener('submit', event => {
      event.preventDefault()
      const args = { name: name.value, color: Number(color.value), linetypeId: linetype.value, lineweight: Number(lineweight.value), visible: (form.elements.namedItem('visible') as HTMLInputElement).checked, locked: (form.elements.namedItem('locked') as HTMLInputElement).checked, frozen: (form.elements.namedItem('frozen') as HTMLInputElement).checked, current: (form.elements.namedItem('current') as HTMLInputElement).checked }
      const operation = layer
        ? this.execute('LAYERUPDATE', { id: layer.id, newName: args.name, patch: { color: args.color, trueColor: null, linetypeId: args.linetypeId, lineweight: args.lineweight, visible: args.visible, locked: args.locked, frozen: args.frozen }, current: args.current })
        : this.execute('LAYERNEW', args)
      void this.#run(async () => { await operation; dialog.close() })
    })
  ...
  #refreshInspector(): void {
    ...
    if (!entity || entity.kind !== 'entity') {
      const scaleField = document.createElement('label'); scaleField.className = 'field'; const label = document.createElement('span'); label.textContent = this.#t('globalLinetypeScale')
      const scale = document.createElement('input'); scale.type = 'number'; scale.min = '0.000001'; scale.step = '0.1'; scale.value = String(drawing.snapshot().header.systemVariables.LTSCALE ?? 1); scale.disabled = this.#readOnly === true; scale.dataset.property = 'global-linetype-scale'; scaleField.append(label, scale); host.append(scaleField)
      if (!this.#readOnly) {
        const apply = document.createElement('button'); apply.type = 'button'; apply.className = 'apply'; apply.textContent = this.#t('apply')
        apply.addEventListener('click', () => void this.#run(() => this.execute('SETVAR', { name: 'LTSCALE', value: Number(scale.value) })), { signal: this.#abort.signal }); host.append(apply)
      }
      const empty = document.createElement('p'); empty.className = 'empty'; empty.textContent = this.#t('noSelection'); host.append(empty); return
    }
    ...
    for (const layer of drawing.getTable('layers')?.records ?? []) {
      const option = document.createElement('option'); option.value = layer.id; option.textContent = layer.name ?? '0'; option.selected = commonLayerId === layer.id
      option.disabled = layer.id !== commonLayerId && (layer.payload.visible === false || layer.payload.locked === true || layer.payload.frozen === true); layerSelect.append(option)
    }
    ...
    const styleSelect = (property: string, labelText: string, values: readonly (readonly [string, string])[], selectedValues: Set<string>) => {
      const field = document.createElement('label'); field.className = 'field'; const span = document.createElement('span'); span.textContent = labelText
      const select = document.createElement('select'); select.dataset.property = property; select.disabled = this.#readOnly === true
      if (selectedValues.size !== 1) { const option = document.createElement('option'); option.value = ''; option.textContent = '—'; option.selected = true; option.disabled = true; select.append(option) }
      const common = selectedValues.size === 1 ? [...selectedValues][0]! : ''
      for (const [value, text] of values) { const option = document.createElement('option'); option.value = value; option.textContent = text; option.selected = value === common; select.append(option) }
      field.append(span, select); host.append(field); return { select, common }
    }
    const colors = styleSelect('color', this.#t('entityColor'), [['256', this.#t('byLayer')], ...Array.from({ length: 255 }, (_, index) => [String(index + 1), String(index + 1)] as const)], new Set(selectedEntities.map(item => String(item.payload.color ?? 256))))
    const linetypes = styleSelect('linetype', this.#t('entityLinetype'), [['', this.#t('byLayer')], ...(drawing.getTable('linetypes')?.records ?? []).map(item => [item.id, String(item.name ?? 'CONTINUOUS')] as const)], new Set(selectedEntities.map(item => String(item.payload.linetypeId ?? ''))))
    const lineweights = styleSelect('lineweight', this.#t('entityLineweight'), LAYER_LINEWEIGHTS.map(value => [String(value), value === -1 ? this.#t('byLayer') : value === 0 ? '0.00 mm' : `${(value / 100).toFixed(2)} mm`] as const), new Set(selectedEntities.map(item => String(item.payload.lineweight ?? -1))))
    const scaleValues = new Set(selectedEntities.map(item => String(item.payload.linetypeScale ?? 1))), commonLinetypeScale = scaleValues.size === 1 ? [...scaleValues][0]! : ''
    const scaleField = document.createElement('label'); scaleField.className = 'field'; const scaleLabel = document.createElement('span'); scaleLabel.textContent = this.#t('linetypeScale')
    const linetypeScale = document.createElement('input'); linetypeScale.type = 'number'; linetypeScale.min = '0.000001'; linetypeScale.step = '0.1'; linetypeScale.value = commonLinetypeScale; linetypeScale.placeholder = '—'; linetypeScale.disabled = this.#readOnly === true; linetypeScale.dataset.property = 'linetype-scale'; scaleField.append(scaleLabel, linetypeScale); host.append(scaleField)
    ...
    if (!multiple && (entity.type === 'CIRCLE' || entity.type === 'ARC')) {
      ...
      const add = (labelText: string, control: HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement): void => { const label = document.createElement('label'); label.className = control.type === 'checkbox' ? 'check' : 'field'; const span = document.createElement('span'); span.textContent = labelText; label.append(span, control); host.append(label) }
      const text = document.createElement('textarea'); text.value = String(annotation?.payload.text ?? entity.payload.text ?? ''); text.dataset.property = 'leader-text'
    ...
    if (!this.#readOnly && editableHatch) {
      ...
      editHatch.addEventListener('click', () => this.#openHatchEditor(editableHatch, hatchSourceIds), { signal: this.#abort.signal })
    ...
    if (!this.#readOnly) {
      ...
      apply.addEventListener('click', () => void this.#run(async () => {
        ...
        if (colors.select.value && colors.select.value !== colors.common) { payload.color = Number(colors.select.value); payload.trueColor = null }
        if (linetypes.select.value !== linetypes.common) { payload.linetypeId = linetypes.select.value || null; payload.linetypeName = linetypes.select.value ? drawing.getObject(linetypes.select.value)?.name : 'BYLAYER' }
        ...
        if (multiple) {
          if (Object.keys(payload).length) await this.execute('PROPERTIES', { ids: selectedEntities.map(item => item.id), patch: { payload } })
          return
        }
        if (leaderFields) {
