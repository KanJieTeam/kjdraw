...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i += 1) {
    ...
    if (!['--workspace', '--scope', '--input', '--blank', '--units', '--proposal-dir', '--candidate-dir', '--previous-mcp-script', '--mcp-script', '--geology-column-pack', '--geology-column-pack-sha256'].includes(key) || !argv[i + 1] || argv[i + 1].startsWith('--')) throw new Error('Unknown or incomplete option')
    options[key === '--proposal-dir' ? 'proposalDir' : key === '--candidate-dir' ? 'candidateDir' : key === '--previous-mcp-script' ? 'previousMcpScript' : key === '--mcp-script' ? 'mcpScript' : key.slice(2)] = argv[++i]
  }
...
export async function connectWorkspace(options, hooks = {}) {
  ...
  if (options.mcpScript && (scope !== 'user' || !isAbsolute(options.mcpScript))) throw new Error('--mcp-script requires an absolute path and user scope')
