...
<style>
:root{font:12px system-ui,-apple-system,"Segoe UI",sans-serif;background:#f4f5f6;color:#26323a;--line:#dce1e4;--muted:#7a858c;--accent:#6ea7c2;--dark:#11171c;--dark-2:#1a2229;--paper:#fffefa}
*{box-sizing:border-box}body{margin:0;min-height:100vh;display:grid;grid-template-rows:56px 1fr 34px;background:#f4f5f6;color:#26323a}
...
.appbar{display:flex;align-items:center;justify-content:space-between;gap:16px;padding:0 18px;border-bottom:1px solid var(--line);background:rgba(255,255,255,.96)}.identity{min-width:0;display:flex;align-items:center;gap:11px}.doc-mark{width:25px;height:29px;border:1px solid #a6b8c1;border-radius:3px;display:grid;place-items:center;color:#5c8596;font-size:10px;letter-spacing:.06em}.doc-copy{min-width:0}.doc-copy strong{display:block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:13px;font-weight:600}.doc-copy span{display:flex;gap:8px;align-items:center;color:var(--muted);font-size:11px}.ready{color:#4b7e67}.app-actions{display:flex;align-items:center;gap:4px;color:#53656f}.app-actions button{font-size:11px}.divider{width:1px;height:20px;background:var(--line);margin:0 5px}
.preview-shell{min-height:0;position:relative;padding:14px 18px 12px;background:#f4f5f6}.canvas-shell{position:relative;height:100%;min-height:420px;overflow:hidden;border:1px solid #cbd2d6;border-radius:8px;background:#20282e;box-shadow:0 8px 24px rgba(35,46,54,.10);touch-action:none;cursor:grab}.canvas-shell.dragging{cursor:grabbing}.canvas-shell.plot{background:#e2e6e8}.canvas-head{position:absolute;z-index:4;top:13px;left:15px;display:flex;align-items:center;gap:8px;color:#eef3f5;font-size:11px;pointer-events:none}.plot .canvas-head{color:#4d606a}.canvas-head strong{font-weight:600}.canvas-head span{opacity:.66}.canvas-grid{position:absolute;inset:0;opacity:.18;background-image:linear-gradient(#90a4ae 1px,transparent 1px),linear-gradient(90deg,#90a4ae 1px,transparent 1px);background-size:28px 28px;pointer-events:none}.plot .canvas-grid{opacity:.48;background-image:linear-gradient(#bbc5c9 1px,transparent 1px),linear-gradient(90deg,#bbc5c9 1px,transparent 1px)}
...
.crosshair{position:absolute;inset:0;pointer-events:none;display:none}.canvas-shell:hover .crosshair{display:block}.crosshair:before,.crosshair:after{content:"";position:absolute;background:#83aec0;opacity:.28}.crosshair:before{left:var(--mx,50%);top:0;width:1px;height:100%}.crosshair:after{top:var(--my,50%);left:0;height:1px;width:100%}.select-box{position:absolute;border:1px solid #75aec3;background:rgba(90,153,181,.13);display:none;pointer-events:none}
.floating-tools{position:absolute;z-index:5;top:42px;left:15px;display:flex;align-items:center;gap:4px;padding:4px;border:1px solid rgba(255,255,255,.16);border-radius:7px;background:rgba(20,27,32,.84);box-shadow:0 5px 18px rgba(0,0,0,.16);backdrop-filter:blur(8px);color:#e6eef1}.plot .floating-tools{border-color:#cbd5d9;background:rgba(255,255,255,.92);color:#455861;box-shadow:0 5px 18px rgba(45,59,68,.12)}.floating-tools button{height:28px;padding:0 8px;font-size:11px}.floating-tools button.icon{width:29px;padding:0;font-size:15px}.floating-tools .readout{min-width:48px;text-align:center;color:inherit;font-variant-numeric:tabular-nums}.display{display:flex;gap:2px;padding-left:3px;margin-left:2px;border-left:1px solid rgba(128,150,159,.3)}.display button{font-size:10px}.hint{position:absolute;z-index:4;left:16px;bottom:11px;color:rgba(234,241,243,.62);font-size:10px;pointer-events:none}.plot .hint{color:#718088}.inspector{position:absolute;z-index:6;right:15px;bottom:15px;width:250px;max-height:55%;overflow:auto;padding:13px 14px;border:1px solid rgba(116,142,151,.34);border-radius:8px;background:rgba(248,250,250,.96);box-shadow:0 12px 28px rgba(31,46,55,.18);backdrop-filter:blur(10px)}.inspector[hidden]{display:none}.inspector button{float:right;width:24px;height:24px;padding:0;border:1px solid #d2dce0;border-radius:4px;color:#6d7b83}.inspector strong{display:block;margin-bottom:10px;font-size:12px;font-weight:600}.inspector dl{display:grid;grid-template-columns:82px minmax(0,1fr);gap:7px 10px;margin:0;font-size:11px}.inspector dt{color:#7a888f}.inspector dd{margin:0;overflow-wrap:anywhere;color:#33434c}.statusbar{display:flex;align-items:center;gap:18px;padding:0 18px;border-top:1px solid var(--line);background:#fff;color:#77848b;font-size:10px}.statusbar .grow{flex:1;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:#55666f}.statusbar span{white-space:nowrap}.kbd{border:1px solid #d2dadd;border-radius:3px;padding:1px 5px;color:#66757d;background:#fafbfb}
...
<main class="preview-shell"><section id="viewport" class="canvas-shell plot" aria-label="KJDraw 图纸预览"><div class="canvas-head"><strong>图纸预览</strong><span>·</span><span id="canvasMode">白底</span></div><div id="gridLayer" class="canvas-grid"></div><div class="sheet"><div id="drawing"></div></div><div class="crosshair"></div><div id="box" class="select-box"></div><nav class="floating-tools" aria-label="图纸操作"><div class="tool-group"><button class="on" title="选择和查看" type="button">选择</button><button id="windowZoom" title="框选放大" type="button">框选</button></div><span class="divider"></span><div class="tool-group"><button id="minus" class="icon" type="button" aria-label="缩小">−</button><span id="zoom" class="readout">100%</span><button id="plus" class="icon" type="button" aria-label="放大">＋</button><button id="fit" type="button">全图</button></div><div class="display"><button id="modeCad" type="button">黑底</button><button id="modePlot" class="on" type="button">白底</button></div><button id="grid" class="on" type="button">网格</button></nav><div class="hint">滚轮缩放 · 拖动平移 · 点击图元查看属性</div><aside id="props" class="inspector" hidden><button id="closeProps" type="button" aria-label="关闭属性">×</button><strong>对象属性</strong><div id="propsBody"></div></aside></section></main>
...
function updateMetrics(){const percent=Math.round(base.width/view.width*100)+'%';document.getElementById('zoom').textContent=percent;document.getElementById('zoomMetric').textContent=percent;document.getElementById('canvasMode').textContent=mode==='plot'?'白底':'黑底'}
