...
<style>
:root{font:12px system-ui,-apple-system,"Segoe UI",sans-serif;background:#f2f4f5;color:#25343f;--panel:#fbfcfc;--line:#d9e0e5;--muted:#74818b;--accent:#4c7897;--good:#4c7b63;--paper:#fffefa}
*{box-sizing:border-box}body{margin:0;min-height:100vh;display:grid;grid-template-rows:auto 1fr auto;background:#f2f4f5;color:#25343f}
button{height:30px;border:1px solid #d3dce2;border-radius:5px;background:#fff;color:#3d4c57;padding:0 10px;cursor:pointer;font:inherit;transition:background .15s,border-color .15s,color .15s}
button:hover{background:#f3f7f8;border-color:#bdcbd4}button:focus-visible{outline:2px solid var(--accent);outline-offset:2px}button.on{background:#eaf1f4;color:#315b75;border-color:#aac2cf}
.topbar{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:18px;align-items:center;padding:11px 16px;border-bottom:1px solid var(--line);background:rgba(255,255,255,.94)}
.identity{min-width:0;display:flex;gap:11px;align-items:center}.brand{width:26px;height:30px;border:1px solid #9fb3bf;border-radius:3px;display:grid;place-items:center;background:#f8fbfb;color:#52758a;font-size:10px;letter-spacing:.08em}.copy{min-width:0}.copy strong{display:block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:13px;font-weight:600}.copy span{display:flex;gap:9px;align-items:center;color:var(--muted);font-size:11px}.ok{color:var(--good)}.tools{display:flex;gap:5px;align-items:center;flex-wrap:wrap;justify-content:flex-end}.separator{width:1px;height:20px;background:#dbe2e6;margin:0 3px}.zoom-readout{min-width:58px;text-align:center;color:#52636e;font-variant-numeric:tabular-nums}
.workspace{min-height:420px;display:grid;grid-template-columns:42px minmax(0,1fr) 248px;background:#e9edef}.rail{border-right:1px solid var(--line);background:#f7f9fa;padding:10px 5px;display:flex;flex-direction:column;gap:6px}.rail button{width:32px;height:32px;padding:0;color:#61727c;font-size:14px}.rail button[disabled]{opacity:.35;cursor:default}.stage{position:relative;overflow:hidden;background:#e9edef;touch-action:none;cursor:grab}.stage.dragging{cursor:grabbing}.stage.plot{background:#e8ecee}
.grid{position:absolute;inset:0;opacity:.38;background-image:linear-gradient(#d3dce0 1px,transparent 1px),linear-gradient(90deg,#d3dce0 1px,transparent 1px);background-size:28px 28px;pointer-events:none}.plot .grid{background-image:linear-gradient(#d3dce0 1px,transparent 1px),linear-gradient(90deg,#d3dce0 1px,transparent 1px)}
.sheet{position:absolute;inset:24px;overflow:hidden;border:1px solid #cbd5da;box-shadow:0 12px 30px rgba(54,70,80,.16);background:var(--paper)}.plot .sheet{box-shadow:0 12px 30px rgba(54,70,80,.16)}
#drawing{width:100%;height:100%;display:block}#drawing>svg{width:100%;height:100%;display:block}.cad #drawing>svg{filter:none}.plot #drawing>svg{filter:none}
.crosshair{position:absolute;inset:0;pointer-events:none;display:none}.stage:hover .crosshair{display:block}.crosshair:before,.crosshair:after{content:"";position:absolute;background:#8caab9;opacity:.32}.crosshair:before{left:var(--mx,50%);top:0;width:1px;height:100%}.crosshair:after{top:var(--my,50%);left:0;height:1px;width:100%}
.select-box{position:absolute;border:1px solid #6e9bb3;background:rgba(94,145,169,.10);display:none;pointer-events:none}.inspector{border-left:1px solid var(--line);background:#fbfcfc;display:flex;flex-direction:column;min-width:0}.inspector header{padding:15px 14px 13px;border-bottom:1px solid var(--line)}.inspector b{display:block;font-size:12px;font-weight:600}.inspector small{display:block;margin-top:4px;color:var(--muted);line-height:1.45}.metrics{display:grid;grid-template-columns:1fr 1fr;border-bottom:1px solid var(--line)}.metrics div{padding:11px 13px;border-right:1px solid var(--line)}.metrics div:nth-child(2n){border-right:0}.metrics span{display:block;color:var(--muted);font-size:10px}.metrics strong{font-size:15px;font-weight:600;font-variant-numeric:tabular-nums}.props{padding:14px;overflow:auto}.props dl{display:grid;grid-template-columns:82px minmax(0,1fr);gap:7px 10px;margin:0}.props dt{color:var(--muted)}.props dd{margin:0;overflow-wrap:anywhere}.empty{color:var(--muted);line-height:1.6}
.status{height:31px;display:flex;gap:16px;align-items:center;padding:0 16px;border-top:1px solid var(--line);background:#fff;color:#7b8992;font-size:11px}.status span{white-space:nowrap}.status .grow{flex:1;overflow:hidden;text-overflow:ellipsis;color:#596a75}.kbd{border:1px solid #ccd7dc;border-radius:3px;padding:1px 5px;color:#63737d;background:#fafcfc}
...
<header class="topbar"><div class="identity"><div class="brand">KJ</div><div class="copy"><strong id="title"></strong><span><span class="ok">候选图纸</span><span>原图未改动</span></span></div></div><nav class="tools" aria-label="KJDraw preview controls"><button id="modeCad" type="button">工作底</button><button id="modePlot" class="on" type="button">纸张</button><span class="separator"></span><button id="fit" type="button">全图</button><button id="minus" type="button" aria-label="缩小">−</button><span id="zoom" class="zoom-readout">100%</span><button id="plus" type="button" aria-label="放大">＋</button><button id="reset" type="button">100%</button><span class="separator"></span><button id="grid" class="on" type="button">网格</button></nav></header>
<main class="workspace"><aside class="rail" aria-label="审阅工具"><button class="on" title="移动和查看" type="button">↖</button><button id="windowZoom" title="框选放大" type="button">□</button><button disabled title="测量">⌁</button></aside><section id="viewport" class="stage plot" aria-label="KJDraw 图纸预览"><div id="gridLayer" class="grid"></div><div class="sheet"><div id="drawing"></div></div><div class="crosshair"></div><div id="box" class="select-box"></div></section><aside class="inspector"><header><b>图纸信息</b><small id="reviewLine">候选文件已生成，可继续审阅</small></header><div class="metrics"><div><span>对象</span><strong id="entityCount">0</strong></div><div><span>缩放</span><strong id="zoomMetric">100%</strong></div><div><span>显示</span><strong id="modeMetric">纸张</strong></div><div><span>视图</span><strong id="viewMetric">全图</strong></div></div><section class="props" id="props"><p class="empty">点击图形查看对象属性。</p></section></aside></main>
<footer class="status"><span class="grow" id="statusTitle"></span><span id="coords">X 0.000 | Y 0.000</span><span>滚轮缩放</span><span><span class="kbd">拖动</span> 平移</span></footer>
<script>
...
let view=null, base=null, drag=null, mode='plot', windowZoom=false, boxStart=null;
function readBase(){const b=svg.viewBox.baseVal;return{x:b.x,y:b.y,width:b.width,height:b.height}}
function updateMetrics(){const percent=Math.round(base.width/view.width*100)+'%';document.getElementById('zoom').textContent=percent;document.getElementById('zoomMetric').textContent=percent;document.getElementById('modeMetric').textContent=mode==='plot'?'纸张':'工作底';document.getElementById('viewMetric').textContent=Math.abs(base.width-view.width)<base.width*.002?'全图':'局部'}
