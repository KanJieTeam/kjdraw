...
test('bootstrap validates an optional project geology pack before registry access or installation writes', async t => {
  ...
  assert.deepEqual(await readdir(project), ['geology.json'])
  const sectionMismatch = spawnSync(process.execPath, [core, ...flags(project), '--geology-section-pack', 'geology.json', '--geology-section-pack-sha256', '0'.repeat(64)], { encoding: 'utf8' })
  assert.equal(sectionMismatch.status, 1)
  assert.match(sectionMismatch.stderr, /do not match the host-supplied SHA-256/u)
  assert.deepEqual(await readdir(project), ['geology.json'])
  const sectionIncomplete = spawnSync(process.execPath, [core, ...flags(project), '--geology-section-pack', 'geology.json'], { encoding: 'utf8' })
  assert.equal(sectionIncomplete.status, 1)
  assert.match(sectionIncomplete.stderr, /path and SHA-256 must be supplied together/u)
  assert.deepEqual(await readdir(project), ['geology.json'])
})
