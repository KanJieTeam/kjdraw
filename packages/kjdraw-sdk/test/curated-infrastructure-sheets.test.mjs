...
test('curated infrastructure sheets reopen as editable CAD and render without diagnostics', async t => {
  const sheets = await buildCuratedInfrastructureSheetDocuments()
  assert.deepEqual(sheets.map(({ id }) => id), [
    'inspection-chamber', 'road-cross-section', 'box-culvert', 'bridge-pier',
  ])
  const ids = new Set()
  for (const sheet of sheets) {
    const entityCount = sheet.document.listEntities().length
    assert.equal(sheet.document.validate().valid, true, sheet.id)
    assert.ok(entityCount >= 45, sheet.id)
    ...
    assert.equal(svg.report.rendered, entityCount, sheet.id)
...
s=open(os.environ['KJDRAW_FILE_STDIN_PATH'],encoding='utf-8').read();d=ezdxf.read(io.StringIO(s));a=d.audit();print(json.dumps({'errors':len(a.errors),'fixes':len(a.fixes),'entities':sum(len(layout) for layout in d.layouts)}))`
        const result = spawnSyncWithFileStdin(
          process.env.KJDRAW_PYTHON ?? 'python', ['-c', script], bytes.toString('utf8'),
