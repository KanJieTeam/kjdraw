...
import { buildCuratedMachineComponentDocuments, buildCuratedMachineComponents } from '../../../examples/curated-machine-components.mjs'

const expectedIds = ['stepped-drive-shaft', 'keyed-hub', 'slotted-linkage', 'pillow-block-housing']
...
print(json.dumps({'errors':len(a.errors),'fixes':len(a.fixes),'dimensions':len(d.modelspace().query('DIMENSION')),'layers':sorted({e.dxf.layer for e in d.modelspace()})}))`
...
test('four original machine components remain editable across KJD, DXF, and SVG', async t => {
  const root = await mkdtemp(join(tmpdir(), 'kjdraw-machine-components-'))
  t.after(() => rm(root, { recursive: true, force: true }))
  const manifest = await buildCuratedMachineComponents(root)
  const inMemory = await buildCuratedMachineComponentDocuments()
  ...
  for (const item of manifest.cases) {
    ...
    assert.ok(original.document.listEntities({ type: 'TEXT' }).some(entity => entity.payload.text === item.title), `${item.id} has title block`)
    assert.ok(original.document.listEntities({ type: 'LWPOLYLINE' }).length >= 3, `${item.id} has native polylines`)
    ...
    for (const format of ['KJD', 'DXF']) {
      ...
      assert.equal(reopened.listEntities({ type: 'DIMENSION' }).length,
        original.document.listEntities({ type: 'DIMENSION' }).length, `${item.id}/${format}`)
  ...
  const shaft = inMemory.find(item => item.id === 'stepped-drive-shaft').document
  assert.deepEqual(shaft.listEntities({ type: 'CIRCLE' }).map(entity => entity.payload.radius).sort((a, b) => a - b), [12, 18])
  const hub = inMemory.find(item => item.id === 'keyed-hub').document
  assert.equal(hub.listEntities({ type: 'HATCH' }).length, 2)
  const linkage = inMemory.find(item => item.id === 'slotted-linkage').document
  assert.equal(linkage.listEntities({ type: 'CIRCLE' }).length, 2)
  assert.equal(linkage.listEntities({ type: 'ARC' }).length, 2)
  const housing = inMemory.find(item => item.id === 'pillow-block-housing').document
  assert.equal(housing.listEntities({ type: 'CIRCLE' }).length, 4)
})
