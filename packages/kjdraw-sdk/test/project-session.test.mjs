...
test('project session journals committed envelopes, snapshots, packages and reopens', async () => {
  ...
  session.assets.set('imports/source.dwg', new Uint8Array([65, 67, 49, 48, 51, 50]))
  session.diagnostics.set('imports/source.json', JSON.stringify({ source: 'source.dwg', warnings: 2 }))
  ...
  assert.deepEqual([...reopened.assets.get('imports/source.dwg')], [65, 67, 49, 48, 51, 50])
  assert.deepEqual(JSON.parse(new TextDecoder().decode(reopened.diagnostics.get('imports/source.json'))), { source: 'source.dwg', warnings: 2 })
