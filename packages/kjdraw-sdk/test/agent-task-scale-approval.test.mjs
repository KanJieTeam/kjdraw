...
import { commitAgentTaskScaleApproval, createAgentTask, readAgentTasks, transitionAgentTask } from '../src/agent-tasks.js'
import { KJAgentToolSession } from '../src/agent-tools.js'
import { createKJDrawSDK } from '../src/sdk.js'
import { createKjpPackage, openKjpPackage } from '../src/project-package.js'
import { stableHash } from '../src/utils.js'

const actor = { kind: 'host', id: 'atomic-scale-test' }
const at = second => `2026-09-12T12:00:${String(second).padStart(2, '0')}.000Z`
const toolNames = ['cad_check_geometry', 'cad_propose_scale']
...
async function fixture(expectedDistance = 12, toolApiVersion = '1') {
  const sdk = createKJDrawSDK()
  const document = sdk.createDocument({ documentId: `atomic-scale-${Math.random()}`, units: 'millimeter' })
  await document.transact('Seed drawing', tx => {
    tx.createEntity('LINE', { start: [2, 0, 0], end: [6, 0, 0] }, { id: 'edge' })
    tx.createEntity('LINE', { start: [0, 0, 0], end: [0, 10, 0] }, { id: 'anchor' })
    tx.createEntity('LINE', { start: [100, 0, 0], end: [110, 0, 0] }, { id: 'outside' })
  })
  const session = new KJAgentToolSession(sdk, document)
  const definition = {
    requirements: [{
      id: 'scaled_end_to_origin',
      description: 'Scale the edge so its end has the required distance from the fixed origin anchor.',
      check: {
        toolName: 'cad_check_geometry', assertion: { path: 'passed', operator: 'is_true', expected: true },
        geometryCheck: { id: 'scaled_end_to_origin', kind: 'point-distance', from: { objectId: 'edge', feature: 'end' }, to: { objectId: 'anchor', feature: 'start' }, expected: expectedDistance, tolerance: 0 },
      },
    }],
    steps: [{ id: 'scale', title: 'Scale and verify the edge', requirementIds: ['scaled_end_to_origin'] }],
  ...
  await document.transact('Create task', tx => createAgentTask(document, tx, {
    id: 'task-scale', expectedRevision: document.revision, title: 'Atomic scale task', goal: 'Scale only the edge by factor two around the origin.',
...
async function propose(value, ids = ['edge']) {
  ...
  const model = { createConversation: () => ({ next: async () => ({ text: '', calls: [{
    id: 'scale-edge', name: 'cad_propose_scale', arguments: { expectedRevision: revision, units: 'millimeter', ids, center: { x: 0, y: 0 }, factor: 2 },
  }] }) }) }
...
async function directProposal(value, ids = ['edge']) {
  const result = await value.session.call('cad_propose_scale', { expectedRevision: value.document.revision, units: 'millimeter', ids, center: { x: 0, y: 0 }, factor: 2 })
...
function approval(value, overrides = {}) {
  ...
  return {
    ...
    sourceToolName: 'cad_propose_scale', toolApiVersion: task.definition.tools.apiVersion, toolContractHash: task.definition.tools.contractHash,
    argumentsDigest: stableHash({ ids: ['edge'], center: [0, 0], factor: 2 }), capabilityLocks: [], planId: 'plan-direct', executionEnvelopeId: 'envelope-direct',
    reviewerId: 'reviewer', scaledEntityIds: ['edge'], at: at(4), ...overrides,
  }
}

test('persisted SCALE commits exact reviewed geometry, checks and task completion in one undo unit', async () => {
  ...
  assert.equal(approved.value.command, 'SCALE')
  assert.equal(value.document.revision, revision + 1)
  assert.deepEqual(value.document.getObject('edge').payload.start, [4, 0, 0])
  assert.deepEqual(value.document.getObject('edge').payload.end, [12, 0, 0])
  assert.deepEqual(value.document.getObject('anchor').payload.end, [0, 10, 0])
  ...
  assert.equal(task.receipts[0].command, 'SCALE')
  assert.equal(task.receipts[0].sourceToolName, 'cad_propose_scale')
  assert.equal(task.receipts[0].argumentsDigest, stableHash(proposed.outputs[0].result.value.arguments))
  assert.equal(task.receipts[0].checks[0].actual, 12)
  ...
  assert.deepEqual(value.document.getObject('edge').payload.end, [6, 0, 0])
  ...
  const kjp = await createKjpPackage({ projectId: 'atomic-scale-project', drawings: { [value.document.id]: value.document }, activeDrawing: value.document.id, createdAt: at(4), modifiedAt: at(4) })
  assert.deepEqual(readAgentTasks((await openKjpPackage(kjp)).activeDocument), readAgentTasks(value.document))
})

test('SCALE check failure rolls back geometry, task, revision and history without consuming the plan', async () => {
  const value = await fixture(11), planId = (await propose(value)).proposalIds[0]
...
test('SCALE refuses out-of-scope targets, revision drift and command replacement', async () => {
  ...
  replaced.sdk.commands.register({ id: 'SCALE', execute() { throw new Error('must not execute') } }, { replace: true, owner: 'forged' })
...
test('SCALE bindings and atomic commit enforce tool API, contract, capability and exact changed IDs', async () => {
  const mutations = [
  ...
  const unsupported = await fixture(12, '999'), unsupportedPlan = await directProposal(unsupported), unsupportedBefore = unsupported.document.serialize()
  ...
  const direct = await fixture(12, '999'), directBefore = direct.document.serialize(), directRevision = direct.document.revision
  await assert.rejects(direct.document.transact('Forged atomic SCALE', tx => commitAgentTaskScaleApproval(direct.document, tx, approval(direct))), /unsupported persistent task tool API version/)
  ...
  await assert.rejects(extra.document.transact('Scale extra entity', async tx => {
    tx.updateObject('edge', { payload: { start: [4, 0, 0], end: [12, 0, 0] } })
    tx.updateObject('outside', { payload: { start: [200, 0, 0], end: [220, 0, 0] } })
    await commitAgentTaskScaleApproval(extra.document, tx, approval(extra))
  }), /must change exactly/)
  assert.equal(extra.document.serialize(), extraBefore)
  assert.equal(extra.document.revision, extraRevision)
})

test('authoritative SCALE uncertainty rolls back local state and forbids replay', async () => {
