...
test('public synthetic DXF corpus passes every declared target version', async () => {
  ...
  assert.equal(report.passedCount, 8, JSON.stringify(report.cases.filter(row => !row.passed), null, 2))
  assert.equal(report.passed, true)
  assert.deepEqual(report.cases.map(row => row.requestedVersion), ['R14', '2000', '2004', '2007', '2010', '2013', '2018', '2024'])
