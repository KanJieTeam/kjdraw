...
test('workbench exposes twelve useful tools and creates ordinary geometry through pattern arrays=[]', async () => {
  const { session, document } = fixture()
  assert.ok(Object.isFrozen(KJDRAW_CHAT_TOOL_NAMES))
  assert.equal(KJDRAW_CHAT_TOOL_NAMES.length, 12)
  assert.deepEqual(KJDRAW_CHAT_TOOL_NAMES.filter(name => name.startsWith('cad_propose_')), ['cad_propose_move', 'cad_propose_rotate', 'cad_propose_scale', 'cad_propose_polyline_edit', 'cad_propose_drawing_pattern', 'cad_propose_drawing_annotated'])
