...
test('nested INSERT masks reuse source geometry and isolate moved instances and changed block contents', async () => {
  ...
  try {
    ...
    assert.equal(created.length, 3, 'copy-on-write retains the unchanged instance mask and rebuilds only the moved projection')
    ...
    renderer.render(); assert.equal(created.length, 3)
    ...
    assert.equal(created.length, 5, 'changed source payload invalidates both projected instances')
    renderer.render(); assert.equal(created.length, 5)
    renderer.setDocument(document)
    assert.equal(created.length, 7, 'changing the document binding clears projected masks')
    renderer.dispose()
  } finally { if (Native === undefined) delete globalThis.OffscreenCanvas; else globalThis.OffscreenCanvas = Native }
})
