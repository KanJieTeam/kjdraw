...
test('Kimi-safe profile exposes a bounded core and rejects hidden full-profile calls', async t => {
  ...
  assert.ok(names.includes('cad_propose_geology_plan'))
...
test('MCP host hash-locks a geology section knowledge pack while ordinary model input remains facts only', async t => {
  ...
  assert.equal(ledger.proposals[0].result.arguments.entities.filter(entity => entity.type === 'TEXT' && /^2024-01-0[23]$/u.test(entity.payload.text)).length, 2)
  ...
  assert.equal(ledger.proposals[0].result.arguments.entities.some(entity => entity.type === 'TEXT' && entity.payload.text === '6.00-99.13'), true)
