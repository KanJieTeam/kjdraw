...
async function fixture(t) {
  ...
  for (let repetition = 1; repetition <= 5; repetition++) for (const task of tasks) for (const [order, arm] of ['kjdraw-tool', 'direct-dxf'].entries()) {
    ...
    const usage = { protocol: 'chat-completions', inputTokens: 100, outputTokens: arm === 'kjdraw-tool' ? 20 : 40, totalTokens: arm === 'kjdraw-tool' ? 120 : 140, cacheReadInputTokens: 60, cacheMissInputTokens: 40, invalidFields: [] }
...
test('complete evidence renders three accessible SVGs, retains failed denominators and computes reported sums and medians', async t => {
  ...
  assert.match(await readFile(join(f.output, 'geometry.svg'), 'utf8'), /14\/15 \(93.3%\)/)
...
test('missing tokens remain null and block comparison; missing optional cache split remains explicitly unknown', async t => {
  ...
  assert.equal(missing.status, 'not-comparable'); assert.equal(missing.summaries['kjdraw-tool'].inputTokens, null)
  ...
  assert.match(await readFile(join(optional.output, 'tokens.svg'), 'utf8'), /cache hit: unknown/)
})
