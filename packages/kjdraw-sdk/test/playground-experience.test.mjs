...
test('playground exposes the deterministic 90-second review workflow', async () => {
  ...
  assert.match(translations, /Review drawing changes/)
  assert.match(translations, /审核图纸修改/)
  assert.match(html, /id="sample-select"/)
  assert.match(html, /data-panel-view="properties"/)
  assert.match(html, /data-panel-view="agent"/)
  assert.match(app, /INITIAL_QUERY\.get\('sample'\)/)
  assert.match(app, /documents\.some\(drawing=>drawing\.id===REQUESTED_SAMPLE_ID\)/)
  assert.match(app, /url\.searchParams\.set\('sample',id\)/)
  assert.match(app, /history\.replaceState\(null,'',url\)/)
