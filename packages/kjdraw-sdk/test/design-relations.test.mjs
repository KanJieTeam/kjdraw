...
test('style edits remain compatible, while protected layers, erased members and unit changes reject atomically', async () => {
  ...
  before = document.serialize()
  await assert.rejects(sdk.executeCommand('ERASE', { ids: ['h2'] }), /must remove design relation .* before erasing/)
  assert.equal(document.serialize(), before)
  await sdk.executeCommand('DESIGNDELETE', { id: design.id })
  assert.deepEqual(readDesignRelations(document), [])
  ...
  before = document.serialize()
  await assert.rejects(sdk.executeCommand('DESIGNUPDATE', { id: design.id, parameters: { width: 400 } }), /does not exist/)
  assert.equal(document.serialize(), before)
})

test('DESIGNDELETE keeps geometry stable, is undoable, and removes the DXF flattening requirement', async () => {
  const { sdk, document, design } = await fixture()
  const geometry = document.listEntities(), revision = document.revision
  await sdk.executeCommand('DESIGNDELETE', { id: design.id })
  assert.equal(document.revision, revision + 1)
  assert.deepEqual(document.listEntities(), geometry)
  assert.deepEqual(readDesignRelations(document), [])
  await sdk.writeDocument(document, { format: 'DXF', version: '2018' })
  await sdk.executeCommand('UNDO')
  assert.deepEqual(document.listEntities(), geometry)
  assert.deepEqual(readDesignRelations(document).map(item => item.id), [design.id])
