...
test('DXF standard resource tables and entity style references survive write and reopen', async () => {
  ...
  const textStyle = await sdk.executeCommand('TEXTSTYLE', { name: 'KJ-CN', fontFile: 'simsun.ttc', bigFontFile: 'hztxt.shx', widthFactor: 0.8, obliqueAngle: 0.1, current: true })
  ...
  const artifact = await sdk.writeDocument(document, { format: 'DXF', version: '2018' })
  assert.match(artifact, /\$DWGCODEPAGE\r\n\s*3\r\nUTF-8\r\n/)
  assert.match(artifact, /\$TEXTSTYLE\r\n\s*7\r\nKJ-CN\r\n/)
  ...
  assert.equal(reopened.getTable('textStyles').currentId, reopenedTextStyle.id)
