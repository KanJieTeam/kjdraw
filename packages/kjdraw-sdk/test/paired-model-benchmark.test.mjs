...
test('fixture paired run uses real HTTP, SDK materialization and independent ezdxf validation for all 30 requests', async t => {
  ...
  assert.equal(report.runs.every(run => run.failure === null && run.humanInterventionCount === 0), true)
  assert.equal(report.runs.filter(run => run.arm === 'kjdraw-tool').every(run => run.harnessProposalApprovalCount === 1), true)
  assert.equal(report.runs.filter(run => run.arm === 'direct-dxf').every(run => run.harnessProposalApprovalCount === 0), true)
  assert.equal(report.summary['kjdraw-tool'].humanInterventionCount, 0)
  assert.equal(report.summary['kjdraw-tool'].failures, 0)
  assert.ok(report.summary['kjdraw-tool'].totalMs > 0)
