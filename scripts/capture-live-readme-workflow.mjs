...
export async function runLiveReadmeWorkflow(configuration) {
  ...
  try {
    ...
    const firstEvidence = page.locator('.chat-manufacturing-evidence').last()
    await page.waitForFunction(() => document.querySelector('.chat-manufacturing-evidence') || document.querySelector('#chat-input')?.disabled === false)
    if (!(await firstEvidence.isVisible())) throw new Error(`Model generation failed before producing a manufacturing proposal: ${(await page.locator('.chat-message.assistant').last().textContent())?.trim() ?? 'unknown error'}`)
    ...
    const proposalCount = await page.locator('.chat-proposal-state').count()
    ...
    const proposals = page.locator('.chat-proposal-state')
    await page.waitForFunction(count => document.querySelectorAll('.chat-proposal-state').length > count || document.querySelector('#chat-input')?.disabled === false, proposalCount)
    if (await proposals.count() <= proposalCount) throw new Error(`Model modification failed before producing a move proposal: ${(await page.locator('.chat-message.assistant').last().textContent())?.trim() ?? 'unknown error'}`)
    await page.waitForFunction(() => document.querySelector('#chat-input')?.disabled === false); await settleResponses()
