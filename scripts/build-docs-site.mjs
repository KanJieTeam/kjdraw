...
function renderPage(page, index) {
  ...
  const homeActions = atRoot ? `<div class="home-actions"><a class="primary" href="./quickstart/">${localized('Start building', '开始构建')}</a><a href="./showcase/">${localized('Explore examples', '浏览案例')}</a></div>` : ''
  const homePreview = atRoot ? `<section class="home-entry-section" aria-label="KJDraw entry points"><div class="home-entry-heading"><h2>${localized('Choose your starting point', '选择你的起点')}</h2><p>${localized('One editable document, three direct ways in.', '一份可编辑图档，三种直接的接入方式。')}</p></div><div class="home-entry-grid"><a href="./quickstart/"><span>01 / EDITOR</span><strong>${localized('Start with an editor', '从编辑器开始')}</strong><small>${localized('Mount and edit in five minutes', '五分钟挂载并编辑')}</small></a><a href="./api/"><span>02 / SDK</span><strong>${localized('Build into your product', '接入自己的产品')}</strong><small>${localized('Typed commands and native files', '类型化命令与原生图档')}</small></a><a href="./mcp/"><span>03 / AGENTS</span><strong>${localized('Connect an AI agent', '连接 AI 智能体')}</strong><small>${localized('Propose, review and verify', '提案、审核与核验')}</small></a></div></section>` : ''
  const homeDemo = atRoot ? `<section class='home-live-demo' aria-label='Interactive KJDraw editor'><div class='home-live-heading'><div><span class='home-live-label'>KJDRAW / LIVE EDITOR</span><h2>${localized('CAD you can actually edit', '可以直接编辑的 CAD')}</h2><p>${localized('Select an object, change it, then undo. This is the real browser editor, not a video.', '选择对象、修改，再撤销。这里是真实的浏览器编辑器，不是演示视频。')}</p></div><a href='../../?sample=specimen-editable-entities&amp;layout=focus' target='_blank' rel='noopener'>${localized('Open full editor', '打开完整编辑器')} ↗</a></div><div class='home-live-frame'><iframe src='../../?sample=specimen-editable-entities&amp;layout=focus' title='KJDraw editable CAD workspace' loading='lazy'></iframe></div></section>` : ''
    ...
    <main>
      <div class="content">
        <article class="lang-en" lang="en" id="en-${page.slug}">
          ${atRoot ? `<header class="home-hero"><p class="eyebrow">KJDraw · Open-source engineering CAD</p><h1>${localized('Build CAD on your terms.', '下一代工程 CAD，由你构建。', 'span')}</h1><p class="lead">${localized('Create, edit and verify engineering drawings in your app, browser or AI workflow.', '在应用、浏览器或 AI 工作流中创建、编辑并核验工程图纸。')}</p>${homeActions}</header>${homeDemo}${homePreview}` : `<header class="page-hero"><p class="eyebrow">KJDRAW / ${escapeHtml(page.title.en.toUpperCase())}</p><h1>${escapeHtml(page.title.en)}</h1><p class="lead">${escapeHtml(page.summary.en)}</p></header>`}${portal('en') ? `\n          ${portal('en')}` : ''}
          ${pageBody('en')}
        </article>
        <article class="lang-zh" lang="zh-CN" id="zh-${page.slug}">
          ${atRoot ? `<header class="home-hero"><p class="eyebrow">KJDraw · 开源工程 CAD</p><h1>${localized('Build CAD on your terms.', '下一代工程 CAD，由你构建。', 'span')}</h1><p class="lead">${localized('Create, edit and verify engineering drawings in your app, browser or AI workflow.', '在应用、浏览器或 AI 工作流中创建、编辑并核验工程图纸。')}</p>${homeActions}</header>${homeDemo}${homePreview}` : `<header class="page-hero"><p class="eyebrow">KJDRAW / ${escapeHtml(page.title.zh)}</p><h1>${escapeHtml(page.title.zh)}</h1><p class="lead">${escapeHtml(page.summary.zh)}</p></header>`}${portal('zh') ? `\n          ${portal('zh')}` : ''}
          ${pageBody('zh')}
        </article>
...
const homeStyle = `
.home-hero{min-height:410px;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:76px 24px 66px;text-align:center;background:#fff}
.home-hero .eyebrow{margin:0 0 19px;color:#60716a;font-size:11px;font-weight:600;letter-spacing:.04em;text-transform:none}
.home-hero h1{max-width:1040px;margin:0;color:#19231f;font-size:clamp(44px,4.6vw,66px);font-weight:600;line-height:1.08;letter-spacing:-.052em}
.home-hero .lead{max-width:670px;margin:20px 0 0;padding:0;border:0;color:#5f6b67;font-size:17px;line-height:1.65}
.home-hero .home-actions{justify-content:center;margin-top:28px}
.home-hero .home-actions a{padding:11px 18px;border-color:#cfd8d2;background:#fff;color:#25362e}
.home-hero .home-actions a.primary{border-color:#1e3227;background:#1e3227;color:#fff}
.home-hero .home-actions a:hover,.home-hero .home-actions a:focus-visible{border-color:#317451;box-shadow:0 0 0 3px #8fcca63b;outline:0}
.home-live-demo{width:min(1120px,calc(100% - 48px));margin:0 auto 0}
.home-live-heading{display:flex;align-items:end;justify-content:space-between;gap:28px;margin-bottom:19px}
.home-live-label{display:block;margin:0 0 8px;color:#47735b;font:600 10px/1.4 Consolas,monospace;letter-spacing:.11em}
.home-live-heading h2{margin:0 0 7px;color:#19231f;font-size:27px;font-weight:600;letter-spacing:-.035em}
.home-live-heading p{max-width:630px;margin:0;color:#66736d;font-size:14px;line-height:1.6}
.home-live-heading>a{flex:0 0 auto;color:#23563b;font-size:13px;font-weight:600;text-decoration:none}
.home-live-heading>a:hover,.home-live-heading>a:focus-visible{text-decoration:underline;text-underline-offset:3px}
.home-live-frame{height:590px;overflow:hidden;border:1px solid #d2dbd6;border-radius:8px;background:#111920;box-shadow:0 12px 38px #172b2022}
.home-live-frame iframe{display:block;width:100%;height:100%;border:0}
.home-entry-section{width:min(1120px,calc(100% - 48px));margin:96px auto 110px}
.home-entry-heading{margin-bottom:24px}
.home-entry-heading h2{margin:0 0 7px;color:#19231f;font-size:28px;font-weight:600;letter-spacing:-.035em}
.home-entry-heading p{margin:0;color:#66736d;font-size:14px;line-height:1.6}
.home-entry-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));border-top:1px solid #dce4df;border-bottom:1px solid #dce4df}
.home-entry-grid a{min-width:0;display:flex;flex-direction:column;gap:9px;padding:27px 26px 30px;color:#202d25;text-decoration:none}
.home-entry-grid a+a{border-left:1px solid #dce4df}
.home-entry-grid a:hover,.home-entry-grid a:focus-visible{background:#f6f9f7;outline:0}
.home-entry-grid a>span{color:#5d7465;font:600 10px/1.4 Consolas,monospace;letter-spacing:.08em}
.home-entry-grid strong{font-size:17px;font-weight:600}
.home-entry-grid small{color:#68766e;font-size:13px;line-height:1.5}
@media(max-width:780px){
.home-hero{min-height:380px;padding:64px 22px 58px}
.home-hero h1{font-size:clamp(38px,9vw,52px)}
.home-hero .lead{font-size:15px}
.home-live-demo,.home-entry-section{width:calc(100% - 36px)}
.home-live-heading{align-items:start;flex-direction:column;gap:12px}
.home-live-heading h2{font-size:23px}
.home-live-heading p{font-size:13px}
.home-live-frame{height:510px}
.home-entry-section{margin:68px auto 74px}
.home-entry-heading h2{font-size:24px}
.home-entry-grid{grid-template-columns:1fr}
.home-entry-grid a{padding:20px 8px}
.home-entry-grid a+a{border-top:1px solid #dce4df;border-left:0}
}
`
const showcaseStyle = `
...
.showcase-thumb img{display:block;width:100%;height:100%;object-fit:contain;filter:invert(1) hue-rotate(180deg)}
