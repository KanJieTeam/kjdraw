...
function renderPage(page, index) {
  ...
  const homeContent = atRoot ? `<header class="home-hero">
    <p class="eyebrow">KJDraw · Open-source engineering CAD</p>
    <h1>${localized('One editable model for every engineering workflow.', '一份可编辑图档，贯通每一种工程工作流。')}</h1>
  ...
  <section class="home-story home-examples"><div class="home-section-intro"><p class="eyebrow">02 / EXPLORE</p><h2>${localized('Different disciplines. The same editable core.', '不同专业，同一套可编辑内核。')}</h2><p>${localized('Open a real case, inspect its objects, then continue editing in the Playground.', '打开真实案例，检查对象，再到工作台继续编辑。')}</p></div><div class="home-example-grid"><a href="./showcase/mechanical-bracket/"><img src="./showcase/assets/mechanical-bracket.svg" alt="" loading="lazy"><span>${localized('Mechanical part', '机械零件')}</span><b aria-hidden="true">↗</b></a><a href="./showcase/geology-section/"><img src="./showcase/assets/geology-section.svg" alt="" loading="lazy"><span>${localized('Geological section', '地质剖面图')}</span><b aria-hidden="true">↗</b></a><a href="./showcase/site-plan/"><img src="./showcase/assets/site-plan.svg" alt="" loading="lazy"><span>${localized('Site plan', '场地平面图')}</span><b aria-hidden="true">↗</b></a></div><a class="home-text-link" href="./showcase/">${localized('Browse all examples', '浏览全部案例')} <span aria-hidden="true">→</span></a></section>
...
html[data-page="introduction"] .home-hero .eyebrow{margin:0 0 28px;color:#586579;font:500 14px/1.4 Inter,"Segoe UI",sans-serif;letter-spacing:.01em}
html[data-page="introduction"] .home-hero h1{max-width:940px;margin:0;color:#142039;font-size:clamp(42px,4.4vw,64px);font-weight:650;line-height:1.08;letter-spacing:-.045em;text-wrap:balance}
html[data-page="introduction"] .home-hero .lead{max-width:820px;margin:20px 0 0;padding:0;border:0;color:#53627a;font-size:clamp(16px,1.5vw,21px);line-height:1.5}
