...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (!['--project', '--blank', '--candidate-sha', '--integrity', '--units'].includes(key) || values.has(key) || !argv[i + 1] || argv[i + 1].startsWith('--')) fail('Unknown, duplicate or incomplete installer option')
