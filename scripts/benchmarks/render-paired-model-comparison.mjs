...
export async function renderPairedModelComparison({ input, output } = {}) {
    ...
    try {
      for (const key of ['request', 'response', 'dxf']) {
        ...
        if (key === 'request') {
          ...
          const validToolChoice = toolChoiceMode === 'auto' ? request.tool_choice === 'auto' : toolChoiceMode === 'required' ? request.tool_choice === 'required' : request.tool_choice?.type === 'function' && request.tool_choice?.function?.name === drawingTool
  ...
  const toolChoiceNote = toolChoiceMode === 'auto' ? 'Auto: the model may choose whether to call the one supplied CAD tool; failed or declined generations remain in the denominator.' : toolChoiceMode === 'required' ? 'Required: the provider must return at least one call to the one supplied CAD tool; KJDraw still requires exactly one valid proposal.' : 'Forced: the request names the one supplied CAD tool; this constrains tool selection. Legacy reports with no mode field use forced.'
  const result = { schema: 'com.kanjie.kjdraw.benchmark.comparison@1', status: reasons.size ? 'not-comparable' : 'comparable', reasons: [...reasons], publicationReviewRequired: true, independentValidationRerun: false, integrityNote: 'Artifact hashes match the supplied report; this does not authenticate the provider or rerun geometry validation.', reportSha256: sha(bytes), requestedModel: report.model, returnedModel: [...returned][0] ?? null, createdAt: report.createdAt, settings: report.settings, drawingTool, taskSuite, scopeNote, toolChoiceMode, toolChoiceNote, source: report.source, validator: report.validator, repetitions: report.repetitions, samplesPerArm: planned / 2, summaries, perTask, charts: [], scope: `${scopeNote} All attempted runs, including failures, remain in denominators and token totals. This one-shot experiment does not establish equal quality or general CAD capability.` }
