...
Select objects by clicking their geometry. Use L to draw a line, C for a circle, V for selection, and Escape to cancel. Scroll to zoom and middle-drag to pan. Snaps use SDK endpoint/midpoint/center/nearest queries. The minimal viewer intentionally excludes complex text formatting, blocks, layouts and several entity types; unsupported view entities remain in the document.
...
See `examples/agent-command.mjs` for command envelopes. `MOVE` accepts `dx` and `dy` (or `from` and `to`), not a `delta` array. Public exports are listed in `packages/kjdraw-sdk/package.json` and `src/index.js`.
