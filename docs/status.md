...
- The source-tree `agent-tools` entry provides six model-neutral starter tool definitions with runtime validation, bound-document queries/measurement and line/circle/move proposals. Approval is a separate trusted-host method; broader rendered previews, durable task recovery and real model-task verification remain incomplete.
- The source-tree `model-adapters` and `agent-runner` entries provide four non-streaming wire adapters, a custom model interface and a bounded proposal loop. Protocol fixtures and packed consumers verify the same CAD workflow across adapters; no live model is certified by those tests. The [model integration guide](https://kanjieteam.github.io/kjdraw/docs/latest/models/) explains host-owned transport, budgets and approval.
