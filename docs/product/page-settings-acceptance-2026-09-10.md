...
## Follow-up: plot windows, named-view references and fit scale

After the layout-query increment, the page form also edits plot area (display/extents/limits/named view/window/layout), four window coordinates, named-view reference and physical origin offsets. A scale selector supports keeping existing settings, fit to paper, or custom ratio. Fit writes standard scale type 0 and sets bit 16; custom clears only that bit. Editing a custom ratio selects custom mode; fit disables custom inputs without discarding stored values. Fields for an inactive plot area are disabled and are not submitted. Switching area preserves unused imported window/view values.

PAGESETUP now validates explicit window activation/coordinate edits against the merged settings: all four coordinates must exist and width/height must be positive. Explicit named-view activation/name edits require a nonblank name. Invalid commands reject atomically. Unrelated edits do not reinterpret preexisting imported settings. The form retains revision checks, readonly handling and no-op/cancel behavior.

Validation: Node 22 and Node 24 **407/407 each**; Chromium, Firefox and WebKit **21/21** page/editor/query/form cases. Two added real-control cases verify window values, origin offsets, fit/custom flags, unchanged geometry/other layouts, undo/redo, no-op reopen, DXF reopening, AI layout-tool readback, missing/reversed windows and required named-view input. Existing narrow Chinese keyboard, readonly and stale-revision cases also pass. Strict types, generated outputs and repository checks pass.

The independent ezdxf 1.4.4 audit additionally checks edited windows, fit flag/type and origin offsets across all 30 fields in three layouts, including undo/redo and zero audit errors/repairs. Named-view configuration records a reference only; it does not resolve or render the referenced view. Fit settings do not constitute a computed print preview or a physical printer run. Complete viewport/output rendering and the stable 1.0 gates remain open.

## Follow-up: host form isolation

The full remote CI for `57a98d5` failed in all three browsers on the existing `allButtonsAreSafe` assertion (240 other cases passed). Page setup introduced a nested form and a submit button inside a workbench that may itself be mounted inside a host form. The fix uses a local container and explicit button/Enter handlers, validates only its own controls, and suppresses repeated application while an edit is pending. The original safety assertion remains unchanged.

Chromium workbench/page-form regression: **13/13**. A new case places an invalid required input in the host form and verifies that both click and Enter can apply valid page settings without submitting or validating that outer form. Invalid local fields keep the dialog open. Existing keyboard, stale revision, readonly, DXF reopen and undo/redo cases pass. The five core page-setting tests and strict TypeScript/generated checks also pass. At the user's request, the full three-browser acceptance is deferred until review of the accumulated functional changes; this record does not claim the previous remote CI failure has already turned green.
