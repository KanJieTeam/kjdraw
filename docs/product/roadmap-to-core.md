...
| W07 CAD agent tools | Versioned tool definitions and runtime validation, explicit units/coordinates, bounded context, task-sized operations and useful errors | Starter session and four protocol adapters implemented in source: read/page/measure and line/circle/move proposals, with host-only approval. Live provider acceptance and geometric preview expansion remain | Discover tools without reading internals; query, propose and validate supported geometry |
