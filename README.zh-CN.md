...
大模型已经能写代码、做文档，也能生成几何。但工程图纸不是一堆互不相干的基础图元：一张图里有数百个关联对象，模型必须在连续对话中持续找到同一个对象，让尺寸、图层、块、填充、引用和它们之间的关系，在撤销、导出、保存、重开之后仍然一致。

让 LLM 逐个输出坐标和基础图元，会同时带来四个问题：

- token 消耗随图纸规模增长；
- 复杂几何和工程约束在多轮对话中漂移；
- 对象没有稳定身份，下一次修改只能靠猜；
- 渲染结果看起来正确，也不能证明图纸仍然可编辑、可审计。
...
KJDraw 不是再训练一个只会画几类图的模型，而是给不同模型和智能体提供同一套 CAD 执行层：模型表达“要画什么”，本地引擎确定性地解决“如何画对”，并负责几何、对象身份、图层、引用、事务、验证和文件输出。
