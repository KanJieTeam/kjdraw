...
## AI 画出第一版不难，难的是第十轮修改后仍然正确

生成一张“看起来像 CAD”的图已经不难。工程 CAD 真正的门槛是：在一张包含数百个关联对象的图纸中，让模型连续多轮都能找到同一个对象，保持尺寸、图层、块、填充和引用关系正确，并在撤销、导出、保存、重开后得到一致结果。

让 LLM 逐个输出坐标和基础图元，会迅速消耗 token；复杂几何与工程约束容易漂移；对象没有稳定身份，下一轮修改只能“猜”；即使画面看起来正确，也很难证明它仍是可编辑、可审计的 CAD。
...
KJDraw 不是再训练一个只会画几类图的模型，而是给不同模型和智能体提供同一套 CAD 执行层：模型负责“画什么”，引擎负责“怎么正确地画”。模型只提交工程意图、事实和约束；KJDraw 在本地确定性完成几何、对象身份、图层、引用、事务、验证和文件输出。
